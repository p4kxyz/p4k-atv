...
### 1. Command cơ bản:
```bash
adb shell am start -n com.files.codes/.view.SearchActivity --es query "Avatar"
```

### 2. Các biến thể command có thể sử dụng:
```bash
# Sử dụng tham số "query" 
adb shell am start -n com.files.codes/.view.SearchActivity --es query "Avengers"

# Sử dụng tham số "search_query" (backup)
adb shell am start -n com.files.codes/.view.SearchActivity --es search_query "Spider-Man"

# Mở SearchActivity không có query (sẽ hiển thị màn hình search trống)
adb shell am start -n com.files.codes/.view.SearchActivity
```
...
# Tìm phim Việt Nam
adb shell am start -n com.files.codes/.view.SearchActivity --es query "Cô Gái Đến Từ Hôm Qua"

# Tìm phim Marvel
adb shell am start -n com.files.codes/.view.SearchActivity --es query "Iron Man"

# Tìm phim hoạt hình
adb shell am start -n com.files.codes/.view.SearchActivity --es query "Frozen"
```

### 4. Lưu ý khi sử dụng:
- Package name hiện tại: `com.files.codes`
...
adb logcat | grep "com.files.codes"
```
