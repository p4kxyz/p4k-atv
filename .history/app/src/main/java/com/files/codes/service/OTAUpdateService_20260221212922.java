...
public class OTAUpdateService extends Service {
    ...
    public void checkForUpdates() {
        ...
        // Remove legacy skip preference (feature removed)
        PreferenceUtils.saveString(context, "ota_skipped_version", "");

        // Set a 15-second timeout in case Firebase doesn't respond
        if (timeoutRunnable != null) mainHandler.removeCallbacks(timeoutRunnable);
        timeoutRunnable = () -> {
            if (isChecking) {
                isChecking = false;
                Log.e(TAG, "⏰ Firebase TIMEOUT - no response after 15 seconds");
                if (updateListener != null) {
                    updateListener.onUpdateCheckFailed("Firebase timeout - no response");
                }
            }
        };
        ...
        updateRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                isChecking = false;
                if (timeoutRunnable != null) mainHandler.removeCallbacks(timeoutRunnable);
                Log.e(TAG, "❌ Firebase update check cancelled: " + error.getMessage());
                if (updateListener != null) {
