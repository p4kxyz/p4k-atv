...
public class OTAUpdateService extends Service {
    ...
    private UpdateInfo parseUpdateInfoFromJson(JSONObject obj) throws Exception {
        ...
        // changelog có thể là String hoặc Object {"vi": "...", "en": "..."}
        Object changelogRaw = obj.opt("changelog");
        if (changelogRaw instanceof JSONObject) {
            JSONObject changelogObj = (JSONObject) changelogRaw;
            info.changelog = changelogObj.optString("vi", changelogObj.optString("en", ""));
        } else if (changelogRaw instanceof String) {
            info.changelog = (String) changelogRaw;
        } else {
            info.changelog = "";
        }
