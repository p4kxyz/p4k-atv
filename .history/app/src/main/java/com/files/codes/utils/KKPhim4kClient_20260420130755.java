...
public class KKPhim4kClient {
    ...
    public void searchMovies(String keyword, KKPhim4kCallback callback) {
        ...
        Call<KKPhim4kSearchResponse> call = apiService.searchMovies(keyword);
        call.enqueue(new Callback<KKPhim4kSearchResponse>() {
            @Override
            public void onResponse(Call<KKPhim4kSearchResponse> call, Response<KKPhim4kSearchResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    KKPhim4kSearchResponse searchResponse = response.body();
                    if ("success".equals(searchResponse.getStatus()) && searchResponse.getData() != null) {
                        List<VideoContent> videos = convertPhim4kMoviesToVideoContents(searchResponse.getData().getItems());
                        // Log.d(TAG, "Search found " + videos.size() + " movies on KKPhim4k");
                        callback.onSuccess(videos);
                    } else {
                        callback.onError("API returned unsuccessful status: " + searchResponse.getStatus());
                    }
                } else {
                    callback.onError("Response failed: " + response.code());
                }
            ...
            @Override
            public void onFailure(Call<KKPhim4kSearchResponse> call, Throwable t) {
    ...
    private VideoContent convertPhim4kMovieToVideoContent(Phim4kMovie movie) {
        ...
        // Fix image URLs (prepend domain if relative path)
        String thumbUrl = movie.getThumbUrl();
        if (thumbUrl != null && !thumbUrl.startsWith("http")) {
            if (thumbUrl.startsWith("/")) {
                thumbUrl = "https://kk.phim4k.lol/img" + thumbUrl;
            } else {
                thumbUrl = "https://kk.phim4k.lol/img/" + thumbUrl;
            }
        }
        ...
        if (posterUrl != null && !posterUrl.startsWith("http")) {
            if (posterUrl.startsWith("/")) {
                posterUrl = "https://kk.phim4k.lol/img" + posterUrl;
            } else {
                posterUrl = "https://kk.phim4k.lol/img/" + posterUrl;
            }
        }
