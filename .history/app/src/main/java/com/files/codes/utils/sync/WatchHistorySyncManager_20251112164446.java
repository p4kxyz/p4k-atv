...
public class WatchHistorySyncManager {
    ...
    /**
     * Force sync lịch sử xem lên server (được gọi từ PlayerActivity)
     * Method này sẽ đồng bộ ngay lập tức mà không cần callback
     */
    public void forceSyncToServer() {
        if (!canAutoSync()) {
            Log.d(TAG, "forceSyncToServer: Auto sync not available, skipping");
            return;
        }
        
        Log.d(TAG, "forceSyncToServer: Starting force sync to server");
        
        // Gọi syncWatchHistoryToServer với callback đơn giản
        syncWatchHistoryToServer(new SyncCallback() {
            @Override
            public void onSuccess(String message) {
                Log.d(TAG, "forceSyncToServer: Successfully synced - " + message);
            }
            
            @Override
            public void onError(String error) {
                Log.w(TAG, "forceSyncToServer: Failed to sync - " + error);
            }
        });
    }
}