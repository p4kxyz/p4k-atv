...
public class WatchHistorySyncManager {
    ...
    /**
     * Thêm hoặc cập nhật một mục lịch sử xem với auto-detect (phương thức đơn giản)
     */
    ...
    /**
     * Method đơn giản với auto-detect - chỉ cần truyền thông tin cơ bản
     */
    public void addWatchHistoryItemSimple(String videoId, String title, String posterUrl, 
                                         long currentPosition, long totalDuration) {
        addWatchHistoryItemWithMetadata(videoId, title, "", posterUrl, posterUrl, "", 
                                      currentPosition, totalDuration, "auto-detect", "", "", "", "", "");
    }
    ...
    /**
     * Tự động detect và fetch metadata để xác định phim lẻ/phim bộ
     */
    private void autoDetectAndFetchMetadata(String videoId, String title, 
                                           WatchHistorySyncItem.WatchHistoryItem historyItem,
                                           Map<String, WatchHistorySyncItem.WatchHistoryItem> localHistory) {
        
        Log.e(TAG, "autoDetectAndFetchMetadata: Starting auto-detection for " + videoId);
        
        // Tự động xác định type từ videoId pattern
        String detectedType = detectContentTypeFromId(videoId, title);
        
        if ("tvseries".equals(detectedType)) {
            // Phim bộ - set isTvSeries = "1" 
            historyItem.setIsTvSeries("1");
            Log.e(TAG, "Auto-detected as TV Series: " + title);
            
            // Thử parse episode info từ title
            parseEpisodeInfoFromTitle(title, historyItem);
            
        } else {
            // Phim lẻ - set isTvSeries = "0"
            historyItem.setIsTvSeries("0");
            Log.e(TAG, "Auto-detected as Movie: " + title);
        }
        
        // Fetch thêm metadata từ API nếu cần
        if (!videoId.startsWith("phim4k_")) {
            fetchMovieMetadata(videoId, detectedType, historyItem, localHistory);
        }
    }
    
    /**
     * Detect content type từ videoId và title patterns
     */
    private String detectContentTypeFromId(String videoId, String title) {
        Log.e(TAG, "🔍 === SYNC MANAGER AUTO-DETECT DEBUG ===");
        Log.e(TAG, "📊 Input: videoId='" + videoId + "', title='" + title + "'");
        
        // Pattern matching để detect phim bộ
        if (videoId != null) {
            boolean videoIdSeasonEp = videoId.matches(".*[Ss]\\d+.*[Ee]\\d+.*");
            boolean videoIdTap = videoId.matches(".*[Tt]ập.*\\d+.*");
            Log.e(TAG, "🔸 VideoId patterns: S##E##=" + videoIdSeasonEp + ", Tập=" + videoIdTap);
            
            if (videoIdSeasonEp || videoIdTap) {
                Log.e(TAG, "✅ DETECTED: TV Series from videoId");
                return "tvseries";
            }
        }
        
        if (title != null) {
            boolean titleSeasonEp = title.matches(".*[Ss]\\d+.*[Ee]\\d+.*");
            boolean titleTap = title.matches(".*[Tt]ập.*\\d+.*");
            boolean titleEpisode = title.toLowerCase().contains("episode");
            boolean titleSeason = title.toLowerCase().contains("season");
            Log.e(TAG, "🔸 Title patterns: S##E##=" + titleSeasonEp + ", Tập=" + titleTap + 
                      ", episode=" + titleEpisode + ", season=" + titleSeason);
            
            if (titleSeasonEp || titleTap || titleEpisode || titleSeason) {
                Log.e(TAG, "✅ DETECTED: TV Series from title");
                return "tvseries";
            }
        }
        
        Log.e(TAG, "📽️ DETECTED: Movie (default)");
        return "movie"; // Default to movie
    }
    
    /**
     * Parse episode info từ title
     */
    private void parseEpisodeInfoFromTitle(String title, WatchHistorySyncItem.WatchHistoryItem historyItem) {
        try {
            // Pattern S01E01
            if (title.matches(".*[Ss](\\d+).*[Ee](\\d+).*")) {
                String seasonMatch = title.replaceAll(".*[Ss](\\d+).*", "$1");
                String episodeMatch = title.replaceAll(".*[Ee](\\d+).*", "$1");
                
                historyItem.setSeasonNumber(Integer.parseInt(seasonMatch));
                historyItem.setEpisodeNumber(Integer.parseInt(episodeMatch));
                historyItem.setSeasonName("Season " + seasonMatch);
                historyItem.setEpisodeName("Episode " + episodeMatch);
                
                Log.e(TAG, "Parsed episode info: Season " + seasonMatch + " Episode " + episodeMatch);
            }
            // Pattern Tập X
            else if (title.matches(".*[Tt]ập.*?(\\d+).*")) {
                String episodeMatch = title.replaceAll(".*[Tt]ập.*?(\\d+).*", "$1");
                
                historyItem.setSeasonNumber(1); // Default season 1
                historyItem.setEpisodeNumber(Integer.parseInt(episodeMatch));
                historyItem.setSeasonName("Season 1");
                historyItem.setEpisodeName("Tập " + episodeMatch);
                
                Log.e(TAG, "Parsed Vietnamese episode info: Tập " + episodeMatch);
            }
        } catch (Exception e) {
            Log.w(TAG, "Error parsing episode info from title: " + title, e);
        }
    }
