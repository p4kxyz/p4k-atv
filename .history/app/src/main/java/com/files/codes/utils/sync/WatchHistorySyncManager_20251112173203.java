    ...
    public void addWatchHistoryItemWithMetadata(String videoId, String title, String description, 
        ...
        try {
            ...
            if ("1".equals(isTvSeries)) {
                Log.e(TAG, "Creating season/episode data for TV series: " + title);
                
                // Tạo current episode từ thông tin hiện có
                curEpisode = new HashMap<>();
                curEpisode.put("episodes_id", videoId + "_ep01"); 
                curEpisode.put("episodes_name", title);
                curEpisode.put("stream_key", videoId + "_key01"); 
                ...
                // Tạo current season với nhiều episodes
                curSeason = new HashMap<>();
                curSeason.put("seasons_id", videoId + "_s01");
                curSeason.put("seasons_name", "Season 01 (HD)");
                
                // Tạo nhiều episodes cho season (giống như mẫu có 38 episodes)
                List<Map<String, Object>> episodes = new ArrayList<>();
                
                // Thêm episode hiện tại
                episodes.add(new HashMap<>(curEpisode));
                
                // Tạo thêm các episodes giả (tối đa 20 episodes)
                for (int i = 2; i <= 20; i++) {
                    Map<String, Object> additionalEpisode = new HashMap<>();
                    String epNum = String.format("%02d", i);
                    additionalEpisode.put("episodes_id", videoId + "_ep" + epNum);
                    additionalEpisode.put("episodes_name", title.replaceFirst("E01", "E" + epNum));
                    additionalEpisode.put("stream_key", videoId + "_key" + epNum);
                    additionalEpisode.put("file_type", "mkv");
                    additionalEpisode.put("image_url", thumbnailUrl != null ? thumbnailUrl : "");
                    additionalEpisode.put("file_url", (videoUrl != null ? videoUrl : "").replace("E01", "E" + epNum));
                    additionalEpisode.put("subtitle", new ArrayList<>());
                    episodes.add(additionalEpisode);
                }
                
                curSeason.put("episodes", episodes);
                
                // Thêm season vào allSeasons
                allSeasons.add(new HashMap<>(curSeason));
                
                Log.e(TAG, "✅ Created season with " + episodes.size() + " episodes - Season ID: " + 
                          curSeason.get("seasons_id") + ", Current Episode ID: " + curEpisode.get("episodes_id"));
            }
