...
public class WatchHistorySyncManager {
    ...
    public void addWatchHistoryItemWithMetadata(String videoId, String title, String description, 
        ...
        Log.e(TAG, "Adding watch history item with NEW STRUCTURE: videoId=" + videoId + ", title=" + title + 
        ...
        try {
            ...
            // Tạo season/episode data cho TV series
            ...
            if ("1".equals(isTvSeries)) {
                Log.e(TAG, "TV Series detected - will fetch real season/episode data from API: " + title);
                
                // Tạm thời tạo dữ liệu cơ bản trước, sau đó fetch từ API để cập nhật
                curEpisode = new HashMap<>();
                curEpisode.put("episodes_id", videoId + "_temp"); 
                curEpisode.put("episodes_name", title);
                curEpisode.put("stream_key", ""); 
                ...
                curSeason.put("seasons_id", videoId + "_temp_s01");
                curSeason.put("seasons_name", "Loading...");
                curSeason.put("episodes", new ArrayList<>());
                
                allSeasons = new ArrayList<>();
                
                // Fetch dữ liệu thật từ API (async)
                fetchRealSeasonData(videoId, currentPosition, totalDuration, videoUrl);
                
                Log.e(TAG, "⏳ Temporary season/episode created, fetching real data from API...");
            }
            
            // Sử dụng cấu trúc mới thay vì cấu trúc cũ
            saveFullWatchHistory(
                ...
                new ArrayList<>(), // cast
                curSeason, // curSeason - có dữ liệu cho TV series
                curEpisode, // curEpisode - có dữ liệu cho TV series  
                allSeasons, // allSeasons - có dữ liệu cho TV series
                currentPosition,
                totalDuration,
                videoUrl != null ? videoUrl : "",
                !"1".equals(isTvSeries) // isMovie
            );
    ...
    /**
     * Fetch dữ liệu season/episode thật từ API và cập nhật lại lịch sử xem
     */
    private void fetchRealSeasonData(String videoId, long position, long duration, String curUrl) {
        Log.e(TAG, "fetchRealSeasonData: Fetching real season data for videoId: " + videoId);
        
        try {
            ApiService apiService = RetrofitClient.getRetrofitInstance().create(ApiService.class);
            Call<MovieSingleDetails> call = apiService.getSingleDetail(videoId, "tvseries");
            ...
            call.enqueue(new Callback<MovieSingleDetails>() {
                ...
                public void onResponse(Call<MovieSingleDetails> call, Response<MovieSingleDetails> response) {
                    if (response.isSuccessful() && response.body() != null) {
                        ...
                        Log.e(TAG, "fetchRealSeasonData: ✅ Successfully fetched movie details for " + videoId);
                        
                        // Xác định current season và episode index (tạm thời dùng 0,0)
                        int currentSeasonIndex = 0;
                        int currentEpisodeIndex = 0;
                        
                        // Sử dụng method có sẵn để lưu với dữ liệu đầy đủ
                        saveWatchHistoryFromMovieDetails(movieDetails, position, duration, curUrl, 
                                                       currentSeasonIndex, currentEpisodeIndex);
                        
                        Log.e(TAG, "fetchRealSeasonData: ✅ Updated watch history with real season/episode data");
                        
                    } else {
                        Log.w(TAG, "fetchRealSeasonData: ⚠️ Failed to fetch movie details for " + videoId + 
                               ", Response code: " + response.code());
                    }
                }
                ...
                @Override
                public void onFailure(Call<MovieSingleDetails> call, Throwable t) {
                    Log.e(TAG, "fetchRealSeasonData: ❌ Error fetching movie details for " + videoId, t);
                }
            });
            
        } catch (Exception e) {
            Log.e(TAG, "fetchRealSeasonData: ❌ Exception while fetching data for " + videoId, e);
        }
    }
