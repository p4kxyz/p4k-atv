...
import com.files.codes.model.movieDetails.Country;
import com.files.codes.model.movieDetails.Director;
import com.files.codes.model.movieDetails.Writer;
import com.files.codes.model.movieDetails.Cast;
...
public class WatchHistorySyncManager {
    ...
    public void syncWatchHistoryToServer(SyncCallback callback) {
        ...
        // Lấy dữ liệu lịch sử xem mới (full structure)
        Map<String, Object> fullHistory = getFullWatchHistory();
        
        if (fullHistory.isEmpty()) {
            Log.e(TAG, "syncWatchHistoryToServer: No full history to sync, trying old format");
            ...
            if (localHistory.isEmpty()) {
                Log.e(TAG, "syncWatchHistoryToServer: No local history to sync either");
                callback.onSuccess("Không có dữ liệu để đồng bộ.");
                return;
            }
            syncOldFormatToServer(userId, localHistory, callback);
            return;
        }
        
        Log.e(TAG, "syncWatchHistoryToServer: Preparing " + fullHistory.size() + " items (FULL STRUCTURE) for upload");
        
        // Gửi dữ liệu full structure sử dụng method mới
        Call<JsonObject> call = syncService.writeFullWatchHistory(userId, fullHistory);
        call.enqueue(new Callback<JsonObject>() {
            ...
            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                ...
                if (response.isSuccessful()) {
                    updateLastSyncTime();
                    Log.e(TAG, "syncWatchHistoryToServer: Successfully synced " + fullHistory.size() + " items (FULL STRUCTURE) to server");
                    
                    if (response.body() != null) {
                        Log.e(TAG, "syncWatchHistoryToServer: Server response: " + response.body().toString());
                    }
                    
                    callback.onSuccess("Đã đồng bộ " + fullHistory.size() + " phim (cấu trúc mới) lên server.");
                } else {
    ...
    /**
     * Sync dữ liệu format cũ lên server (fallback)
     */
    private void syncOldFormatToServer(String userId, Map<String, WatchHistorySyncItem.WatchHistoryItem> localHistory, SyncCallback callback) {
        Log.e(TAG, "syncOldFormatToServer: Syncing " + localHistory.size() + " items in OLD FORMAT");
        ...
        call.enqueue(new Callback<JsonObject>() {
            ...
            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                if (response.isSuccessful()) {
                    updateLastSyncTime();
                    Log.e(TAG, "syncOldFormatToServer: Successfully synced " + localHistory.size() + " items (OLD FORMAT) to server");
                    callback.onSuccess("Đã đồng bộ " + localHistory.size() + " phim (format cũ) lên server.");
                } else {
                    Log.e(TAG, "syncOldFormatToServer: Failed - Response code: " + response.code());
                    callback.onError("Không thể đồng bộ format cũ lên server. Mã lỗi: " + response.code());
                }
            }
            
            @Override
            public void onFailure(Call<JsonObject> call, Throwable t) {
                Log.e(TAG, "syncOldFormatToServer: Network failure", t);
                callback.onError("Lỗi kết nối (format cũ): " + t.getMessage());
            }
        });
    }
    ...
    /**
     * Thêm hoặc cập nhật một mục lịch sử xem với đầy đủ metadata
     * Fetch dữ liệu thật từ API thay vì tạo fake data
     */
    public void addWatchHistoryItemWithMetadata(String videoId, String title, String description, 
        ...
        Log.d(TAG, "🎬 Fetching real data from API for: videoId=" + videoId + ", title=" + title + 
                  ", position=" + currentPosition + ", duration=" + totalDuration + ", type=" + videoType);
        
        // Fetch dữ liệu thật từ API
        fetchRealMovieDataAndSave(videoId, title, description, posterUrl, thumbnailUrl, videoUrl,
                                 currentPosition, totalDuration, videoType, releaseDate, imdbRating, 
                                 runtime, videoQuality, isTvSeries);
    }
    
    /**
     * Fetch dữ liệu thật từ API và lưu vào lịch sử xem
     */
    private void fetchRealMovieDataAndSave(String videoId, String title, String description, 
                                          String posterUrl, String thumbnailUrl, String videoUrl,
                                          long currentPosition, long totalDuration, String videoType,
                                          String releaseDate, String imdbRating, String runtime, 
                                          String videoQuality, String isTvSeries) {
        try {
            ...
            retrofit2.Retrofit retrofit = com.files.codes.model.api.RetrofitClient.getRetrofitInstance();
            com.files.codes.model.api.ApiService apiService = retrofit.create(com.files.codes.model.api.ApiService.class);
            
            // Gọi API để lấy chi tiết phim thật
            retrofit2.Call<com.files.codes.model.movieDetails.MovieSingleDetails> call = 
                apiService.getSingleDetail(com.files.codes.utils.AppConfig.getApiKey(), videoType, videoId);
            
            call.enqueue(new retrofit2.Callback<com.files.codes.model.movieDetails.MovieSingleDetails>() {
                @Override
                public void onResponse(retrofit2.Call<com.files.codes.model.movieDetails.MovieSingleDetails> call, 
                                     retrofit2.Response<com.files.codes.model.movieDetails.MovieSingleDetails> response) {
                    if (response.isSuccessful() && response.body() != null) {
                        com.files.codes.model.movieDetails.MovieSingleDetails movieDetails = response.body();
                        Log.d(TAG, "✅ Successfully fetched movie details from API for: " + videoId);
                        
                        // Sử dụng dữ liệu thật từ API
                        saveWithRealApiData(movieDetails, videoId, currentPosition, totalDuration, videoUrl);
                    } else {
                        Log.w(TAG, "⚠️ API call failed, using fallback data for: " + videoId);
                        // Fallback với dữ liệu cơ bản nếu API fail
                        saveWithFallbackData(videoId, title, description, posterUrl, thumbnailUrl, videoUrl,
                                           currentPosition, totalDuration, videoType, releaseDate, imdbRating, 
                                           runtime, videoQuality, isTvSeries);
                    }
                }
                ...
                @Override
                public void onFailure(retrofit2.Call<com.files.codes.model.movieDetails.MovieSingleDetails> call, Throwable t) {
                    Log.e(TAG, "❌ API call failed for videoId: " + videoId + ", error: " + t.getMessage());
            ...
            Log.e(TAG, "❌ Error setting up API call: " + e.getMessage());
            // Fallback với dữ liệu cơ bản
            saveWithFallbackData(videoId, title, description, posterUrl, thumbnailUrl, videoUrl,
                               currentPosition, totalDuration, videoType, releaseDate, imdbRating, 
                               runtime, videoQuality, isTvSeries);
    ...
    /**
     * Lưu với dữ liệu thật từ API
     */
    private void saveWithRealApiData(com.files.codes.model.movieDetails.MovieSingleDetails movieDetails, 
                                   String videoId, long currentPosition, long totalDuration, String videoUrl) {
        try {
            // Chuyển đổi Genre từ API
            List<Map<String, String>> genreList = new ArrayList<>();
            if (movieDetails.getGenre() != null && !movieDetails.getGenre().isEmpty()) {
                for (com.files.codes.model.Genre genre : movieDetails.getGenre()) {
                    Map<String, String> genreMap = new HashMap<>();
                    genreMap.put("genre_id", genre.getGenreId() != null ? genre.getGenreId() : "");
                    genreMap.put("name", genre.getName() != null ? genre.getName() : "");
                    genreMap.put("url", genre.getUrl() != null ? genre.getUrl() : "#");
                    genreList.add(genreMap);
                }
            }
            
            // Chuyển đổi Country từ API  
            List<Map<String, String>> countryList = new ArrayList<>();
            if (movieDetails.getCountry() != null && !movieDetails.getCountry().isEmpty()) {
                for (com.files.codes.model.Country country : movieDetails.getCountry()) {
                    Map<String, String> countryMap = new HashMap<>();
                    countryMap.put("country_id", country.getCountryId() != null ? country.getCountryId() : "");
                    countryMap.put("name", country.getName() != null ? country.getName() : "");
                    countryMap.put("url", country.getUrl() != null ? country.getUrl() : "#");
                    countryList.add(countryMap);
                }
            }
            
            // Chuyển đổi Director từ API
            List<Map<String, String>> directorList = new ArrayList<>();
            if (movieDetails.getDirector() != null && !movieDetails.getDirector().isEmpty()) {
                for (com.files.codes.model.movieDetails.Director director : movieDetails.getDirector()) {
                    Map<String, String> directorMap = new HashMap<>();
                    directorMap.put("star_id", director.getStarId() != null ? director.getStarId() : "");
                    directorMap.put("name", director.getName() != null ? director.getName() : "");
                    directorMap.put("url", director.getUrl() != null ? director.getUrl() : "#");
                    directorList.add(directorMap);
                }
            }
            
            // Chuyển đổi Cast từ API
            List<Map<String, String>> castList = new ArrayList<>();
            if (movieDetails.getCast() != null && !movieDetails.getCast().isEmpty()) {
                for (com.files.codes.model.movieDetails.Cast cast : movieDetails.getCast()) {
                    Map<String, String> castMap = new HashMap<>();
                    castMap.put("star_id", cast.getStarId() != null ? cast.getStarId() : "");
                    castMap.put("name", cast.getName() != null ? cast.getName() : "");
                    castMap.put("url", cast.getUrl() != null ? cast.getUrl() : "#");
                    castList.add(castMap);
                }
            }
            
            // Xử lý Season/Episode cho TV Series
            Map<String, Object> curSeason = null;
            Map<String, Object> curEpisode = null;
            List<Map<String, Object>> allSeasons = new ArrayList<>();
            
            if ("1".equals(movieDetails.getIsTvseries()) && movieDetails.getSeason() != null && !movieDetails.getSeason().isEmpty()) {
                // Sử dụng season data thật từ API
                for (com.files.codes.model.movieDetails.Season apiSeason : movieDetails.getSeason()) {
                    Map<String, Object> seasonMap = new HashMap<>();
                    seasonMap.put("seasons_id", apiSeason.getSeasonsId());
                    seasonMap.put("seasons_name", apiSeason.getSeasonsName());
                    
                    List<Map<String, Object>> episodesList = new ArrayList<>();
                    if (apiSeason.getEpisodes() != null) {
                        for (com.files.codes.model.movieDetails.Episode apiEpisode : apiSeason.getEpisodes()) {
                            Map<String, Object> episodeMap = new HashMap<>();
                            episodeMap.put("episodes_id", apiEpisode.getEpisodesId());
                            episodeMap.put("episodes_name", apiEpisode.getEpisodesName());
                            episodeMap.put("stream_key", apiEpisode.getStreamKey());
                            episodeMap.put("file_type", apiEpisode.getFileType());
                            episodeMap.put("image_url", apiEpisode.getImageUrl());
                            episodeMap.put("file_url", apiEpisode.getFileUrl());
                            episodeMap.put("subtitle", apiEpisode.getSubtitle() != null ? apiEpisode.getSubtitle() : new ArrayList<>());
                            episodesList.add(episodeMap);
                            
                            // Nếu đây là episode đầu tiên của season đầu tiên, dùng làm current episode
                            if (curEpisode == null) {
                                curEpisode = new HashMap<>(episodeMap);
                            }
                        }
                    }
                    seasonMap.put("episodes", episodesList);
                    allSeasons.add(seasonMap);
                    
                    // Season đầu tiên làm current season
                    if (curSeason == null) {
                        curSeason = new HashMap<>(seasonMap);
                    }
                }
                
                Log.d(TAG, "✅ Using real season/episode data from API for TV series: " + movieDetails.getTitle());
            }
            
            // Lưu với dữ liệu thật từ API
            saveFullWatchHistory(
                movieDetails.getVideosId(),
                movieDetails.getTitle(),
                movieDetails.getDescription() != null ? movieDetails.getDescription() : "",
                movieDetails.getSlug() != null ? movieDetails.getSlug() : "",
                movieDetails.getRelease() != null ? movieDetails.getRelease() : "",
                movieDetails.getRuntime() != null ? movieDetails.getRuntime() : "",
                movieDetails.getVideoQuality() != null ? movieDetails.getVideoQuality() : "",
                movieDetails.getIsTvseries() != null ? movieDetails.getIsTvseries() : "0",
                movieDetails.getIsPaid() != null ? movieDetails.getIsPaid() : "0",
                movieDetails.getEnableDownload() != null ? movieDetails.getEnableDownload() : "1",
                movieDetails.getThumbnailUrl() != null ? movieDetails.getThumbnailUrl() : "",
                movieDetails.getPosterUrl() != null ? movieDetails.getPosterUrl() : "",
                movieDetails.getImdbRating(),
                genreList,
                countryList,
                directorList,
                new ArrayList<>(), // writer - không có trong API response
                castList,
                curSeason,
                curEpisode,
                allSeasons,
                ...
                totalDuration,
                videoUrl,
                !"1".equals(movieDetails.getIsTvseries())
            );
            
            Log.d(TAG, "✅ Successfully saved watch history with REAL API DATA for: " + movieDetails.getTitle());
            
        } catch (Exception e) {
            Log.e(TAG, "❌ Error saving with real API data: " + e.getMessage());
        }
    }
    
    /**
     * Lưu với dữ liệu cơ bản khi API fail (fallback)
     */
    private void saveWithFallbackData(String videoId, String title, String description, 
                                    String posterUrl, String thumbnailUrl, String videoUrl,
                                    long currentPosition, long totalDuration, String videoType,
                                    String releaseDate, String imdbRating, String runtime, 
                                    String videoQuality, String isTvSeries) {
        Log.d(TAG, "⚠️ Using fallback data for: " + title);
        
        // Tạo genre và country mặc định
        List<Map<String, String>> genre = new ArrayList<>();
        Map<String, String> defaultGenre = new HashMap<>();
        defaultGenre.put("genre_id", "");
        defaultGenre.put("name", "Unknown");
        defaultGenre.put("url", "#");
        genre.add(defaultGenre);
        
        List<Map<String, String>> country = new ArrayList<>();
        Map<String, String> defaultCountry = new HashMap<>();
        defaultCountry.put("country_id", "");
        defaultCountry.put("name", "Unknown");
        defaultCountry.put("url", "#");
        country.add(defaultCountry);
        
        // Tạo season/episode data cơ bản cho TV series
        Map<String, Object> curSeason = null;
        Map<String, Object> curEpisode = null;
        List<Map<String, Object>> allSeasons = new ArrayList<>();
        
        if ("1".equals(isTvSeries)) {
            // Tạo episode hiện tại từ thông tin có sẵn  
            curEpisode = new HashMap<>();
            curEpisode.put("episodes_id", videoId);
            curEpisode.put("episodes_name", title);
            curEpisode.put("stream_key", videoId); 
            curEpisode.put("file_type", "mkv");
            curEpisode.put("image_url", thumbnailUrl != null ? thumbnailUrl : "");
            curEpisode.put("file_url", videoUrl != null ? videoUrl : "");
            curEpisode.put("subtitle", new ArrayList<>());
            
            // Tạo season hiện tại
            curSeason = new HashMap<>();
            curSeason.put("seasons_id", videoId + "_s01");
            curSeason.put("seasons_name", "Season 01 (HD)");
            
            // Chỉ có episode hiện tại trong season
            List<Map<String, Object>> episodes = new ArrayList<>();
            episodes.add(new HashMap<>(curEpisode));
            curSeason.put("episodes", episodes);
            
            // Season list
            allSeasons = new ArrayList<>();
            allSeasons.add(new HashMap<>(curSeason));
        }
        ...
        saveFullWatchHistory(
            ...
            Log.e(TAG, "✅ Successfully saved watch history with NEW STRUCTURE for: " + title);
            
            // Nếu thiếu metadata, thử fetch từ API để cập nhật sau
            if (TextUtils.isEmpty(description) || TextUtils.isEmpty(releaseDate)) {
                Log.e(TAG, "Missing metadata for videoId " + videoId + ", will try to fetch and update later");
                // TODO: Có thể thêm logic fetch metadata và update lại cấu trúc đầy đủ
            }
    ...
    /**
     * Lưu lịch sử xem với cấu trúc đầy đủ theo mẫu mau.json
     * Bao gồm: position, duration, curUrl, curSeason, curEpisode, metadata đầy đủ
     */
    public void saveFullWatchHistory(String videoId, String title, String description, 
                                   String slug, String releaseDate, String runtime, 
                                   String videoQuality, String isTvSeries, String isPaid, 
                                   String enableDownload, String thumbnailUrl, String posterUrl,
                                   String imdbRating, List<Map<String, String>> genre,
                                   List<Map<String, String>> country, List<Map<String, String>> director,
                                   List<Map<String, String>> writer, List<Map<String, String>> cast,
                                   Map<String, Object> curSeason, Map<String, Object> curEpisode,
                                   List<Map<String, Object>> allSeasons,
                                   long position, long duration, String curUrl, boolean isMovie) {
        
        if (TextUtils.isEmpty(videoId)) {
            Log.w(TAG, "Cannot save full watch history - videoId is empty");
            return;
        }
        
        Log.e(TAG, "Saving full watch history for videoId: " + videoId + ", title: " + title);
        
        try {
            // Tạo cấu trúc dữ liệu theo mẫu mau.json
            Map<String, Object> historyData = new HashMap<>();
            
            // Thông tin playback cơ bản
            historyData.put("position", position);
            historyData.put("duration", duration);
            historyData.put("curUrl", curUrl != null ? curUrl : "");
            
            // Season và Episode hiện tại (cho TV series)
            if (curSeason != null) {
                historyData.put("curSeason", curSeason);
            }
            if (curEpisode != null) {
                historyData.put("curEpisode", curEpisode);
            }
            ...
            // Metadata video
            historyData.put("videos_id", videoId);
            historyData.put("title", title != null ? title : "");
            historyData.put("description", description != null ? description : "");
            historyData.put("slug", slug != null ? slug : "");
            historyData.put("release", releaseDate != null ? releaseDate : "");
            historyData.put("runtime", runtime != null ? runtime : "");
            historyData.put("video_quality", videoQuality != null ? videoQuality : "");
            historyData.put("is_tvseries", isTvSeries != null ? isTvSeries : "0");
            historyData.put("is_paid", isPaid != null ? isPaid : "0");
            historyData.put("enable_download", enableDownload != null ? enableDownload : "1");
            historyData.put("thumbnail_url", thumbnailUrl != null ? thumbnailUrl : "");
            historyData.put("poster_url", posterUrl != null ? posterUrl : "");
            historyData.put("imdb_rating", imdbRating);
            
            // Genre, Country, Director, Writer, Cast
            historyData.put("genre", genre != null ? genre : new ArrayList<>());
            historyData.put("country", country != null ? country : new ArrayList<>());
            historyData.put("director", director != null ? director : new ArrayList<>());
            historyData.put("writer", writer != null ? writer : new ArrayList<>());
            historyData.put("cast", cast != null ? cast : new ArrayList<>());
            historyData.put("cast_and_crew", new ArrayList<>());
            
            // Danh sách tất cả seasons (cho TV series)
            if (allSeasons != null && !allSeasons.isEmpty()) {
                historyData.put("season", allSeasons);
            } else {
                historyData.put("season", new ArrayList<>());
            }
            
            // Đánh dấu loại content
            historyData.put("isMovie", isMovie);
            
            // Lưu vào SharedPreferences với key là videoId
            String jsonData = gson.toJson(historyData);
            String existingHistoryJson = sharedPreferences.getString("full_watch_history", "{}");
            
            Type historyMapType = new TypeToken<Map<String, Object>>(){}.getType();
            Map<String, Object> fullHistory = gson.fromJson(existingHistoryJson, historyMapType);
            if (fullHistory == null) {
                fullHistory = new HashMap<>();
            }
            
            // Lưu với key là videoId
            fullHistory.put(videoId, historyData);
            
            // Lưu lại toàn bộ
            String updatedHistoryJson = gson.toJson(fullHistory);
            sharedPreferences.edit()
                .putString("full_watch_history", updatedHistoryJson)
                .apply();
            
            Log.e(TAG, "Successfully saved full watch history for videoId: " + videoId);
            
            // Auto sync nếu được cấu hình
            if (isSyncConfigured()) {
                syncToServer(new SyncCallback() {
                    @Override
                    public void onSuccess(String message) {
                        Log.d(TAG, "Auto sync full history successful: " + message);
                    }
                    
                    @Override
                    public void onError(String error) {
                        Log.w(TAG, "Auto sync full history failed: " + error);
                    }
                });
            }
            
        } catch (Exception e) {
            Log.e(TAG, "Error saving full watch history for videoId: " + videoId, e);
        }
    }
    
    /**
     * Lấy lịch sử xem đầy đủ theo cấu trúc mau.json
     */
    public Map<String, Object> getFullWatchHistory() {
        try {
            String historyJson = sharedPreferences.getString("full_watch_history", "{}");
            Type historyMapType = new TypeToken<Map<String, Object>>(){}.getType();
            Map<String, Object> fullHistory = gson.fromJson(historyJson, historyMapType);
            return fullHistory != null ? fullHistory : new HashMap<>();
        } catch (Exception e) {
            Log.e(TAG, "Error getting full watch history", e);
            return new HashMap<>();
        }
    }
    
    /**
     * Lấy thông tin lịch sử xem cho một video cụ thể
     */
    public Map<String, Object> getFullWatchHistoryItem(String videoId) {
        ...
        Map<String, Object> fullHistory = getFullWatchHistory();
        return (Map<String, Object>) fullHistory.get(videoId);
    }
    ...
    /**
     * Cập nhật method addWatchHistoryItemWithMetadata để sử dụng cấu trúc mới
     */
    public void addWatchHistoryItemWithFullStructure(String videoId, String title, String description,
                                                   String posterUrl, String thumbnailUrl, String videoUrl,
                                                   long currentPosition, long totalDuration, String videoType,
                                                   String releaseDate, String imdbRating, String runtime,
                                                   String videoQuality, String isTvSeries,
        ...
        if (TextUtils.isEmpty(videoId) || TextUtils.isEmpty(title)) {
            Log.w(TAG, "Cannot add full watch history item - missing required data");
            return;
        }
        
        Log.e(TAG, "Adding full watch history item: videoId=" + videoId + ", title=" + title);
        
        try {
            // Tạo genre và country mặc định nếu không có
            List<Map<String, String>> genre = new ArrayList<>();
            Map<String, String> defaultGenre = new HashMap<>();
            defaultGenre.put("genre_id", "");
            defaultGenre.put("name", "Unknown");
            ...
            // Gọi saveFullWatchHistory với cấu trúc đầy đủ
            saveFullWatchHistory(
                videoId, title, description, "", releaseDate, runtime, videoQuality,
                isTvSeries, "0", "1", thumbnailUrl, posterUrl, imdbRating,
                genre, country, new ArrayList<>(), new ArrayList<>(), new ArrayList<>(),
                curSeason, curEpisode, allSeasons,
                currentPosition, totalDuration, videoUrl,
                !"1".equals(isTvSeries)
            );
            
        } catch (Exception e) {
            Log.e(TAG, "Error adding full watch history item", e);
        }
    }

    /**
     * Lưu lịch sử xem từ MovieSingleDetails với thông tin season/episode
     */
    public void saveWatchHistoryFromMovieDetails(MovieSingleDetails movieDetails, 
                                               long position, long duration, String curUrl,
                                               int currentSeasonIndex, int currentEpisodeIndex) {
        if (movieDetails == null) {
            Log.w(TAG, "Cannot save watch history - MovieSingleDetails is null");
            return;
        }
        
        String videoId = movieDetails.getVideosId();
        if (TextUtils.isEmpty(videoId)) {
            Log.w(TAG, "Cannot save watch history - videoId is empty");
            return;
        }
        
        Log.e(TAG, "Saving watch history from MovieSingleDetails: " + videoId + ", title: " + movieDetails.getTitle());
        
        try {
            // Chuyển đổi genre list
            List<Map<String, String>> genreList = new ArrayList<>();
            if (movieDetails.getGenre() != null) {
                for (Genre genre : movieDetails.getGenre()) {
                    Map<String, String> genreMap = new HashMap<>();
                    genreMap.put("genre_id", genre.getGenreId() != null ? genre.getGenreId() : "");
                    genreMap.put("name", genre.getName() != null ? genre.getName() : "Unknown");
                    genreMap.put("url", genre.getSlug() != null ? "#/genre/" + genre.getSlug() : "#");
                    genreList.add(genreMap);
                }
            }
            
            // Chuyển đổi country list
            List<Map<String, String>> countryList = new ArrayList<>();
            if (movieDetails.getCountry() != null) {
                for (Country country : movieDetails.getCountry()) {
                    Map<String, String> countryMap = new HashMap<>();
                    countryMap.put("country_id", country.getCountryId() != null ? country.getCountryId() : "");
                    countryMap.put("name", country.getName() != null ? country.getName() : "Unknown");
                    countryMap.put("url", country.getUrl() != null ? country.getUrl() : "#");
                    countryList.add(countryMap);
                }
            }
            
            // Chuyển đổi director list
            List<Map<String, String>> directorList = new ArrayList<>();
            if (movieDetails.getDirector() != null) {
                for (Director director : movieDetails.getDirector()) {
                    Map<String, String> directorMap = new HashMap<>();
                    directorMap.put("star_id", director.getStarId() != null ? director.getStarId() : "");
                    directorMap.put("name", director.getName() != null ? director.getName() : "");
                    directorMap.put("url", director.getUrl() != null ? director.getUrl() : "#");
            ...
            // Chuyển đổi writer list
            List<Map<String, String>> writerList = new ArrayList<>();
            if (movieDetails.getWriter() != null) {
                for (Writer writer : movieDetails.getWriter()) {
                    Map<String, String> writerMap = new HashMap<>();
                    writerMap.put("star_id", writer.getStarId() != null ? writer.getStarId() : "");
                    writerMap.put("name", writer.getName() != null ? writer.getName() : "");
                    writerMap.put("url", writer.getUrl() != null ? writer.getUrl() : "#");
                    writerList.add(writerMap);
                }
            }
            
            // Chuyển đổi cast list
            List<Map<String, String>> castList = new ArrayList<>();
            if (movieDetails.getCast() != null) {
                for (Cast cast : movieDetails.getCast()) {
                    Map<String, String> castMap = new HashMap<>();
                    castMap.put("star_id", cast.getStarId() != null ? cast.getStarId() : "");
                    castMap.put("name", cast.getName() != null ? cast.getName() : "");
                    castMap.put("url", cast.getUrl() != null ? cast.getUrl() : "#");
            ...
            // Chuyển đổi season list
            List<Map<String, Object>> seasonList = new ArrayList<>();
            Map<String, Object> curSeason = null;
            Map<String, Object> curEpisode = null;
            
            if (movieDetails.getSeason() != null) {
                for (int i = 0; i < movieDetails.getSeason().size(); i++) {
                    Season season = movieDetails.getSeason().get(i);
                    Map<String, Object> seasonMap = new HashMap<>();
                    ...
                    if (season.getEpisodes() != null) {
                        for (Episode episode : season.getEpisodes()) {
                            ...
                            episodeMap.put("subtitle", episode.getSubtitle() != null ? episode.getSubtitle() : new ArrayList<>());
                            episodeList.add(episodeMap);
                        }
                    }
                    seasonMap.put("episodes", episodeList);
                    seasonList.add(seasonMap);
                    
                    // Xác định current season và episode
                    if (i == currentSeasonIndex) {
                        curSeason = new HashMap<>(seasonMap);
                        if (currentEpisodeIndex >= 0 && currentEpisodeIndex < episodeList.size()) {
                            curEpisode = new HashMap<>((Map<String, Object>) episodeList.get(currentEpisodeIndex));
                        }
                    }
                }
            }
            
            // Xác định xem có phải movie không
            boolean isMovie = !"1".equals(movieDetails.getIsTvseries());
            
            // Gọi method saveFullWatchHistory
            saveFullWatchHistory(
                videoId,
                movieDetails.getTitle(),
                movieDetails.getDescription(),
                movieDetails.getSlug(),
                movieDetails.getRelease(),
                movieDetails.getRuntime(),
                movieDetails.getVideoQuality(),
                movieDetails.getIsTvseries(),
                movieDetails.getIsPaid(),
                movieDetails.getEnableDownload(),
                movieDetails.getThumbnailUrl(),
            ...
            Log.e(TAG, "Error saving watch history from MovieSingleDetails", e);
        }
    }
