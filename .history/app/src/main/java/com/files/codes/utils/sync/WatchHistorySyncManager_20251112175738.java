...
public class WatchHistorySyncManager {
    ...
    /**
     * Save full watch history with complete data structure like mau.json
     * To be called from HeroStyleVideoDetailsActivity/PlayerActivity when user starts watching
     */
    public void saveFullWatchHistory(
        String videoId, String title, String description, String slug, String releaseDate, 
        String runtime, String videoQuality, String isTvSeries, String isPaid, String enableDownload,
        ...
        try {
            ...
            // Create or update history item with complete data structure
            WatchHistorySyncItem.WatchHistoryItem historyItem;
            if (localHistory.containsKey(videoId)) {
                historyItem = localHistory.get(videoId);
                Log.d(TAG, "Updating existing full watch history for: " + videoId);
            } else {
                historyItem = new WatchHistorySyncItem.WatchHistoryItem();
                historyItem.setVideoId(videoId);
                Log.d(TAG, "Creating new full watch history for: " + videoId);
            }
            
            // Set basic playback data
            historyItem.setCurrentPosition(position);
            historyItem.setTotalDuration(duration);
            historyItem.setUpdatedAt(System.currentTimeMillis());
            historyItem.setVideoUrl(curUrl);
            
            // Calculate progress percentage
            if (duration > 0) {
                float progressPercentage = (float) ((position * 100) / duration);
                historyItem.setProgressPercentage(progressPercentage);
            }
            
            // Set complete movie/series metadata (like mau.json structure)
            historyItem.setTitle(title);
            historyItem.setDescription(description);
            ...
            // Set complex data structures using Map to maintain JSON structure
            if (genre != null) {
            ...
            Log.e(TAG, "✅ Saved full watch history with complete metadata: " + title + 
                      " (pos: " + position + "ms, dur: " + duration + "ms, isMovie: " + isMovie + ")");
            
            // Auto sync to server if configured
            if (isSyncConfigured()) {
                Log.e(TAG, "🔄 Auto syncing full watch history to server");
                syncToServer(new SyncCallback() {
                    @Override
                    public void onSuccess(String message) {
                        Log.d(TAG, "✅ Full watch history auto sync successful: " + message);
                    }
                    
                    @Override
                    public void onError(String error) {
                        Log.w(TAG, "❌ Full watch history auto sync failed: " + error);
                    }
                });
            }
            
        } catch (Exception e) {
            Log.e(TAG, "❌ Error saving full watch history with metadata", e);
        }
    }
    ...
    public void saveFullWatchHistory(String videoId, String title, String description, String slug,
        ...
        try {
            ...
            historyItem.setCurrentPosition(position);
            historyItem.setTotalDuration(duration);
            historyItem.setLastWatchedTime(System.currentTimeMillis());
            
            // Convert lists to JSON strings for storage
            Gson gson = new Gson();
            historyItem.setGenreJson(gson.toJson(genre));
            historyItem.setCountryJson(gson.toJson(country));
            historyItem.setDirectorJson(gson.toJson(director));
            historyItem.setWriterJson(gson.toJson(writer));
            historyItem.setCastJson(gson.toJson(cast));
            
            // Store season/episode data for TV series
            if (!isMovie && curSeason != null) {
                historyItem.setCurSeasonJson(gson.toJson(curSeason));
                if (curEpisode != null) {
                    historyItem.setCurEpisodeJson(gson.toJson(curEpisode));
                }
                if (allSeasons != null) {
                    historyItem.setAllSeasonsJson(gson.toJson(allSeasons));
                }
            }
            ...
            syncWatchHistoryToServer(new SyncCallback() {
                @Override
                public void onSuccess(String message) {
                    Log.d(TAG, "✅ Full watch history saved successfully: " + title);
                }
                
                @Override
                public void onError(String error) {
                    Log.e(TAG, "❌ Failed to sync full watch history: " + error);
                }
            });
            
            Log.d(TAG, "🎬 Saved full watch history: " + title + " at position " + WatchHistoryTracker.formatTime(position));
