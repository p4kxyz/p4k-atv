...
//                }
    ...
    */
    ...
    private void showNextEpisodeDialog() {
        ...
        builder.setTitle("Tập tiếp theo");
        
        if (nextEpisodeTitle != null && !nextEpisodeTitle.isEmpty()) {
            builder.setMessage("Bạn có muốn xem tập tiếp theo?\n\n" + nextEpisodeTitle);
        } else {
            builder.setMessage("Bạn có muốn xem tập tiếp theo không?");
        }
        
        builder.setPositiveButton("Xem ngay", new DialogInterface.OnClickListener() {
            @Override
