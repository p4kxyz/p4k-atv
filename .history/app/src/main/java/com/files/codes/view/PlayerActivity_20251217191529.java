...
//                }
    ...
    */
    ...
    /**
     * Fetch season data and then launch external player
     */
    private void fetchSeasonDataAndLaunchExternal(String movieId) {
        Log.d("PlayerActivity", "Fetching season data before launching external player: " + movieId);
        ...
        call.enqueue(new Callback<MovieSingleDetails>() {
            ...
            public void onResponse(Call<MovieSingleDetails> call, Response<MovieSingleDetails> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    if (movieDetails.getSeason() != null && !movieDetails.getSeason().isEmpty()) {
                        Log.d("PlayerActivity", "✅ Successfully fetched season data for external player");
                        
                        // Update model with season data
                        model.setAllSeasons(movieDetails.getSeason());
                        
                        // Try to find current episode in seasons to set correct index
                ...
                // Launch external player regardless of success (fallback to single video if failed)
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (progressBar != null) progressBar.setVisibility(View.GONE);
                        Log.e(TAG, "🚀 Launching External Player after data fetch...");
                        launchExternalPlayer();
                    }
                });
            }
            ...
            @Override
            public void onFailure(Call<MovieSingleDetails> call, Throwable t) {
                Log.e(TAG, "Error fetching season data for external player", t);
                // Launch anyway
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
