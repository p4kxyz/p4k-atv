    ...
    private void applySubtitleSettings() {
        ...
        if (subtitleView != null) {
            CaptionStyleCompat style = new CaptionStyleCompat(
                    ...
                    edgeType, // Enable outline/shadow/etc
            ...
            // Apply outline thickness if edge type is OUTLINE
            // Note: ExoPlayer's SubtitleView doesn't have a direct setOutlineWidth method in older versions
            // We can try to use reflection or check if we can access the internal WebVttCssStyle
            // For now, we'll use a workaround by adjusting the shadow radius if possible, 
            // but standard CaptionStyleCompat doesn't expose thickness directly.
            // However, we can try to set the shadow layer on the internal TextView if accessible,
            // but SubtitleView is a FrameLayout containing a CanvasSubtitleOutput or WebVttSubtitleOutput.
            
            // Since we can't easily change outline thickness via standard API, 
            // we will rely on the "Bold" setting and "Edge Style" to give the perception of thickness.
            // If the user selected "Extra Thick", we can simulate it by using a heavier font weight if available
            // or by using a shadow layer with 0 offset and large radius if we could access the paint.
            
            // WORKAROUND: If "Thick" or "Extra Thick" is selected, force BOLD to make it look thicker
            int outlineThickness = prefs.getInt("outline_thickness", 1);
            if (outlineThickness >= 2 && !isBold) {
                 // If thick/extra thick and not already bold, force bold typeface for this render
                 Typeface boldTypeface = Typeface.create(typeface, Typeface.BOLD);
                 style = new CaptionStyleCompat(
                    textColor, background ? Color.BLACK : Color.TRANSPARENT, Color.TRANSPARENT,
                    edgeType, outlineColor, boldTypeface
                );
                subtitleView.setStyle(style);
            }
