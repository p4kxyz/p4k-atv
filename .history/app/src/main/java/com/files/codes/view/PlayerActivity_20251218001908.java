...
import com.files.codes.view.custom.StrokeTextView;
...
//                }
    ...
    private void openSubtitleDialog() {
        ...
        Log.d(TAG, "🎬 Video object: " + (video != null ? "EXISTS" : "NULL"));
        if (video != null) {
            Log.d(TAG, "🎬 Video subtitle: " + (video.getSubtitle() != null ? video.getSubtitle().size() + " items" : "NULL"));
        }
        ...
        if (video == null || video.getSubtitle() == null || video.getSubtitle().isEmpty()) {
            Log.d(TAG, "🎬 No subtitle data found, loading from API...");
        ...
        if (video != null && video.getSubtitle() != null && !video.getSubtitle().isEmpty()) {
            ...
            Log.d(TAG, "🎬 Subtitle RecyclerView setup complete");
   ...
   /* @SuppressLint("StaticFieldLeak")
    ...
    private void applySubtitleSettings() {
        ...
        // Get bold setting
        boolean isBold = prefs.getBoolean("font_bold", false);
        if (isBold) {
            typeface = Typeface.create(typeface, Typeface.BOLD);
        }

        // Get edge style
        int edgeStyleIndex = prefs.getInt("edge_style", 0); // 0 = Outline
        int edgeType = CaptionStyleCompat.EDGE_TYPE_OUTLINE;
        if (edgeStyleIndex == 1) edgeType = CaptionStyleCompat.EDGE_TYPE_DROP_SHADOW;
        else if (edgeStyleIndex == 2) edgeType = CaptionStyleCompat.EDGE_TYPE_RAISED;
        else if (edgeStyleIndex == 3) edgeType = CaptionStyleCompat.EDGE_TYPE_DEPRESSED;
        else if (edgeStyleIndex == 4) edgeType = CaptionStyleCompat.EDGE_TYPE_NONE;

        // Apply to the single SubtitleView
        if (subtitleView != null) {
            // Use custom StrokeTextView logic if available
            if (subtitleView instanceof StrokeTextView) {
                StrokeTextView strokeView = (StrokeTextView) subtitleView;
                
                // Set basic text properties
                strokeView.setTextColor(textColor);
                strokeView.setTypeface(typeface);
                strokeView.setTextSize(fontSize);
                
                // Handle background
                if (background) {
                    strokeView.setBackgroundColor(Color.BLACK);
                } else {
                    strokeView.setBackgroundColor(Color.TRANSPARENT);
                }
                
                // Handle Outline / Stroke
                if (edgeStyleIndex == 0) { // Outline
                    // Use raw value for precise control (pt/px)
                    int outlineThickness = prefs.getInt("outline_thickness", 4); 
                    // Ensure it's at least 1 if outline is enabled
                    if (outlineThickness < 1) outlineThickness = 1;
                    
                    strokeView.setStroke((float)outlineThickness, outlineColor);
                    strokeView.setShadowLayer(0, 0, 0, 0); // Clear shadow
                } else if (edgeStyleIndex == 1) { // Drop Shadow
                    strokeView.setStroke(0, 0); // Clear stroke
                    strokeView.setShadowLayer(5.0f, 5.0f, 5.0f, Color.BLACK);
                } else {
                    strokeView.setStroke(0, 0); // Clear stroke
                    strokeView.setShadowLayer(0, 0, 0, 0); // Clear shadow
                }
            } else {
                // Fallback for standard SubtitleView (if layout wasn't updated correctly)
                // Note: This branch should not be reached if XML is updated to StrokeTextView
            }
            ...
            float bottomPaddingFraction = 0.01f + (verticalOffset * 0.01f); // 1% base + offset
            bottomPaddingFraction = Math.max(0.0f, Math.min(0.85f, bottomPaddingFraction)); // Clamp between 0% and 85%
            
            // Convert fraction to pixels based on screen height
            int screenHeight = getResources().getDisplayMetrics().heightPixels;
            int bottomMarginPx = (int) (screenHeight * bottomPaddingFraction);
            
            if (subtitleView.getLayoutParams() instanceof RelativeLayout.LayoutParams) {
                RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) subtitleView.getLayoutParams();
            ...
            Log.d(TAG, "Applied subtitle - colors: text=" + Integer.toHexString(textColor) + 
                      ...
                      ", position: " + verticalOffset + "% offset, bottomPaddingPx: " + bottomMarginPx);
        }
    }
    ...
    */
    ...
    private void openUnifiedSettingsDialog() {
        ...
        // Edge Style controls (Outline, Shadow, etc.)
        TextView edgeStyleLabel = new TextView(this);
        edgeStyleLabel.setText("Edge Style:");
        edgeStyleLabel.setTextSize(16);
        edgeStyleLabel.setPadding(0, 20, 0, 10);

        LinearLayout edgeStyleLayout = new LinearLayout(this);
        edgeStyleLayout.setOrientation(LinearLayout.HORIZONTAL);

        Button edgeStylePrevBtn = new Button(this);
        edgeStylePrevBtn.setText("◀");
        Button edgeStyleNextBtn = new Button(this);
        edgeStyleNextBtn.setText("▶");
        TextView edgeStyleTV = new TextView(this);

        String[] edgeStyleNames = {"Outline", "Drop Shadow", "Raised", "Depressed", "None"};
        int currentEdgeStyle = prefs.getInt("edge_style", 0);
        if (currentEdgeStyle >= edgeStyleNames.length) {
            currentEdgeStyle = 0;
            prefs.edit().putInt("edge_style", currentEdgeStyle).apply();
        }
        edgeStyleTV.setText(edgeStyleNames[currentEdgeStyle]);
        edgeStyleTV.setGravity(android.view.Gravity.CENTER);
        edgeStyleTV.setPadding(20, 0, 20, 0);

        edgeStyleLayout.addView(edgeStylePrevBtn);
        edgeStyleLayout.addView(edgeStyleTV);
        edgeStyleLayout.addView(edgeStyleNextBtn);

        // Bold Text Switch
        LinearLayout boldLayout = new LinearLayout(this);
        boldLayout.setOrientation(LinearLayout.HORIZONTAL);
        boldLayout.setPadding(0, 20, 0, 10);
        TextView boldLabel = new TextView(this);
        boldLabel.setText("Bold Text: ");
        boldLabel.setTextSize(16);
        Switch boldSwitch = new Switch(this);
        boldSwitch.setChecked(prefs.getBoolean("font_bold", false));

        boldLayout.addView(boldLabel);
        boldLayout.addView(boldSwitch);
        ...
        String[] outlineThicknessNames = {"Thin", "Normal", "Thick", "Extra Thick"};
        int currentOutlineThickness = prefs.getInt("outline_thickness", 1); // Default to Normal
        if (currentOutlineThickness >= outlineThicknessNames.length) {
            currentOutlineThickness = 1;
            prefs.edit().putInt("outline_thickness", currentOutlineThickness).apply();
        }
        outlineThicknessTV.setText(outlineThicknessNames[currentOutlineThickness]);
        ...
        // Add Edge Style and Bold controls
        layout.addView(edgeStyleLabel);
        layout.addView(edgeStyleLayout);
        layout.addView(boldLayout);
        ...
        outlineThicknessPrevBtn.setOnClickListener(v -> {
            int index = (prefs.getInt("outline_thickness", 1) - 1 + outlineThicknessNames.length) % outlineThicknessNames.length;
            prefs.edit().putInt("outline_thickness", index).apply();
            outlineThicknessTV.setText(outlineThicknessNames[index]);
            applySubtitleSettings();
        });

        outlineThicknessNextBtn.setOnClickListener(v -> {
            int index = (prefs.getInt("outline_thickness", 1) + 1) % outlineThicknessNames.length;
            prefs.edit().putInt("outline_thickness", index).apply();
            outlineThicknessTV.setText(outlineThicknessNames[index]);
            applySubtitleSettings();
        });
        ...
        // Edge Style controls
        edgeStylePrevBtn.setOnClickListener(v -> {
            int index = (prefs.getInt("edge_style", 0) - 1 + edgeStyleNames.length) % edgeStyleNames.length;
            prefs.edit().putInt("edge_style", index).apply();
            edgeStyleTV.setText(edgeStyleNames[index]);
            applySubtitleSettings();
        });

        edgeStyleNextBtn.setOnClickListener(v -> {
            int index = (prefs.getInt("edge_style", 0) + 1) % edgeStyleNames.length;
            prefs.edit().putInt("edge_style", index).apply();
            edgeStyleTV.setText(edgeStyleNames[index]);
            applySubtitleSettings();
        });
        ...
        boldSwitch.setOnCheckedChangeListener((buttonView, isChecked) -> {
            saveAndApplySubtitleSetting("font_bold", isChecked);
        });
        ...
        resetBtn.setOnClickListener(v -> {
            ...
            editor.putInt("outline_thickness", 1); // Normal
            editor.putInt("edge_style", 0); // Outline
            editor.putBoolean("font_bold", false);
            ...
            outlineThicknessTV.setText(outlineThicknessNames[1]);
            edgeStyleTV.setText(edgeStyleNames[0]);
