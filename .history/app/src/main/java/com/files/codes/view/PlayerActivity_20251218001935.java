    ...
    */
    ...
    private void openUnifiedSettingsDialog() {
        ...
        outlineThicknessPrevBtn.setOnClickListener(v -> {
            int current = prefs.getInt("outline_thickness", 4);
            if (current > 1) {
                current--;
                prefs.edit().putInt("outline_thickness", current).apply();
                outlineThicknessTV.setText(current + " pt");
                applySubtitleSettings();
            }
        });
