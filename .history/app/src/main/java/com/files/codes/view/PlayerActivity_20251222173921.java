...
//                }
    ...
    public void initVideoPlayer(String url, String type) {
        ...
        // Configure RenderersFactory to use extension renderers (FFmpeg) only if hardware cannot handle it
        // This allows Passthrough (TrueHD/DTS to AVR) to work if supported, 
        // while still falling back to SW Decoding if not supported.
        int extensionMode = useSoftwareAudioDecoder 
                ? DefaultRenderersFactory.EXTENSION_RENDERER_MODE_PREFER // Force SW
                : DefaultRenderersFactory.EXTENSION_RENDERER_MODE_ON;    // HW first, then SW
        ...
        applyPreferredLanguageSettings();
                        ...
                        try {
                            ...
                            if (mappedTrackInfo != null && savedAudioRendererIndex < mappedTrackInfo.getRendererCount()) {
                                ...
                                if (savedAudioOverride.groupIndex < trackGroups.length) {
                                    ...
                                    Log.d(TAG, "✅ Restored audio track selection successfully");
                                } else {
   ...
   /* @SuppressLint("StaticFieldLeak")
            ...
            if (mappedTrackInfo.getRendererType(i) == C.TRACK_TYPE_AUDIO) {
                ...
                for (int groupIndex = 0; groupIndex < trackGroups.length; groupIndex++) {
                    ...
                    for (int trackIndex = 0; trackIndex < group.length; trackIndex++) {
                        ...
                        // Append codec info for better visibility
                        if (format.sampleMimeType != null) {
                             String codec = format.sampleMimeType.replace("audio/", "").toUpperCase();
                             if (codec.contains("MP4A")) codec = "AAC";
                             else if (codec.contains("AC3")) codec = "AC3";
                             else if (codec.contains("EAC3")) codec = "EAC3";
                             else if (codec.contains("TRUEHD")) codec = "TrueHD";
                             else if (codec.contains("DTS")) codec = "DTS";
                             else if (codec.contains("OPUS")) codec = "Opus";
                             else if (codec.contains("FLAC")) codec = "FLAC";
                             
                             if (!label.contains(codec)) {
                                 label += " - " + codec;
                             }
                        }
    ...
    private void applyPreferredLanguageSettings() {
        if (trackSelector == null) return;

        String preferredAudio = PreferenceUtils.getPreferredAudio(this);
        String preferredSubtitle = PreferenceUtils.getPreferredSubtitle(this);

        Log.d(TAG, "Applying preferred languages - Audio: " + preferredAudio + ", Subtitle: " + preferredSubtitle);

        DefaultTrackSelector.Parameters.Builder builder = trackSelector.buildUponParameters();

        if (!"default".equals(preferredAudio)) {
            builder.setPreferredAudioLanguage(preferredAudio);
        }

        if (!"default".equals(preferredSubtitle)) {
            builder.setPreferredTextLanguage(preferredSubtitle);
        }
            ...
            public void run() {
                try {
                    ...
                    if (!useSoftwareAudioDecoder) {
                        ...
                        // SAVE CURRENT TRACK SELECTION BEFORE RESTART
                            ...
                            if (mappedTrackInfo != null) {
                                for (int i = 0; i < mappedTrackInfo.getRendererCount(); i++) {
                                    if (mappedTrackInfo.getRendererType(i) == C.TRACK_TYPE_AUDIO) {
                                        ...
                                        DefaultTrackSelector.SelectionOverride override = parameters.getSelectionOverride(i, mappedTrackInfo.getTrackGroups(i));
                                        if (override != null) {
                                            savedAudioOverride = override;
                                            savedAudioRendererIndex = i;
                                            Log.d("PlayerActivity", "💾 Saved audio track selection: Group=" + override.groupIndex + ", Track=" + override.tracks[0]);
                                        }
                                        break; // Only save the first active audio renderer override
                                    }
                                }
                            }
                        }
