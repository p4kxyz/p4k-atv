    ...
    public void initVideoPlayer(String url, String type) {
        ...
        player.addListener(new Player.Listener() {
            ...
            public void onTracksChanged(TrackGroupArray trackGroups, TrackSelectionArray trackSelections) {
                Log.d(TAG, "🎵 onTracksChanged triggered");
                ...
                if (mappedTrackInfo != null) {
                    for (int i = 0; i < mappedTrackInfo.getRendererCount(); i++) {
                        ...
                        switch(rendererType) {
                            case C.TRACK_TYPE_AUDIO: typeName = "AUDIO"; break;
                            case C.TRACK_TYPE_VIDEO: typeName = "VIDEO"; break;
                            case C.TRACK_TYPE_TEXT: typeName = "TEXT"; break;
                        }
                        
                        Log.d(TAG, "   Renderer " + i + " [" + typeName + "]");
                        
                        TrackSelection selection = trackSelections.get(i);
                        if (selection != null) {
                            Format format = selection.getSelectedFormat();
                            Log.d(TAG, "     👉 SELECTED: " + format.sampleMimeType + 
                            ...
                            Log.d(TAG, "     ❌ NO SELECTION (Renderer disabled or no track selected)");
                            
                            // Check if this renderer has potential tracks but none selected
                            TrackGroupArray rendererTrackGroups = mappedTrackInfo.getTrackGroups(i);
                            if (rendererTrackGroups.length > 0) {
                                Log.d(TAG, "     ⚠️ Renderer has " + rendererTrackGroups.length + " track groups available but none selected");
                            }
                        }
                    }
                } else {
                    Log.w(TAG, "⚠️ onTracksChanged: MappedTrackInfo is null");
                }
            }
