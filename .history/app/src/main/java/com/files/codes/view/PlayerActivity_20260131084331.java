...
//                }
    ...
    public void initVideoPlayer(String url, String type) {
        ...
        Log.d("PlayerActivity", "🔊 Extension Renderer Mode: " + 
               (extensionMode == DefaultRenderersFactory.EXTENSION_RENDERER_MODE_PREFER ? "PREFER (Software Force)" : "ON (Hardware/Extension mix)") + 
               " | useSoftwareAudioDecoder=" + useSoftwareAudioDecoder);
                
        // Create a custom DefaultRenderersFactory that only enables ONE audio renderer at a time
        // This is necessary because enabling multiple audio renderers (HW + SW) even if only one track is selected
        // can sometimes cause "Multiple renderer media clocks enabled" on some buggy devices/ROMs.
        RenderersFactory renderersFactory = new DefaultRenderersFactory(this) {
            ...
            public void buildAudioRenderers(Context context, int extensionRendererMode, MediaCodecSelector mediaCodecSelector, boolean enableDecoderFallback, AudioSink audioSink, Handler eventHandler, AudioRendererEventListener eventListener, ArrayList<Renderer> out) {
                // If software decoder is forced, ONLY add the extension renderer (FFmpeg)
                // Do NOT add MediaCodecAudioRenderer (Hardware) to avoid conflicts
                if (useSoftwareAudioDecoder) {
                     Log.d("PlayerActivity", "🔨 Building ONLY Software Audio Renderer (FFmpeg)");
                     ...
                     // Remove Hardware Audio Renderer if it was added by super (just to be safe)
                     // Usually EXTENSION_RENDERER_MODE_PREFER puts extension first, but we want to ensure uniqueness of Media Clock
                     // However, DefaultRenderersFactory might still add MediaCodecAudioRenderer.
                     // The safest way is to trust the mode, but if crash persists, we might need to filter 'out' list.
                } else {
                     Log.d("PlayerActivity", "🔨 Building Standard Audio Renderers (HW + SW fallback)");
