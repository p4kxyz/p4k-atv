    ...
    */
    ...
    private void launchMXPlayerHWPlus(String videoUrl) {
        ...
        try {
            ...
            // Force HW+ decode mode (FFmpeg decode + GPU render)
            intent.putExtra("decode_mode", 2); // 0=HW, 1=SW, 2=HW+
            ...
            // Pass current position for resume
            if (player != null && player.getCurrentPosition() > 0) {
                intent.putExtra("position", (int) player.getCurrentPosition());
            }
            
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            Log.d(TAG, "🎬 Launching MX Player (HW+) for: " + videoUrl);
            startActivity(intent);
        } catch (Exception e) {
