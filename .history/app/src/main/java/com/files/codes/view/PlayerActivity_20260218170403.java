...
//                }
        ...
        player.addListener(new Player.Listener() {
                    ...
                    try {
                        ...
                        if (videoCheckInfo != null) {
                            ...
                            if (hasVideoTracks && !videoSelected) {
                                Log.e("PlayerActivity", "⚠️ VIDEO CODEC NOT SUPPORTED: " + unsupportedCodec + " - Thiết bị không hỗ trợ giải mã codec này");
            ...
            public void onPlayerError(androidx.media3.common.PlaybackException error) {
                ...
                if (isVideoCodecError) {
                    ...
                    Log.e("PlayerActivity", "❌ HTTP Error: " + responseCode);
                    
                    if (responseCode == 416) {
                        // HTTP 416 Range Not Satisfiable - server rejected Range request
                        if (!has416Retried) {
                            has416Retried = true;
                            Log.e("PlayerActivity", "🔄 HTTP 416: Range Not Satisfiable - retrying with fresh MediaSource");
                            ...
                            Log.e("PlayerActivity", "❌ HTTP 416 persists after retry - showing error");
                            showErrorDialog("Lỗi phát video", "Server từ chối yêu cầu Range (416). Video có thể không khả dụng.");
                        }
                    } else if (responseCode == 403) {
                        showErrorDialog("Lỗi truy cập", "Server từ chối truy cập (403). Link phát có thể đã hết hạn.");
                    } else if (responseCode == 404) {
                        showErrorDialog("Không tìm thấy", "Video không tồn tại trên server (404).");
                    } else if (responseCode >= 500) {
                        showErrorDialog("Lỗi server", "Server gặp sự cố (" + responseCode + "). Vui lòng thử lại sau.");
                    } else {
                        showErrorDialog("Lỗi HTTP", "Lỗi HTTP " + responseCode + ". Vui lòng thử lại.");
                    }
    ...
    */
    ...
    private void launchMXPlayerHWPlus(String videoUrl) {
        ...
        if (mxPackage == null) {
            Log.e(TAG, "❌ MX Player not installed");
            return;
        }
        ...
        try {
            ...
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            Log.d(TAG, "🎬 Launching MX Player (HW+) for: " + videoUrl + " [package=" + mxPackage + "]");
            startActivity(intent);
        } catch (android.content.ActivityNotFoundException e) {
            Log.e(TAG, "❌ MX Player Activity not found, trying generic intent", e);
            ...
            try {
                ...
                Log.e(TAG, "❌ Failed to launch MX Player (fallback)", e2);
                android.widget.Toast.makeText(this, "Không thể mở MX Player", android.widget.Toast.LENGTH_SHORT).show();
            }
        } catch (Exception e) {
    ...
    private void downloadMXPlayer() {
        try {
            ...
            android.widget.Toast.makeText(this, "Đang tải MX Player... Sau khi cài xong, mở lại video này.", android.widget.Toast.LENGTH_LONG).show();
        } catch (Exception e) {
            Log.e(TAG, "❌ Failed to open download URL", e);
            android.widget.Toast.makeText(this, "Không thể mở link tải", android.widget.Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void showUnsupportedCodecDialog(String codecInfo) {
        ...
        builder.setTitle("⚠️ Codec video không hỗ trợ");
        
        if (hasMXPlayer) {
            builder.setMessage("Codec \"" + codecInfo + "\" không được thiết bị hỗ trợ.\n\n" +
                "Video chỉ có âm thanh, không có hình ảnh.\n\n" +
                "Bạn có thể mở bằng MX Player (chế độ HW+) để phát mượt.");
            builder.setPositiveButton("Mở MX Player", (dialog, which) -> {
                launchMXPlayerHWPlus(url);
            });
            builder.setNegativeButton("Tiếp tục nghe", (dialog, which) -> {
                dialog.dismiss();
            });
        } else {
            builder.setMessage("Codec \"" + codecInfo + "\" không được thiết bị hỗ trợ.\n\n" +
                "Video chỉ có âm thanh, không có hình ảnh.\n\n" +
                "Cài MX Player để phát các video có codec đặc biệt.");
            builder.setPositiveButton("Tải MX Player", (dialog, which) -> {
                downloadMXPlayer();
            });
            builder.setNegativeButton("Tiếp tục nghe", (dialog, which) -> {
                dialog.dismiss();
