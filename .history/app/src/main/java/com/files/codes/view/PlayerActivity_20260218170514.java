...
public class PlayerActivity extends Activity {
    ...
    private boolean has416Retried = false; // Prevent infinite 416 retry loop
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d(TAG, "ðŸ”Š Audio Mode Initialized: " + audioMode + " (1:SW, 0:HW, 2:PASSTROUGH)");
        ...
        if (intent != null && intent.hasExtra("complete_movie_data")) {
            ...
            // Log.d(TAG, "ðŸ“¥ onCreate - Received complete movie data: " + (completeMovieData != null ? completeMovieData.getTitle() : "null"));
            
            // Debug complete movie data immediately
            if (completeMovieData != null) {
                /*
                Log.d(TAG, "ðŸ” onCreate - Complete movie data debug:");
                ...
                */
                ...
                if (!hasCompleteInfo && completeMovieData.getVideosId() != null && !completeMovieData.getVideosId().equals("null")) {
                    // Log.w(TAG, "âš ï¸ CompleteMovieData incomplete, fetching from API for ID: " + completeMovieData.getVideosId());
            ...
            // Log.d(TAG, "ðŸ“¥ onCreate - No complete movie data provided, will fetch from API");
        }
        ...
        if (model == null && intent != null && intent.hasExtra("id")) {
            ...
            // Log.d(TAG, "ðŸ“¥ onCreate - Watch History Intent: fromWatchHistory=" + fromWatchHistory + ", ID=" + videoId + ", Type=" + videoType);
            ...
            if (videoType != null && (videoType.equalsIgnoreCase("tvseries") || videoType.equalsIgnoreCase("tv") || videoType.equalsIgnoreCase("episode"))) {
                ...
                // Log.d(TAG, "ðŸ“¥ Set isTvSeries = 1 for type: " + videoType);
            } else {
                model.setIsTvSeries("0");
                // Log.d(TAG, "ðŸ“¥ Set isTvSeries = 0 for type: " + videoType);
            }
        ...
        if (model == null) {
            new ToastMsg(PlayerActivity.this).toastIconError("Lá»—i: KhÃ´ng cÃ³ dá»¯ liá»‡u video");
            finish();
            return;
        }
        ...
        if (completeMovieData == null && model != null && model.getMovieId() != null && 
            ...
            // Log.d(TAG, "ðŸ” Fetching complete movie data early for ID: " + model.getMovieId());
            fetchCompleteMovieDataForWatchHistory(model.getMovieId(), model.getVideoType() != null ? model.getVideoType() : "movie");
        } else if (model != null) {
            // Log.w(TAG, "âš ï¸ Cannot fetch complete movie data - MovieId: " + model.getMovieId() + ", CompleteData: " + (completeMovieData != null ? "exists" : "null"));
        }
        ...
        Log.e(TAG, "ðŸ” onCreate - External Player Check: " + useExternalPlayer);

        if (useExternalPlayer) {
            Log.e(TAG, "ðŸš€ onCreate - Launching External Player sequence...");
    ...
    private void intiViews() {
        ...
        Log.d(TAG, "ðŸŽ¬ Subtitle button setup - subtitleButton: " + (subtitleButton != null ? "FOUND âœ…" : "NULL âŒ"));
        Log.d(TAG, "ðŸŽ¬ Subtitle settings button setup - subtitleSettingsButton: " + (subtitleSettingsButton != null ? "FOUND âœ…" : "NULL âŒ"));
        ...
        Log.d(TAG, "ðŸŽ¬ Seek buttons setup - Rewind: " + (btnRewind != null ? "FOUND" : "NULL") + 
    ...
    protected void onStart() {
        ...
        if (subtitleButton != null) {
            subtitleButton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    Log.d(TAG, "ðŸŽ¬ SUBTITLE BUTTON CLICKED! Opening subtitle dialog...");
        ...
        if (movieTitleTV != null) {
            ...
            // Example: "TÃªn Viá»‡t (2024) TÃªn Gá»‘c" -> "TÃªn Viá»‡t (2024)"
            if (titleText != null) {
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if (event.getAction() == KeyEvent.ACTION_DOWN && exoPlayerView != null && exoPlayerView.isControllerFullyVisible()) {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_BACK:
                ...
                if (exoPlayerView.isControllerFullyVisible()) {
                    ...
                    Log.d(TAG, "ðŸ”™ KEYCODE_BACK - From watch history: " + fromWatchHistory);
                    
                    if (fromWatchHistory && model != null) {
                        Log.d(TAG, "ðŸ”™ Redirecting to HeroStyleVideoDetailsActivity - ID: " + model.getMovieId() + ", Type: " + model.getCategory());
...
//                }
    ...
    private void launchExternalPlayer() {
        try {
            ...
            if (videoUrl == null || videoUrl.isEmpty()) {
                new ToastMsg(this).toastIconError("URL video khÃ´ng há»£p lá»‡");
                return;
            }
            ...
            if (!originalUrl.equals(videoUrl)) {
                Log.w(TAG, "ðŸ§¹ URL cleaned - Original: '" + originalUrl + "'");
                Log.d(TAG, "ðŸ§¹ URL cleaned - Clean: '" + videoUrl + "'");
            }
            
            // Validate and log video URL
            Log.e(TAG, "ðŸ” Video URL: " + videoUrl);
            Log.e(TAG, "ðŸŽ¬ Movie Title: " + (model.getTitle() != null ? model.getTitle() : "Unknown"));
            
            // Check for problematic URLs that might cause issues
            if (isProblematicUrl(videoUrl)) {
                Log.w(TAG, "âš ï¸ Detected potentially problematic URL, using fallback");
            ...
            Log.e(TAG, "ðŸŽ® Selected Player: " + selectedPlayer);
            ...
            new ToastMsg(this).toastIconError("Lá»—i khi má»Ÿ trÃ¬nh phÃ¡t ngoÃ i: " + e.getMessage());
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        Log.e(TAG, "ðŸ“¥ onActivityResult - RequestCode: " + requestCode + ", ResultCode: " + resultCode);
        ...
        if (resultCode == Activity.RESULT_OK && data != null) {
            ...
            if (requestCode == 1001) {
                ...
                Log.e(TAG, "ðŸ“¥ MX Player Result - Position: " + position + ", Duration: " + duration);
            }
            // VLC Player (1002)
            else if (requestCode == 1002) {
                position = data.getLongExtra("extra_position", -1);
                duration = data.getLongExtra("extra_duration", -1);
                Log.e(TAG, "ðŸ“¥ VLC Player Result - Position: " + position + ", Duration: " + duration);
            }
            // Just Player (1003)
            else if (requestCode == 1003) {
                position = data.getLongExtra("position", -1);
                duration = data.getLongExtra("duration", -1);
                Log.e(TAG, "ðŸ“¥ Just Player Result - Position: " + position + ", Duration: " + duration);
            }

            // Save watch history if valid position returned
            if (position > 0) {
                Log.e(TAG, "ðŸ’¾ Saving watch history from External Player: " + position + "ms");
                ...
                if (lastPlayedIndex == -1) {
                    ...
                    if (returnedUri != null && model != null && model.getAllSeasons() != null) {
                        Log.d(TAG, "ðŸ”„ MX Player returned URI: " + returnedUri + ". Searching for match...");
                                ...
                                for (Episode episode : season.getEpisodes()) {
                                    ...
                                    if (epUrl != null) {
                                        ...
                                        if (cleanEpUrl.equals(returnedUriString) || returnedUriString.contains(cleanEpUrl) || cleanEpUrl.contains(returnedUriString)) {
                                            Log.d(TAG, "âœ… Found matching episode by URI at index: " + currentIndex);
                ...
                if (lastPlayedIndex != -1 && model != null && model.getAllSeasons() != null) {
                    Log.d(TAG, "ðŸ”„ MX Player returned index: " + lastPlayedIndex + ". Updating model to correct episode.");
                ...
                new ToastMsg(this).toastIconSuccess("ÄÃ£ lÆ°u lá»‹ch sá»­ xem tá»« trÃ¬nh phÃ¡t ngoÃ i");
            } else {
                Log.e(TAG, "âš ï¸ Position is 0 or invalid, not saving history.");
            }
        } else {
            Log.e(TAG, "âš ï¸ onActivityResult - Result not OK or Data null. ResultCode: " + resultCode);
        }
    ...
    private void launchP4KPlayer(String videoUrl) {
        try {
            ...
            Log.d(TAG, "ðŸŽ¬ Launching P4K Player - URL: " + videoUrl);
            Log.d(TAG, "ðŸ“¡ User-Agent: " + userAgent);
            ...
            new ToastMsg(this).toastIconError("Lá»—i khi má»Ÿ P4K Player: " + e.getMessage());
        }
    }
    ...
    private void launchKodiPlayer(String videoUrl) {
        try {
            ...
            try {
                ...
                Log.d(TAG, "ðŸŽ¬ Trying Kodi direct action - URL: " + videoUrl);
                startActivity(intent);
                launched = true;
                
            } catch (Exception e) {
                Log.d(TAG, "âš ï¸ Kodi direct action failed, trying standard method: " + e.getMessage());
            }
            ...
            if (!launched) {
                try {
                    ...
                    Log.d(TAG, "ðŸŽ¬ Launching Kodi standard method - URL: " + videoUrl);
                    Log.d(TAG, "ðŸ“¡ User-Agent: " + userAgent);
                    startActivity(intent);
                    launched = true;
                    
                } catch (Exception e) {
                    Log.e(TAG, "âŒ Standard Kodi launch failed: " + e.getMessage());
                }
            }
                        ...
                        new Handler().postDelayed(new Runnable() {
                            ...
                            public void run() {
                                try {
                                    ...
                                    Log.e(TAG, "âŒ Delayed Kodi video send failed: " + e.getMessage());
                                }
                            }
                        }, 2000);
                        
                        launched = true;
                        Log.d(TAG, "ðŸŽ¬ Launched Kodi with delayed video send");
                    }
                } catch (Exception e) {
                    Log.e(TAG, "âŒ Kodi fallback launch failed: " + e.getMessage());
                }
            }
            ...
            if (launched) {
                ...
                new ToastMsg(this).toastIconError("KhÃ´ng thá»ƒ má»Ÿ Kodi trÃªn Google TV. HÃ£y thá»­ trÃ¬nh phÃ¡t khÃ¡c.");
            }
            
        } catch (Exception e) {
            Log.e(TAG, "Error launching Kodi", e);
            new ToastMsg(this).toastIconError("Lá»—i khi má»Ÿ Kodi: " + e.getMessage());
        }
    }
    ...
    private void launchJustPlayer(String videoUrl) {
        try {
            ...
            try {
                ...
                Log.d(TAG, "ðŸ“¦ Just Player package check: " + (isInstalled ? "INSTALLED" : "NOT FOUND"));
            } catch (PackageManager.NameNotFoundException e) {
                Log.d(TAG, "ðŸ“¦ Just Player package NOT FOUND: " + e.getMessage());
                isInstalled = false;
            } catch (Exception e) {
                Log.e(TAG, "ðŸ“¦ Just Player package check ERROR: " + e.getMessage());
                isInstalled = false;
            }
            
            if (!isInstalled) {
                Log.d(TAG, "âŒ Just Player not installed, showing install dialog");
            ...
            if (getPackageManager().resolveActivity(testIntent, 0) == null) {
                Log.d(TAG, "âŒ Just Player cannot handle this intent, showing install dialog");
                showPlayerInstallDialog("Just Player", "com.brouken.player");
                return;
            }
            ...
            Log.d(TAG, "ðŸŽ¬ Launching Just Player - URL: " + videoUrl);
            Log.d(TAG, "ðŸ“¡ User-Agent: " + userAgent);
            ...
            Log.e(TAG, "âŒ ActivityNotFoundException for Just Player", e);
            new ToastMsg(this).toastIconError("Just Player chÆ°a Ä‘Æ°á»£c cÃ i Ä‘áº·t hoáº·c khÃ´ng tÆ°Æ¡ng thÃ­ch");
            // Fallback to internal player
            fallbackToInternalPlayer();
        } catch (Exception e) {
            Log.e(TAG, "Error launching Just Player", e);
            new ToastMsg(this).toastIconError("Lá»—i khi má»Ÿ Just Player: " + e.getMessage());
            // Fallback to internal player
            fallbackToInternalPlayer();
        }
    }
    ...
    private void launchMXPlayer(String videoUrl) {
        try {
            ...
            if (model != null && model.getAllSeasons() != null && model.getAllSeasons().size() > 0) {
                try {
                    ...
                    if (videoList.size() > 1 && currentPosition != -1) {
                        ...
                        Log.d(TAG, "ðŸ“º Added playlist to MX Player: " + videoList.size() + " episodes, starting at " + currentPosition);
                    }
                } catch (Exception e) {
            ...
            Log.e(TAG, "ðŸŽ¬ Launching MX Player - URL: " + videoUrl);
            Log.e(TAG, "ðŸ“¡ User-Agent: " + userAgent);
            ...
            new ToastMsg(this).toastIconError("Lá»—i khi má»Ÿ MX Player: " + e.getMessage());
        }
    }
    ...
    private void launchVimuPlayer(String videoUrl) {
        try {
            ...
            Log.d(TAG, "ðŸŽ¬ Launching Vimu Player - URL: " + videoUrl);
            Log.d(TAG, "ðŸ“¡ User-Agent: " + userAgent);
            ...
            new ToastMsg(this).toastIconError("Lá»—i khi má»Ÿ Vimu Player: " + e.getMessage());
        }
    }
    ...
    private void launchVLCPlayer(String videoUrl) {
        try {
            ...
            Log.d(TAG, "ðŸŽ¬ Launching VLC Player - URL: " + videoUrl);
            Log.d(TAG, "ðŸ“¡ User-Agent: " + userAgent);
            ...
            new ToastMsg(this).toastIconError("Lá»—i khi má»Ÿ VLC Player: " + e.getMessage());
        }
    }
    ...
    private void launchNPlayer(String videoUrl) {
        try {
            ...
            Log.d(TAG, "ðŸŽ¬ Launching nPlayer (" + installedPackage + ") - URL: " + videoUrl);
            Log.d(TAG, "ðŸ“¡ User-Agent: " + userAgent);
            ...
            new ToastMsg(this).toastIconError("Lá»—i khi má»Ÿ nPlayer: " + e.getMessage());
        }
    }
    ...
    private void launchDuneHDRealtek(String videoUrl) {
        try {
            ...
            Log.d(TAG, "ðŸŽ¬ Launching Dune HD (Realtek) - URL: " + videoUrl);
            Log.d(TAG, "ðŸ“¡ User-Agent: " + userAgent);
            ...
            new ToastMsg(this).toastIconError("Lá»—i khi má»Ÿ Dune HD: " + e.getMessage());
        }
    }
    ...
    private void launchDuneHDAmlogic(String videoUrl) {
        try {
            ...
            Log.d(TAG, "ðŸŽ¬ Launching Dune HD (Amlogic) - URL: " + videoUrl);
            Log.d(TAG, "ðŸ“¡ User-Agent: " + userAgent);
    ...
    private void launchZidooRealtek(String videoUrl) {
        try {
            ...
            Log.d(TAG, "ðŸŽ¬ Launching Zidoo (Realtek) - URL: " + videoUrl);
            Log.d(TAG, "ðŸ“¡ User-Agent: " + userAgent);
            ...
            new ToastMsg(this).toastIconError("Lá»—i khi má»Ÿ Zidoo: " + e.getMessage());
        }
    }
    ...
    private void launchZidooAmlogic(String videoUrl) {
        try {
            ...
            Log.d(TAG, "ðŸŽ¬ Launching Zidoo (Amlogic - Dolby Vision) - URL: " + videoUrl);
            Log.d(TAG, "ðŸ“¡ User-Agent: " + userAgent);
    ...
    private void showPlayerInstallDialog(final String playerName, final String packageName) {
        ...
        builder.setMessage(playerName + " chÆ°a Ä‘Æ°á»£c cÃ i Ä‘áº·t trÃªn thiáº¿t bá»‹ cá»§a báº¡n. Báº¡n cÃ³ muá»‘n cÃ i Ä‘áº·t khÃ´ng?");
    ...
    private String cleanVideoUrl(String videoUrl) {
        ...
        Log.d(TAG, "ðŸ§¹ URL cleaning result: '" + cleaned + "'");
        return cleaned;
    }
    ...
    private void showUrlIssueDialog(String videoUrl) {
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("Váº¥n Ä‘á» URL Video");
        builder.setMessage("URL video nÃ y cÃ³ thá»ƒ gÃ¢y lá»—i vá»›i trÃ¬nh phÃ¡t ngoÃ i. Báº¡n muá»‘n:\n\nâ€¢ Thá»­ internal player\nâ€¢ Hoáº·c thá»­ external player khÃ¡c?");
        
        builder.setPositiveButton("Internal Player", (dialog, which) -> {
            Log.d(TAG, "ðŸŽ¯ User chose internal player for problematic URL");
            fallbackToInternalPlayer();
        });
        
        builder.setNegativeButton("Thá»­ External Player", (dialog, which) -> {
            Log.d(TAG, "ðŸŽ¯ User chose to try external player anyway");
            dialog.dismiss();
            // Continue with external player
            continueExternalPlayerLaunch();
        });
        
        builder.setNeutralButton("Há»§y", (dialog, which) -> {
            dialog.dismiss();
            finish();
        });
    ...
    private void fallbackToInternalPlayer() {
        Log.d(TAG, "ðŸ”„ Falling back to internal player");
        
        // Clear external player preference temporarily for this session
        getWindow().clearFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
        
        // Continue with internal player setup
        new ToastMsg(this).toastIconSuccess("Chuyá»ƒn sang trÃ¬nh phÃ¡t ná»™i bá»™");
    ...
    private void openSubtitleDialog() {
        // Log.d(TAG, "ðŸŽ¬ openSubtitleDialog() called");
        ...
        if (video == null || video.getSubtitle() == null || video.getSubtitle().isEmpty()) {
            // Log.d(TAG, "ðŸŽ¬ No external subtitle data found, loading from API...");
        ...
        adapter.setListener(new SubtitleListAdapter.OnSubtitleItemClickListener() {
            ...
            public void onSubtitleItemClick(View view, Subtitle subtitle, int position, SubtitleListAdapter.SubtitleViewHolder holder) {
                ...
                // Log.d(TAG, "ðŸŽ¬ Selected subtitle: " + subtitle.getLanguage() + ", URL: " + url);
    ...
    private void setPlaybackSpeed(float speed) {
        if (player != null) {
            ...
            new ToastMsg(this).toastIconSuccess("Tá»‘c Ä‘á»™ phÃ¡t: " + speed + "x");
        }
    }

    private void setSelectedSubtitle(androidx.media3.exoplayer.source.MediaSource mediaSource, String url) {
        ...
        if (url != null) {
            ...
            androidx.media3.exoplayer.source.MediaSource subtitleSource = new SingleSampleMediaSource
            ...
            Toast.makeText(PlayerActivity.this, "KhÃ´ng tÃ¬m tháº¥y phá»¥ Ä‘á»", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void initVideoPlayer(String url, String type) {
        ...
        trackSelector = new
                DefaultTrackSelector(PlayerActivity.this);
        ...
        Log.d("PlayerActivity", "ðŸ”Š Track selector configured. Tunneling: " + tunnelingEnabled);
        ...
        Log.d("PlayerActivity", "ðŸ”Š Extension Renderer Mode Target: " + finalExtensionMode);
        ...
        RenderersFactory renderersFactory = new DefaultRenderersFactory(this) {
             ...
             */
            @Override
            protected androidx.media3.exoplayer.audio.AudioSink buildAudioSink(Context context, boolean enableFloatOutput, boolean enableAudioOutputPlaybackParams) {
                if (audioMode == 2) { 
                    Log.d("PlayerActivity", "ðŸ”Š Passthrough Mode: Forcing AudioSink capabilities");
                    return new androidx.media3.exoplayer.audio.DefaultAudioSink.Builder()
                        .setAudioCapabilities(new androidx.media3.exoplayer.audio.AudioCapabilities(
                            new int[] {
                                androidx.media3.common.C.ENCODING_PCM_16BIT,
                                androidx.media3.common.C.ENCODING_AC3,
                                androidx.media3.common.C.ENCODING_E_AC3,
                                androidx.media3.common.C.ENCODING_DTS,
                                androidx.media3.common.C.ENCODING_DTS_HD,
                                androidx.media3.common.C.ENCODING_DOLBY_TRUEHD
                            }, 
                            10
                        ))
                        .build();
            ...
            @Override
            protected void buildAudioRenderers(Context context, int extensionRendererMode, androidx.media3.exoplayer.mediacodec.MediaCodecSelector mediaCodecSelector, boolean enableDecoderFallback, androidx.media3.exoplayer.audio.AudioSink audioSink, Handler eventHandler, androidx.media3.exoplayer.audio.AudioRendererEventListener eventListener, java.util.ArrayList<androidx.media3.exoplayer.Renderer> out) {
                ...
                if (audioMode == 1) { // Software Mode
                     Log.d("PlayerActivity", "ðŸ”¨ FORCE SW MODE: Requesting Extension Renderer Prefer...");
                     ...
                     for (androidx.media3.exoplayer.Renderer r : out) {
                         if (r.getClass().getSimpleName().contains("Ffmpeg") || r.getClass().getSimpleName().contains("Lib")) {
                             hasExtension = true;
                             Log.d("PlayerActivity", "âœ… Software Renderer detected in list: " + r.getClass().getSimpleName());
                         }
                     }
                     if (!hasExtension) {
                         Log.w("PlayerActivity", "âš ï¸ Software requested but no Extension Renderer found in final list!");
                     }
                     
                } else if (audioMode == 0) { // Hardware Mode
                     // ... existing HW logic ...
                     Log.d("PlayerActivity", "ðŸ”¨ FORCE HW MODE: Building MediaCodec Renderers Only");
                     ...
                     Log.d("PlayerActivity", "ðŸ”¨ PASSTHROUGH MODE: Building Renderers (No Extensions)");
        ...
        player.addAnalyticsListener(new androidx.media3.exoplayer.analytics.AnalyticsListener() {
            ...
            public void onAudioDecoderInitialized(EventTime eventTime, String decoderName, long initializationDurationMs) {
                Log.e("PlayerActivity", "ðŸŽ§ Audio Decoder Initialized: " + decoderName); // Log as Error to see in user's filter
                runOnUiThread(() -> {
                     new ToastMsg(PlayerActivity.this).toastIconSuccess("Ã‚m thanh: " + decoderName);
                });
            }
            @Override
            public void onAudioCodecError(EventTime eventTime, Exception audioCodecError) {
                Log.e("PlayerActivity", "ðŸŽ§ Audio Codec Error: " + audioCodecError.getMessage());
                 runOnUiThread(() -> {
                     new ToastMsg(PlayerActivity.this).toastIconError("Lá»—i Codec: " + audioCodecError.getMessage());
                });
            }
        });
        ...
        Log.d("PlayerActivity", "ðŸŽ¬ initVideoPlayer called with URL: " + url + ", Type: " + type);
        ...
        // Auto-detect format when type is generic (video, tvseries, movie, null, etc.)
        if ("video".equals(type) || type == null || type.isEmpty() || 
            "tvseries".equals(type) || "movie".equals(type)) {
            detectedType = detectVideoFormat(url);
            Log.d("PlayerActivity", "ðŸ” Auto-detected format: " + detectedType + " from URL: " + url);
        } else {
            Log.d("PlayerActivity", "ðŸ” Using provided type: " + detectedType);
        }

        switch (detectedType) {
            case "hls":
            case "m3u8":
                Log.d("PlayerActivity", "ðŸŽ¬ Using HLS MediaSource for: " + url);
           ...
           /* case "youtube":
            ...
            case "youtube-live":
                ...
                break;*/
            case "rtmp":
                Log.d("PlayerActivity", "ðŸŽ¬ Using RTMP MediaSource for: " + url);
                mediaSource = rtmpMediaSource(uri);
                break;
            default:
                Log.d("PlayerActivity", "ðŸŽ¬ Using Progressive MediaSource for: " + url);
                mediaSource = mediaSource(uri, PlayerActivity.this);
                break;
        }

        if (type == null || !type.contains("youtube")) {
        ...
        player.addListener(new Player.Listener() {
            ...
            public void onPlaybackStateChanged(int playbackState) {
                if (player.getPlayWhenReady() && playbackState == Player.STATE_READY) {
                    ...
                    lastErrorTime = 0;
                    Log.d(TAG, "âœ… Player ready and playing - error flags cleared");
                    ...
                    Log.d(TAG, "ðŸ” PLAYER READY: Inspecting Audio Tracks...");
                    ...
                    if (debugMappedTrackInfo != null) {
                        for (int i = 0; i < debugMappedTrackInfo.getRendererCount(); i++) {
                            ...
                            if (rendererType == C.TRACK_TYPE_AUDIO) {
                                androidx.media3.exoplayer.trackselection.TrackSelection selection = player.getCurrentTrackSelections().get(i);
                                if (selection != null) {
                                    Log.d(TAG, "   ðŸ”Š AUDIO RENDERER [" + i + "] ACTIVE");
                                    ...
                                    Log.d(TAG, "   ðŸ”‡ AUDIO RENDERER [" + i + "] NOT SELECTED");
                    ...
                    // VIDEO CODEC CHECK: Detect unsupported video codec (e.g. VC-1) causing black screen
                    try {
                        MappingTrackSelector.MappedTrackInfo videoCheckInfo = trackSelector.getCurrentMappedTrackInfo();
                        if (videoCheckInfo != null) {
                            boolean hasVideoTracks = false;
                            boolean videoSelected = false;
                            String unsupportedCodec = "";
                            
                            for (int i = 0; i < videoCheckInfo.getRendererCount(); i++) {
                                if (videoCheckInfo.getRendererType(i) == C.TRACK_TYPE_VIDEO) {
                                    TrackGroupArray groups = videoCheckInfo.getTrackGroups(i);
                                    if (groups.length > 0) {
                                        hasVideoTracks = true;
                                        // Check what codec the video track uses
                                        for (int g = 0; g < groups.length; g++) {
                                            for (int t = 0; t < groups.get(g).length; t++) {
                                                Format f = groups.get(g).getFormat(t);
                                                if (f.sampleMimeType != null) {
                                                    unsupportedCodec = f.sampleMimeType;
                                                    if (f.codecs != null) unsupportedCodec += " (" + f.codecs + ")";
                                                }
                                            }
                                        }
                                    }
                            ...
                            if (hasVideoTracks && !videoSelected) {
                                Log.e("PlayerActivity", "âš ï¸ VIDEO CODEC NOT SUPPORTED: " + unsupportedCodec + " - Thiáº¿t bá»‹ khÃ´ng há»— trá»£ giáº£i mÃ£ codec nÃ y");
                                final String codecInfo = unsupportedCodec;
                                runOnUiThread(() -> showUnsupportedCodecDialog(codecInfo));
                            }
                        }
                    } catch (Exception e) {
                        Log.e("PlayerActivity", "Error checking video codec support", e);
                    }
                        ...
                        try {
                            ...
                            if (trackInfo != null) {
                                ...
                                if (hasAudioTracks && !audioSelected) {
                                    Log.e("PlayerActivity", "ðŸ”‡ SILENT FAILURE DETECTED: Audio tracks exist but HW decoder rejected them. Switching to Software...");
                                    runOnUiThread(() -> {
                    ...
                    if (savedAudioOverride != null && savedAudioRendererIndex != -1) {
                        Log.d(TAG, "ðŸ”„ Restoring saved audio track selection...");
                        try {
                            MappingTrackSelector.MappedTrackInfo mappedTrackInfo = trackSelector.getCurrentMappedTrackInfo();
                            if (mappedTrackInfo != null && savedAudioRendererIndex < mappedTrackInfo.getRendererCount()) {
                                ...
                                if (savedAudioOverride.groupIndex < trackGroups.length) {
                                    ...
                                    Log.d(TAG, "âœ… Restored audio track selection successfully (Exclusive Mode)");
                                } else {
                                    Log.w(TAG, "âš ï¸ Cannot restore audio track: Group index out of bounds");
                                }
                            }
                        } catch (Exception e) {
                            Log.e(TAG, "âŒ Failed to restore audio track selection", e);
                        }
                        // Clear saved state
                    ...
                    Log.d(TAG, "ðŸ“º Video ended - checking for auto-play next episode");
                    Log.d(TAG, "ðŸ” Category: " + category);
                    Log.d(TAG, "ðŸ” hasPlayerError: " + hasPlayerError);
                    Log.d(TAG, "ðŸ” canNavigateToNext: " + canNavigateToNextEpisode());
                    ...
                    if (hasPlayerError || recentError) {
                        Log.d(TAG, "ðŸš« Video ended due to player error (flag=" + hasPlayerError + ", recent=" + recentError + ") - auto-play disabled");
                    ...
                    Log.d(TAG, "ðŸ“º Video progress: " + (currentPosition/1000) + "s / " + (duration/1000) + "s");
                    ...
                    if (duration > 0 && currentPosition > 0) {
                        ...
                        Log.d(TAG, "ï¿½ Progress percentage: " + String.format("%.1f", progressPercentage) + "%");
                        
                        // Only prevent auto-next if video barely started (less than 10% watched)
                        if (progressPercentage < 10.0) {
                            Log.d(TAG, "ðŸš« Video ended at " + String.format("%.1f", progressPercentage) + "% - likely an error, auto-play disabled");
                            return;
                        }
                    }
                    ...
                    if (category.equalsIgnoreCase("tvseries") && canNavigateToNextEpisode()) {
                        Log.d(TAG, "âœ… Video completed normally - auto-navigating to next episode");
                        navigateToNextEpisode();
                    } else {
                        Log.d(TAG, "ðŸ“º No next episode or not a TV series - video ended normally");
                    }
            ...
            public void onPositionDiscontinuity(Player.PositionInfo oldPosition, Player.PositionInfo newPosition, int reason) {
                ...
                if (reason == Player.DISCONTINUITY_REASON_SEEK || reason == Player.DISCONTINUITY_REASON_SEEK_ADJUSTMENT) {
                    ...
                    if (diff > 2000) {
                        Log.d("PlayerActivity", "ðŸŽ¯ Detected user seek: " + (oldPos/1000) + "s â†’ " + (newPos/1000) + "s");
                        isUserSeeking = true;
                        
                        // Clear flag after delay
                        new Handler().postDelayed(() -> {
                            isUserSeeking = false;
                            Log.d("PlayerActivity", "ðŸŽ¯ Cleared seek flag after position discontinuity");
                        }, 2000);
                    }
                }
            }
            ...
            public void onPlayerError(androidx.media3.common.PlaybackException error) {
                Log.e("PlayerActivity", "ðŸŽ¬ ExoPlayer Error: " + error.getMessage());
                progressBar.setVisibility(View.GONE);
                ...
                if (isEOFException) {
                    Log.e("PlayerActivity", "ðŸ“„ EOFException detected - this is normal at end of file, auto-next will proceed");
                    hasPlayerError = false; // Allow auto-next for EOF
                } else {
                    Log.e("PlayerActivity", "ðŸš« Serious error detected - auto-next disabled");
                ...
                Log.e("PlayerActivity", "ðŸ” Error details - Message: " + error.getMessage());
                if (error.getCause() != null) {
                    Log.e("PlayerActivity", "ðŸ” Error cause: " + error.getCause().getClass().getSimpleName());
                    Log.e("PlayerActivity", "ðŸ” Error cause message: " + error.getCause().getMessage());
                }
                ...
                if (error.getCause() != null) {
                    ...
                    if ((causeMessage != null && causeMessage.contains("Multiple renderer media clocks")) ||
                        (errorMessage != null && errorMessage.contains("Multiple renderer media clocks"))) {
                         Log.e("PlayerActivity", "ðŸ”¥ CRITICAL ERROR: Multiple Media Clocks detected! Forcing Software Decoder reset.");
                    ...
                    if ((errorMessage != null && (
                        ...
                        Log.e("PlayerActivity", "ðŸ”Š Audio codec error detected: " + errorMessage);
                    }
                    ...
                    if (causeMessage != null && (
                        ...
                        Log.e("PlayerActivity", "ðŸŽ¬ Video codec error detected: " + causeMessage);
                    }
                    ...
                    if (!useSoftwareAudioDecoder && !isVideoCodecError) {
                        ...
                        if (error.errorCode == androidx.media3.common.PlaybackException.ERROR_CODE_DECODER_INIT_FAILED || 
                            ...
                            errorString.contains("audio/ac3") || errorString.contains("audio/eac3")) {
                             ...
                             Log.e("PlayerActivity", "âš ï¸ Generic decoding error in HW mode (likely AC3/EAC3). Attempting Fallback.");
                             isAudioCodecError = true;
                        }
                    }
                }
                ...
                if (isVideoCodecError) {
                    Log.e("PlayerActivity", "ðŸŽ¬ Handling video codec error - applying video fallback settings");
                    handleVideoCodecError();
                } else if (isAudioCodecError) {
                    if (!hasHandledAudioCodecError) {
                        Log.e("PlayerActivity", "ðŸ”Š Audio codec error detected - trying smart track switching");
                        hasHandledAudioCodecError = true; // Prevent infinite loop
                        handleAudioCodecError();
                    } else {
                        Log.e("PlayerActivity", "ðŸ”Š Audio codec error already handled - showing user dialog");
                        showAudioCodecErrorDialog();
                    }
                } else if (isUnrecognizedFormat) {
                    Log.e("PlayerActivity", "âŒ Unrecognized input format detected - trying HLS fallback");
                    handleUnrecognizedFormat();
                } else if (error.getCause() instanceof java.io.EOFException) {
                    Log.e("PlayerActivity", "ðŸ“„ EOFException detected - triggering auto-next for TV series");
                    ...
                    if (category != null && category.equalsIgnoreCase("tvseries") && canNavigateToNextEpisode()) {
                        Log.e("PlayerActivity", "âœ… Auto-navigating to next episode due to EOFException");
                        navigateToNextEpisode();
                    } else {
                        Log.e("PlayerActivity", "ðŸ“º EOFException but no next episode or not TV series");
                    }
                } else if (error.getCause() instanceof androidx.media3.datasource.HttpDataSource.InvalidResponseCodeException) {
                    ...
                    Log.e("PlayerActivity", "âŒ HTTP Error: " + responseCode);
                    
                    if (responseCode == 416) {
                        // HTTP 416 Range Not Satisfiable - server rejected Range request
                        if (!has416Retried) {
                            has416Retried = true;
                            Log.e("PlayerActivity", "ðŸ”„ HTTP 416: Range Not Satisfiable - retrying with fresh MediaSource");
                            mStartingPosition = -1L;
                            if (player != null && url != null && !url.isEmpty()) {
                                // Create a brand-new MediaSource to avoid stale internal state
                                Uri freshUri = Uri.parse(url);
                                String freshType = detectVideoFormat(url);
                                androidx.media3.exoplayer.source.MediaSource freshSource;
                                if ("hls".equals(freshType) || "m3u8".equals(freshType)) {
                                    freshSource = hlsMediaSource(freshUri, PlayerActivity.this);
                                } else {
                                    freshSource = mediaSource(freshUri, PlayerActivity.this);
                                }
                                mediaSource = freshSource;
                                player.prepare(freshSource, true, true);
                                player.setPlayWhenReady(true);
                            }
                        } else {
                            Log.e("PlayerActivity", "âŒ HTTP 416 persists after retry - showing error");
                            showErrorDialog("Lá»—i phÃ¡t video", "Server tá»« chá»‘i yÃªu cáº§u Range (416). Video cÃ³ thá»ƒ khÃ´ng kháº£ dá»¥ng.");
                        }
                    } else if (responseCode == 403) {
                        showErrorDialog("Lá»—i truy cáº­p", "Server tá»« chá»‘i truy cáº­p (403). Link phÃ¡t cÃ³ thá»ƒ Ä‘Ã£ háº¿t háº¡n.");
                    } else if (responseCode == 404) {
                        showErrorDialog("KhÃ´ng tÃ¬m tháº¥y", "Video khÃ´ng tá»“n táº¡i trÃªn server (404).");
                    } else if (responseCode >= 500) {
                        showErrorDialog("Lá»—i server", "Server gáº·p sá»± cá»‘ (" + responseCode + "). Vui lÃ²ng thá»­ láº¡i sau.");
                    } else {
                        showErrorDialog("Lá»—i HTTP", "Lá»—i HTTP " + responseCode + ". Vui lÃ²ng thá»­ láº¡i.");
                    }
                } else if (error.getCause() != null && error.getCause().getMessage() != null 
                    && error.getCause().getMessage().contains("network")) {
                    Log.e("PlayerActivity", "âŒ Network connection failed");
                    showErrorDialog("Lá»—i káº¿t ná»‘i máº¡ng", "KhÃ´ng thá»ƒ káº¿t ná»‘i Ä‘áº¿n server. Vui lÃ²ng kiá»ƒm tra káº¿t ná»‘i internet.");
                } else {
                    Log.e("PlayerActivity", "âŒ General playback error: " + error.getMessage());
                    showErrorDialog("Lá»—i phÃ¡t video", "KhÃ´ng thá»ƒ phÃ¡t video nÃ y. Vui lÃ²ng thá»­ láº¡i sau.");
                }
            }
        });
        ...
        if (ev.getAction() == MotionEvent.ACTION_DOWN || ev.getAction() == MotionEvent.ACTION_MOVE) {
            ...
            if (seekBarView != null && isTouchOnView(ev, seekBarView)) {
                if (ev.getAction() == MotionEvent.ACTION_DOWN) {
                    isUserSeeking = true;
                    Log.d("PlayerActivity", "ðŸŽ¯ Touch detected on seek bar - user seeking");
                }
            }
            ...
            if (isUserSeeking) {
                ...
                new Handler().postDelayed(() -> {
                    isUserSeeking = false;
                    Log.d("PlayerActivity", "ðŸŽ¯ Touch ended - cleared seeking flag");
                }, 1500);
            }
        }
    ...
    private void addTouchEventMonitoring() {
        ...
        Log.d("PlayerActivity", "ðŸ”§ Touch event monitoring enabled for seek detection");
    }
    ...
    private void setupSeekBarListener() {
        try {
            ...
            View timeBar = exoPlayerView.findViewById(R.id.exo_progress);
            if (timeBar != null && timeBar instanceof androidx.media3.ui.DefaultTimeBar) {
                Log.d("PlayerActivity", "âœ… Found DefaultTimeBar, adding scrub listener");
                ((androidx.media3.ui.DefaultTimeBar) timeBar).addListener(new androidx.media3.ui.TimeBar.OnScrubListener() {
                    ...
                    public void onScrubStart(androidx.media3.ui.TimeBar timeBar, long position) {
                        isUserSeeking = true;
                        Log.d("PlayerActivity", "ðŸŽ¯ User started manual seek at: " + (position / 1000) + "s");
                    }
                    ...
                    public void onScrubStop(androidx.media3.ui.TimeBar timeBar, long position, boolean canceled) {
                        ...
                        Log.d("PlayerActivity", "ðŸŽ¯ User stopped manual seek at: " + (position / 1000) + "s, canceled: " + canceled);
                        new Handler().postDelayed(() -> {
                            isUserSeeking = false;
                            Log.d("PlayerActivity", "ðŸŽ¯ Cleared user seeking flag");
                ...
                timeBar.setOnFocusChangeListener(new View.OnFocusChangeListener() {
                    ...
                    public void onFocusChange(View v, boolean hasFocus) {
                        if (hasFocus) {
                            Log.d("PlayerActivity", "ðŸŽ¯ Time bar focused - DPAD LEFT/RIGHT to seek");
                            // Toast.makeText(PlayerActivity.this, "â—€â–¶ LEFT/RIGHT Ä‘á»ƒ tua", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
            } else {
                Log.w("PlayerActivity", "âš ï¸ Could not find DefaultTimeBar for seek detection");
    ...
    private void tryAlternativeSeekDetection() {
        try {
            ...
            Log.d("PlayerActivity", "ðŸ” Trying alternative seek bar detection");
            
            // This is a fallback - we'll rely more on the player listener
            Log.d("PlayerActivity", "ðŸ”„ Using player listener for seek detection");
        } catch (Exception e) {
    ...
    private void addPlayerSeekListener() {
        ...
        if (player != null) {
            Log.d("PlayerActivity", "âœ… Adding enhanced player seek listener");
    ...
    private void updateTrackButtonVisibility() {
        ...
        if (subtitleButton != null) {
            ...
            Log.d("PlayerActivity", "ðŸ”¤ Subtitle button visibility - hasEmbedded: " + hasText + 
    ...
    private void seekToStartPosition() {
        ...
        if (mStartingPosition > -1L && !isUserSeeking) {
            if (player.getPlayWhenReady()) {
                Log.d("PlayerActivity", "ðŸ“ Seeking to start position: " + (mStartingPosition / 1000) + "s");
    ...
    private void seekForward(long seekTimeMs) {
        if (player != null && player.isCurrentWindowSeekable()) {
            ...
            if (duration != C.TIME_UNSET) {
                ...
                // Show seek feedback
                // Toast.makeText(this, "Tua tá»›i +" + (seekTimeMs / 1000) + "s", Toast.LENGTH_SHORT).show();
                Log.i("PlayerActivity", "ðŸ”„ Seeking forward to: " + (newPosition / 1000) + "s");
            }
        } else if (player != null && !player.isCurrentWindowSeekable()) {
            Toast.makeText(this, "KhÃ´ng thá»ƒ tua trong ná»™i dung trá»±c tiáº¿p", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void seekBackward(long seekTimeMs) {
        if (player != null && player.isCurrentWindowSeekable()) {
            ...
            // Show seek feedback
            // Toast.makeText(this, "Tua lÃ¹i -" + (seekTimeMs / 1000) + "s", Toast.LENGTH_SHORT).show();
            Log.i("PlayerActivity", "ðŸ”„ Seeking backward to: " + (newPosition / 1000) + "s");
        } else if (player != null && !player.isCurrentWindowSeekable()) {
            Toast.makeText(this, "KhÃ´ng thá»ƒ tua trong ná»™i dung trá»±c tiáº¿p", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private androidx.media3.exoplayer.source.MediaSource mp3MediaSource(Uri uri) {
    ...
    private androidx.media3.exoplayer.source.MediaSource mediaSource(Uri uri, Context context) {
    ...
    private androidx.media3.exoplayer.source.MediaSource rtmpMediaSource(Uri uri) {
        androidx.media3.exoplayer.source.MediaSource videoSource = null;

        // RtmpDataSourceFactory is exoplayer2-based; wrap with adapter for Media3 compatibility
        final RtmpDataSourceFactory rtmpFactory = new RtmpDataSourceFactory();
        androidx.media3.datasource.DataSource.Factory dataSourceFactory = () -> new androidx.media3.datasource.DataSource() {
            private final com.google.android.exoplayer2.upstream.DataSource inner = rtmpFactory.createDataSource();
            @Override public void addTransferListener(androidx.media3.datasource.TransferListener transferListener) {}
            @Override public long open(androidx.media3.datasource.DataSpec dataSpec) throws androidx.media3.datasource.HttpDataSource.HttpDataSourceException {
                try {
                    com.google.android.exoplayer2.upstream.DataSpec exoSpec = new com.google.android.exoplayer2.upstream.DataSpec(dataSpec.uri);
                    return inner.open(exoSpec);
                } catch (Exception e) { throw new RuntimeException(e); }
            }
            @Override public int read(byte[] buffer, int offset, int length) throws androidx.media3.datasource.HttpDataSource.HttpDataSourceException {
                try { return inner.read(buffer, offset, length); } catch (Exception e) { throw new RuntimeException(e); }
            }
            @Override public android.net.Uri getUri() { return inner.getUri(); }
            @Override public java.util.Map<String, java.util.List<String>> getResponseHeaders() { return java.util.Collections.emptyMap(); }
            @Override public void close() throws androidx.media3.datasource.HttpDataSource.HttpDataSourceException {
                try { inner.close(); } catch (Exception e) { throw new RuntimeException(e); }
            }
        };
    ExtractorsFactory extractorsFactory = new DefaultExtractorsFactory();
   ...
   /* @SuppressLint("StaticFieldLeak")
        ...
        new YouTubeExtractor(context) {
            ...
            public void onExtractionComplete(SparseArray<YtFile> ytFiles, VideoMeta vMeta) {
                if (ytFiles != null) {
                    ...
                    try {
                        androidx.media3.exoplayer.source.MediaSource source = mediaSource(Uri.parse(dashUrl), context);
    ...
    private androidx.media3.exoplayer.source.MediaSource hlsMediaSource(Uri uri, Context context) {

        DefaultBandwidthMeter bandwidthMeter = new DefaultBandwidthMeter.Builder(context).build();
        DataSource.Factory dataSourceFactory = new DefaultDataSourceFactory(context,
                Util.getUserAgent(context, "oxoo"), bandwidthMeter);
        androidx.media3.exoplayer.source.MediaSource videoSource = new HlsMediaSource.Factory(dataSourceFactory)
                .createMediaSource(MediaItem.fromUri(uri));
    ...
    public void onBackPressed() {
        Log.d(TAG, "ðŸ”™ onBackPressed() called");
        
        // Check if controls are currently showing
        boolean controlsVisible = exoPlayerView.isControllerFullyVisible();
        Log.d(TAG, "ðŸ”™ Controls visible: " + controlsVisible);
        ...
        if (controlsVisible) {
            ...
            // If so, go to video details instead of going back to home
            boolean fromWatchHistory = getIntent().getBooleanExtra("from_watch_history", false);
            Log.d(TAG, "ðŸ”™ From watch history: " + fromWatchHistory);
            ...
            if (fromWatchHistory && model != null) {
                ...
                Log.d(TAG, "ðŸ”™ Normal back press - going to previous activity");
                super.onBackPressed();
            }
        }
    }
    ...
    private void saveWatchHistoryWithData(long currentPosition, long duration, boolean forceSave) {
        if (model == null || watchHistorySyncManager == null) {
            Log.e(TAG, "âŒ saveWatchHistoryWithData: model or manager is null");
            return;
        }
        ...
        if (checkId != null && (checkId.startsWith("phim4k_") || checkId.startsWith("kkphim4k_"))) {
            Log.d(TAG, "â­ï¸ Skipping watch history for free source: " + checkId);
            return;
        }
        ...
        if (validDuration && validPosition && watchHistorySyncManager != null) {
            try {
                ...
                if (completeMovieData != null && (hasCompleteInfo || !forceSave)) {
                    ...
                    if (!hasCompleteInfo) {
                        Log.w(TAG, "âš ï¸ CompleteMovieData exists but lacks essential info, will try API fetch for: " + completeMovieData.getVideosId());
                        ...
                        if (completeMovieData.getVideosId() != null && !completeMovieData.getVideosId().equals("null")) {
                            ...
                            Log.d(TAG, "ðŸ• Waiting for API data, will retry save later");
                            return; // Don't save yet, wait for API
                        }
                    }
                    
                    Log.e(TAG, "ðŸ’¾ Saving ENHANCED watch history with complete metadata - ID: " + completeMovieData.getVideosId() + 
                              ", Title: " + completeMovieData.getTitle() + 
                              ", Position: " + currentPosition + "ms");
                    
                    // Debug: Log complete movie data structure
                    Log.d(TAG, "ðŸ” CompleteMovieData debug:");
                    Log.d(TAG, "  - VideosId: '" + completeMovieData.getVideosId() + "'");
                    Log.d(TAG, "  - Title: '" + completeMovieData.getTitle() + "'");
                    Log.d(TAG, "  - Slug: '" + completeMovieData.getSlug() + "'");
                    Log.d(TAG, "  - Type: '" + completeMovieData.getType() + "'");
                    ...
                    if (videoId == null || videoId.isEmpty() || videoId.equals("null")) {
                        ...
                        if (model != null && model.getMovieId() != null && !model.getMovieId().equals("null") && !model.getMovieId().isEmpty()) {
                            ...
                            Log.w(TAG, "âš ï¸ CompleteMovieData.getVideosId() is invalid, using model.getMovieId(): " + videoId);
                        } else if (getIntent() != null && getIntent().getStringExtra("id") != null) {
                            videoId = getIntent().getStringExtra("id");
                            Log.w(TAG, "âš ï¸ CompleteMovieData.getVideosId() is invalid, using intent id: " + videoId);
                        } else {
                            videoId = String.valueOf(System.currentTimeMillis()); // Fallback to timestamp
                            Log.e(TAG, "âŒ All videoId sources invalid, using timestamp: " + videoId);
                        }
                    }
                    ...
                    Log.d(TAG, "ðŸ” Using title from intent: '" + getIntent().getStringExtra("title") + "'");
                    Log.d(TAG, "ðŸ” Final title for save: '" + title + "'");
                    ...
                    Log.e(TAG, "âš ï¸ Using saveFullWatchHistory with PlaybackModel data (no completeMovieData available)");
                    ...
                    if (saveId == null || saveId.equals("null") || saveId.isEmpty()) {
                        ...
                        Log.w(TAG, "âš ï¸ MovieId is invalid, using episode/video ID: " + saveId);
                    }
                    ...
                    Log.e(TAG, "ðŸ’¾ Saving ENHANCED watch history with PlaybackModel data - ID: " + saveId + 
    ...
    private void loadEpisode(int seasonIndex, int episodeIndex) {
        ...
        // IMPORTANT: Use episode's fileType for correct MediaSource selection (hls, m3u8, etc.)
        // Previously hardcoded "tvseries" which bypassed format auto-detection
        String episodeFileType = episode.getFileType();
        if (episodeFileType != null && !episodeFileType.isEmpty()) {
            newModel.setVideoType(episodeFileType);
        }
        // Keep category as "tvseries" for watch history and navigation logic
        ...
        // Reset starting position so new episode starts from beginning
        mStartingPosition = -1L;
        
        // Restart the activity with the new episode
        Intent intent = new Intent(this, PlayerActivity.class);
        intent.putExtra(VideoPlaybackActivity.EXTRA_VIDEO, newModel);
        // Explicitly set position to -1 to prevent resume from previous episode
        intent.putExtra(VideoPlaybackActivity.EXTRA_POSITION, -1L);
    ...
    private void showSubtitleSelectionDialog() {
        ...
        if (totalTracks == 0) {
            Toast.makeText(this, "KhÃ´ng tÃ¬m tháº¥y phá»¥ Ä‘á»", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        for (int groupIndex = 0; groupIndex < trackGroups.length; groupIndex++) {
            ...
            for (int trackIndex = 0; trackIndex < group.length; trackIndex++) {
                ...
                else label = "Phá»¥ Ä‘á» " + (labels.size());
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("Chá»n phá»¥ Ä‘á» (" + totalTracks + " cÃ³ sáºµn)");
        builder.setItems(labels.toArray(new String[0]), (dialog, which) -> {
            ...
            try {
                ...
                Toast.makeText(this, "Lá»—i khi Ä‘á»•i phá»¥ Ä‘á»", Toast.LENGTH_SHORT).show();
            }
        });
        builder.show();
    }
    ...
    private void showAudioTrackSelectionDialog() {
        ...
        Log.d("PlayerActivity", "ðŸ”Š Populating Audio Selection Dialog...");
        for (int i = 0; i < mappedTrackInfo.getRendererCount(); i++) {
        ...
        if (labels.size() <= 1) {
            Toast.makeText(this, "Chá»‰ cÃ³ má»™t luá»“ng Ã¢m thanh", Toast.LENGTH_SHORT).show();
            return;
        }

        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("Chá»n Ã¢m thanh (" + labels.size() + " cÃ³ sáºµn)");
        builder.setItems(labels.toArray(new String[0]), (dialog, which) -> {
            int[] pair = selectionPairs.get(which);
            ...
            try {
                ...
                // Show toast
                Toast.makeText(this, "ÄÃ£ chá»n: " + labels.get(which), Toast.LENGTH_SHORT).show();
                
            } catch (Exception e) {
                Log.e(TAG, "Error changing audio track", e);
                Toast.makeText(this, "Lá»—i khi Ä‘á»•i Ã¢m thanh", Toast.LENGTH_SHORT).show();
            }
        });
        builder.show();
    }
    ...
    */
    ...
    private void openUnifiedSettingsDialog() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("CÃ i Ä‘áº·t phá»¥ Ä‘á»");
        ...
        fontTypeLabel.setText("Kiá»ƒu chá»¯:");
        ...
        fontTypePrevBtn.setText("â—€");
        Button fontTypeNextBtn = new Button(this);
        fontTypeNextBtn.setText("â–¶");
        TextView fontTypeTV = new TextView(this);
        
        String[] fontNames = {"Máº·c Ä‘á»‹nh", "Sans Serif", "Serif", "Monospace", "Tiáº¿ng Viá»‡t"};
        ...
        positionUpBtn.setText("LÃªn");
        Button positionDownBtn = new Button(this);
        positionDownBtn.setText("Xuá»‘ng");
        ...
        positionTV.setText("Dá»‹ch chuyá»ƒn (" + (currentOffset > 0 ? "+" : "") + currentOffset + "%)");
        ...
        textColorLabel.setText("MÃ u chá»¯:");
        ...
        textColorPrevBtn.setText("â—€");
        Button textColorNextBtn = new Button(this);
        textColorNextBtn.setText("â–¶");
        TextView textColorTV = new TextView(this);
        
        String[] colorNames = {"Tráº¯ng", "VÃ ng", "Äá»", "Xanh lÃ¡", "Xanh dÆ°Æ¡ng", "Cam", "Há»“ng", "Xanh lÆ¡"};
        int[] colorValues = {Color.WHITE, Color.YELLOW, Color.RED, Color.GREEN, 
                            Color.BLUE, 0xFFFF8C00, 0xFFFFC0CB, Color.CYAN}; // Orange, Pink
        ...
        outlineColorLabel.setText("MÃ u viá»n:");
        ...
        outlineColorPrevBtn.setText("â—€");
        Button outlineColorNextBtn = new Button(this);
        outlineColorNextBtn.setText("â–¶");
        TextView outlineColorTV = new TextView(this);
        
        String[] outlineColorNames = {"Trong suá»‘t", "Äen", "Tráº¯ng", "Äá»", "Xanh dÆ°Æ¡ng", "VÃ ng"};
        int[] outlineColorValues = {Color.TRANSPARENT, Color.BLACK, Color.WHITE, Color.RED, Color.BLUE, Color.YELLOW};
        ...
        playbackSpeedLabel.setText("Tá»‘c Ä‘á»™ phÃ¡t:");
        ...
        resetBtn.setText("KhÃ´i phá»¥c máº·c Ä‘á»‹nh");
        ...
        positionUpBtn.setOnClickListener(v -> {
            ...
            String newPositionText = offset == 0 ? "Giá»¯a" : "LÃªn +" + offset + "%";
        ...
        positionDownBtn.setOnClickListener(v -> {
            ...
            String newPositionText = offset == 0 ? "Giá»¯a" : "Xuá»‘ng " + offset + "%";
            positionTV.setText(newPositionText);
            saveAndApplySubtitleSetting("vertical_offset", offset);
        });
        ...
        resetBtn.setOnClickListener(v -> {
            ...
            Toast.makeText(PlayerActivity.this, "ÄÃ£ khÃ´i phá»¥c cÃ i Ä‘áº·t gá»‘c", Toast.LENGTH_SHORT).show();
            applySubtitleSettings();
        });
        ...
        builder.setPositiveButton("ÄÃ³ng", (dialog, which) -> dialog.dismiss());
        builder.show();
    }
    ...
    private void showAspectRatioDialog() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("Chá»n tá»· lá»‡ mÃ n hÃ¬nh");
        builder.setItems(aspectRatioOptions, (dialog, which) -> {
            if (which == 0) {
                // Tá»· lá»‡ gá»‘c - reset vá» aspect ratio máº·c Ä‘á»‹nh cá»§a video
                exoPlayerView.setResizeMode(AspectRatioFrameLayout.RESIZE_MODE_FIT);
            ...
            new ToastMsg(PlayerActivity.this).toastIconSuccess("ÄÃ£ chuyá»ƒn: " + aspectRatioOptions[which]);
        });
        builder.show();
    }
    ...
    private void loadSubtitleDataAndShowDialog() {
        ...
        Log.d(TAG, "ðŸŽ¬ Loading subtitle data for ID: " + videoId + ", Type: " + videoType);
        
        // Show loading indicator
        runOnUiThread(() -> {
            new ToastMsg(this).toastIconSuccess("Äang táº£i phá»¥ Ä‘á»...");
        });
                ...
                call.enqueue(new Callback<MovieSingleDetails>() {
                    ...
                    public void onResponse(Call<MovieSingleDetails> call, Response<MovieSingleDetails> response) {
                        if (response.isSuccessful() && response.body() != null) {
                                    ...
                                    if (season.getEpisodes() != null && !season.getEpisodes().isEmpty()) {
                                        for (Episode episode : season.getEpisodes()) {
                                            if (episode.getSubtitle() != null && !episode.getSubtitle().isEmpty()) {
                                                ...
                                                Log.d(TAG, "âœ… Found subtitles in episode " + episode.getEpisodesId() + ": " + episode.getSubtitle().size() + " tracks");
                            ...
                            if (movieDetails.getVideos() != null && !movieDetails.getVideos().isEmpty()) {
                                for (Video videoItem : movieDetails.getVideos()) {
                                    if (videoItem.getSubtitle() != null && !videoItem.getSubtitle().isEmpty()) {
                                        ...
                                        Log.d(TAG, "âœ… Found subtitles in video: " + videoItem.getSubtitle().size() + " tracks");
                            ...
                            // No subtitles found
                            Log.d(TAG, "âŒ No subtitles found in API response");
                            runOnUiThread(() -> {
                                showNoSubtitleMessage();
                            });
                        } else {
    ...
    private void showNoSubtitleMessage() {
        ...
        if (hasEmbeddedSub) {
            ...
            new ToastMsg(this).toastIconError("KhÃ´ng tÃ¬m tháº¥y phá»¥ Ä‘á». Báº¡n cÃ³ thá»ƒ tÃ¹y chá»‰nh subtitle trong Settings.");
        }
    }
    ...
    private void handleUnrecognizedFormat() {
        Log.d("PlayerActivity", "ðŸ”„ Attempting to handle unrecognized format for URL: " + url);
            ...
            public void run() {
                try {
                    ...
                    if (!hasRetriedWithHLS) {
                        Log.d("PlayerActivity", "ðŸŽ¯ Trying HLS MediaSource as fallback...");
                        retryWithHLS();
                    } else {
                        Log.d("PlayerActivity", "âŒ Already retried with HLS, showing error dialog");
                    ...
                    //     Log.d("PlayerActivity", "ðŸ” Re-detected format as: " + detectedFormat);
                    //     
                    //     if ("hls".equals(detectedFormat)) {
                    //         Log.d("PlayerActivity", "ðŸŽ¯ Retrying with HLS MediaSource...");
                    //         retryWithHLS();
                    //         return;
                    //     }
                    // }
    ...
    private void retryWithHLS() {
        try {
            ...
            Uri uri = Uri.parse(url);
            androidx.media3.exoplayer.source.MediaSource hlsSource = hlsMediaSource(uri, this);
            ...
            Log.d("PlayerActivity", "âœ… Retrying playback with HLS MediaSource for URL: " + url);
    ...
    private void showFormatErrorDialog() {
        new android.app.AlertDialog.Builder(this)
            .setTitle("âš ï¸ Äá»‹nh dáº¡ng khÃ´ng há»— trá»£")
            .setMessage("Video nÃ y cÃ³ Ä‘á»‹nh dáº¡ng khÃ´ng Ä‘Æ°á»£c há»— trá»£. Báº¡n cÃ³ muá»‘n thá»­ cháº¥t lÆ°á»£ng khÃ¡c khÃ´ng?")
            .setPositiveButton("Thá»­ láº¡i", (dialog, which) -> {
                // Try to reload with different settings
                recreatePlayer();
            })
            .setNegativeButton("Quay láº¡i", (dialog, which) -> {
                finish();
            })
            .setCancelable(false)
            .show();
    ...
    // ==========================================
    // MX PLAYER EXTERNAL FALLBACK
    // ==========================================
    
    private static final String MX_PLAYER_PACKAGE_FREE = "com.mxtech.videoplayer.ad";
    private static final String MX_PLAYER_PACKAGE_PRO = "com.mxtech.videoplayer.pro";
    private static final String MX_PLAYER_DOWNLOAD_URL = "https://dl.dropboxusercontent.com/scl/fi/s62l7sot0aymi7jh4xz9h/mx32.apk.apk?rlkey=56q47m7yf2n0fsqpu8jftl675&st=dgsagi78&dl=0";
    
    /**
     * Check if MX Player (free or pro) is installed
     */
    private String getInstalledMXPlayerPackage() {
        android.content.pm.PackageManager pm = getPackageManager();
        try {
            pm.getPackageInfo(MX_PLAYER_PACKAGE_PRO, 0);
            return MX_PLAYER_PACKAGE_PRO;
        } catch (android.content.pm.PackageManager.NameNotFoundException e) { /* not installed */ }
        try {
            pm.getPackageInfo(MX_PLAYER_PACKAGE_FREE, 0);
            return MX_PLAYER_PACKAGE_FREE;
        } catch (android.content.pm.PackageManager.NameNotFoundException e) { /* not installed */ }
        return null;
    }
    
    /**
     * Launch MX Player with HW+ decode mode for unsupported codecs
     */
    private void launchMXPlayerHWPlus(String videoUrl) {
        String mxPackage = getInstalledMXPlayerPackage();
        if (mxPackage == null) {
            Log.e(TAG, "âŒ MX Player not installed");
            return;
        }
        
        try {
            // MX Player API: Use specific Activity names for decode modes
            // HW  = com.mxtech.videoplayer.ActivityScreen (default)
            // HW+ = com.mxtech.videoplayer.ActivityScreen$HW_Plus  (not available via intent)
            // SW  = com.mxtech.videoplayer.ActivityScreen$SW
            //
            // Official MX Player Intent API:
            // api_MX_A = HW decoder
            // api_MX_B = HW+ decoder (FFmpeg + GPU)  
            // api_MX_C = SW decoder (FFmpeg + CPU)
            
            Intent intent = new Intent(Intent.ACTION_VIEW);
            intent.setDataAndType(Uri.parse(videoUrl), "video/*");
            intent.setPackage(mxPackage);
            
            // Force HW+ decode mode via component name
            // This directly launches the HW+ Activity, no user setting needed
            intent.setComponent(new android.content.ComponentName(mxPackage, 
                mxPackage + ".ActivityScreen"));
            
            // MX Player extras for decode mode
            intent.putExtra("decode_mode", 1);     // 0=HW, 1=HW+, 2=SW (MX Player API v1)
            
            // Also try the API v2 approach  
            intent.putExtra("video.decoder", 1);    // 0=auto, 1=HW+, 2=SW
            
            // Pass video title if available
            if (model != null && model.getTitle() != null) {
                intent.putExtra("title", model.getTitle());
            }
            
            // Pass current position for resume (in milliseconds)
            if (player != null && player.getCurrentPosition() > 0) {
                intent.putExtra("position", (int) player.getCurrentPosition());
            }
            
            // Return result when MX Player finishes
            intent.putExtra("return_result", true);
            
            // Headers (if needed for authenticated URLs)
            // String[] headers = {"User-Agent", "exoplayer"};
            // intent.putExtra("headers", headers);
            
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            Log.d(TAG, "ðŸŽ¬ Launching MX Player (HW+) for: " + videoUrl + " [package=" + mxPackage + "]");
            startActivity(intent);
        } catch (android.content.ActivityNotFoundException e) {
            Log.e(TAG, "âŒ MX Player Activity not found, trying generic intent", e);
            // Fallback: generic ACTION_VIEW without specifying component
            try {
                Intent fallback = new Intent(Intent.ACTION_VIEW);
                fallback.setDataAndType(Uri.parse(videoUrl), "video/*");
                fallback.setPackage(mxPackage);
                fallback.putExtra("decode_mode", 1);
                fallback.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                startActivity(fallback);
            } catch (Exception e2) {
                Log.e(TAG, "âŒ Failed to launch MX Player (fallback)", e2);
                android.widget.Toast.makeText(this, "KhÃ´ng thá»ƒ má»Ÿ MX Player", android.widget.Toast.LENGTH_SHORT).show();
            }
        } catch (Exception e) {
            Log.e(TAG, "âŒ Failed to launch MX Player", e);
            android.widget.Toast.makeText(this, "KhÃ´ng thá»ƒ má»Ÿ MX Player", android.widget.Toast.LENGTH_SHORT).show();
        }
    }
    
    /**
     * Download MX Player APK from Dropbox
     */
    private void downloadMXPlayer() {
        try {
            Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(MX_PLAYER_DOWNLOAD_URL));
            browserIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            startActivity(browserIntent);
            android.widget.Toast.makeText(this, "Äang táº£i MX Player... Sau khi cÃ i xong, má»Ÿ láº¡i video nÃ y.", android.widget.Toast.LENGTH_LONG).show();
        } catch (Exception e) {
            Log.e(TAG, "âŒ Failed to open download URL", e);
            android.widget.Toast.makeText(this, "KhÃ´ng thá»ƒ má»Ÿ link táº£i", android.widget.Toast.LENGTH_SHORT).show();
        }
    }
    
    /**
     * Show dialog when video codec is not supported by device hardware
     * Offers to open with MX Player (HW+ mode) or download MX Player if not installed
     */
    private void showUnsupportedCodecDialog(String codecInfo) {
        String mxPackage = getInstalledMXPlayerPackage();
        boolean hasMXPlayer = mxPackage != null;
        
        android.app.AlertDialog.Builder builder = new android.app.AlertDialog.Builder(PlayerActivity.this);
        builder.setTitle("âš ï¸ Codec video khÃ´ng há»— trá»£");
        
        if (hasMXPlayer) {
            builder.setMessage("Codec \"" + codecInfo + "\" khÃ´ng Ä‘Æ°á»£c thiáº¿t bá»‹ há»— trá»£.\n\n" +
                "Video chá»‰ cÃ³ Ã¢m thanh, khÃ´ng cÃ³ hÃ¬nh áº£nh.\n\n" +
                "Báº¡n cÃ³ thá»ƒ má»Ÿ báº±ng MX Player (cháº¿ Ä‘á»™ HW+) Ä‘á»ƒ phÃ¡t mÆ°á»£t.");
            builder.setPositiveButton("Má»Ÿ MX Player", (dialog, which) -> {
                launchMXPlayerHWPlus(url);
            });
            builder.setNegativeButton("Tiáº¿p tá»¥c nghe", (dialog, which) -> {
                dialog.dismiss();
            });
        } else {
            builder.setMessage("Codec \"" + codecInfo + "\" khÃ´ng Ä‘Æ°á»£c thiáº¿t bá»‹ há»— trá»£.\n\n" +
                "Video chá»‰ cÃ³ Ã¢m thanh, khÃ´ng cÃ³ hÃ¬nh áº£nh.\n\n" +
                "CÃ i MX Player Ä‘á»ƒ phÃ¡t cÃ¡c video cÃ³ codec Ä‘áº·c biá»‡t.");
            builder.setPositiveButton("Táº£i MX Player", (dialog, which) -> {
                downloadMXPlayer();
            });
            builder.setNegativeButton("Tiáº¿p tá»¥c nghe", (dialog, which) -> {
                dialog.dismiss();
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                new android.app.AlertDialog.Builder(PlayerActivity.this)
                    .setTitle(title)
                    .setMessage(message)
                    .setPositiveButton("ÄÃ³ng", (dialog, which) -> {
                        finish();
                    })
                    .setCancelable(false)
                    .show();
    ...
    private void recreatePlayer() {
        try {
            ...
            if (url != null && !url.isEmpty()) {
                Log.d("PlayerActivity", "ðŸ”„ Restarting playback with URL: " + url);
                // We can't easily recreate the entire player setup here
                // Instead, just restart the current activity or show options
            }
            
            Log.d("PlayerActivity", "ðŸ”„ Player recreated for fallback playback");
        } catch (Exception e) {
            Log.e("PlayerActivity", "Error recreating player", e);
            showErrorDialog("Lá»—i khá»Ÿi táº¡o player", "KhÃ´ng thá»ƒ khá»Ÿi táº¡o láº¡i player.");
        }
    }
    ...
    private void handleAudioCodecError() {
        Log.d("PlayerActivity", "ðŸ”Š Handling audio codec error");
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    if (!useSoftwareAudioDecoder) {
                        Log.d("PlayerActivity", "ðŸ”Š Attempting to switch to Software Audio Decoder (FFmpeg)");
                        ...
                        savedAudioOverride = null;
                        savedAudioRendererIndex = -1;
                        Log.d("PlayerActivity", "ðŸ§¹ Cleared saved audio selection to allow auto-selection for Software Decoder");
                        ...
                        new ToastMsg(PlayerActivity.this).toastIconSuccess("Äang chuyá»ƒn sang cháº¿ Ä‘á»™ giáº£i mÃ£ Ã¢m thanh báº±ng pháº§n má»m...");
                    ...
                    Log.d("PlayerActivity", "ðŸ”Š Software decoding failed, trying to find compatible audio track");
                    ...
                    if (foundCompatibleTrack) {
                        Log.d("PlayerActivity", "âœ… Found and selected compatible audio track");
                        showAudioTrackSwitchedNotification();
                    } else {
                        Log.d("PlayerActivity", "âŒ No compatible audio track found, disabling audio");
    ...
    private void showAudioDisabledNotification() {
        new android.app.AlertDialog.Builder(this)
            .setTitle("ðŸ”‡ Ã‚m thanh Ä‘Ã£ bá»‹ táº¯t")
            .setMessage("Äá»‹nh dáº¡ng Ã¢m thanh EAC3 khÃ´ng Ä‘Æ°á»£c há»— trá»£ trÃªn thiáº¿t bá»‹ nÃ y. Video sáº½ phÃ¡t khÃ´ng cÃ³ tiáº¿ng.\n\nBáº¡n cÃ³ muá»‘n tiáº¿p tá»¥c xem khÃ´ng?")
            .setPositiveButton("Xem khÃ´ng tiáº¿ng", (dialog, which) -> {
    ...
    private boolean trySelectCompatibleAudioTrack(MappingTrackSelector.MappedTrackInfo mappedTrackInfo) {
                ...
                for (int groupIndex = 0; groupIndex < trackGroups.length; groupIndex++) {
                    ...
                    for (int trackIndex = 0; trackIndex < group.length; trackIndex++) {
                        ...
                        if (mimeType != null && (
                            ...
                            Log.d("PlayerActivity", "ðŸ”Š Found compatible audio track: " + mimeType + " (language: " + format.language + ")");
                            ...
                            Log.d("PlayerActivity", "ðŸ”Š Skipping incompatible audio track: " + mimeType);
                        }
                    }
                }
            }
        ...
        Log.d("PlayerActivity", "ðŸ”Š No compatible audio tracks found");
        return false;
    }
    ...
    private void disableAudioRenderer(MappingTrackSelector.MappedTrackInfo mappedTrackInfo) {
        for (int rendererIndex = 0; rendererIndex < mappedTrackInfo.getRendererCount(); rendererIndex++) {
            if (mappedTrackInfo.getRendererType(rendererIndex) == C.TRACK_TYPE_AUDIO) {
                Log.d("PlayerActivity", "ðŸ”Š Disabling audio renderer " + rendererIndex + " due to no compatible tracks");
                ...
                Log.d("PlayerActivity", "âœ… Audio renderer disabled - video will play without audio");
                return;
            }
        }
    }
    ...
    private void showAudioTrackSwitchedNotification() {
        new android.app.AlertDialog.Builder(this)
            .setTitle("ðŸ”„ ÄÃ£ chuyá»ƒn Ä‘á»•i Ã¢m thanh")
            .setMessage("ÄÃ£ tá»± Ä‘á»™ng chuyá»ƒn tá»« EAC3 sang Ä‘á»‹nh dáº¡ng Ã¢m thanh tÆ°Æ¡ng thÃ­ch khÃ¡c. Video sáº½ phÃ¡t vá»›i Ã¢m thanh má»›i.")
    ...
    private void showAudioCodecErrorDialog() {
        new android.app.AlertDialog.Builder(this)
            .setTitle("âš ï¸ Lá»—i Ã¢m thanh")
            .setMessage("Äá»‹nh dáº¡ng Ã¢m thanh khÃ´ng Ä‘Æ°á»£c há»— trá»£ trÃªn thiáº¿t bá»‹ nÃ y. Video sáº½ phÃ¡t khÃ´ng cÃ³ tiáº¿ng hoáº·c vá»›i Ã¢m thanh thay tháº¿.\n\nBáº¡n cÃ³ muá»‘n tiáº¿p tá»¥c khÃ´ng?")
            .setPositiveButton("Tiáº¿p tá»¥c", (dialog, which) -> {
    ...
    private void handleVideoCodecError() {
        Log.d("PlayerActivity", "ðŸŽ¬ Handling video codec error - configuring video renderer for compatibility");
            ...
            public void run() {
                try {
                    if (trackSelector == null || player == null) {
                        ...
                        showErrorDialog("Lá»—i video", "KhÃ´ng thá»ƒ khá»Ÿi táº¡o video renderer. Thá»­ chá»n cháº¥t lÆ°á»£ng khÃ¡c.");
                        return;
                    }
                    ...
                    Log.d("PlayerActivity", "âœ… Applied video codec error handling - track selector configured");
                    
                    // Try to restart playback with new video settings
                    if (player != null && url != null) {
                        Log.d("PlayerActivity", "ðŸ”„ Restarting playback with video codec fallback settings");
                        ...
                        Uri uri = Uri.parse(url);
                        androidx.media3.exoplayer.source.MediaSource mediaSource = createMediaSourceForUrl(uri, videoType);
                        ...
                        if (mediaSource != null) {
                            ...
                            Log.d("PlayerActivity", "âœ… Playback restarted with video fallback at position: " + currentPosition);
                        } else {
    ...
    private void showVideoCodecErrorDialog() {
        new android.app.AlertDialog.Builder(this)
            .setTitle("âš ï¸ Lá»—i video codec")
            .setMessage("Äá»‹nh dáº¡ng video khÃ´ng Ä‘Æ°á»£c há»— trá»£ Ä‘áº§y Ä‘á»§ trÃªn thiáº¿t bá»‹ nÃ y. Video cÃ³ thá»ƒ phÃ¡t cháº­m hoáº·c cÃ³ lá»—i hiá»ƒn thá»‹.\n\nBáº¡n cÃ³ muá»‘n thá»­ cháº¥t lÆ°á»£ng tháº¥p hÆ¡n khÃ´ng?")
            .setPositiveButton("Thá»­ cháº¥t lÆ°á»£ng tháº¥p", (dialog, which) -> {
     ...
     */
    private androidx.media3.exoplayer.source.MediaSource createMediaSourceForUrl(Uri uri, String type) {
        try {
    ...
    private String detectVideoFormat(String url) {
        ...
        if (lowerUrl.contains(".m3u8") || lowerUrl.contains("/hls/") || 
            ...
            Log.d("PlayerActivity", "ðŸŽ¯ Detected HLS format from URL patterns");
            return "hls";
        }
        
        // RTMP detection
        if (lowerUrl.startsWith("rtmp://") || lowerUrl.startsWith("rtmps://")) {
            Log.d("PlayerActivity", "ðŸŽ¯ Detected RTMP format from protocol");
            return "rtmp";
        }
        
        // DASH detection
        if (lowerUrl.contains(".mpd") || lowerUrl.contains("/dash/")) {
            Log.d("PlayerActivity", "ðŸŽ¯ Detected DASH format from URL patterns");
            return "dash";
        }
        
        // YouTube detection
        if (lowerUrl.contains("youtube.com") || lowerUrl.contains("youtu.be")) {
            Log.d("PlayerActivity", "ðŸŽ¯ Detected YouTube format from domain");
            return "youtube";
        }
        ...
        if (lowerUrl.contains(".mp4") || lowerUrl.contains(".mkv") || 
            ...
            Log.d("PlayerActivity", "ðŸŽ¯ Detected progressive format from extension");
            return "video";
        }
        
        Log.d("PlayerActivity", "ðŸ¤” Could not detect format, using default progressive");
    ...
    private void showNextEpisodeDialog() {
        ...
        builder.setTitle("ðŸŽ¬ Táº­p Ä‘Ã£ káº¿t thÃºc");
        
        if (nextEpisodeTitle != null && !nextEpisodeTitle.isEmpty()) {
            builder.setMessage("Báº¡n cÃ³ muá»‘n tiáº¿p tá»¥c xem táº­p tiáº¿p theo?\n\nðŸ“º " + nextEpisodeTitle);
        } else {
            builder.setMessage("Báº¡n cÃ³ muá»‘n tiáº¿p tá»¥c xem táº­p tiáº¿p theo khÃ´ng?");
        }
        
        builder.setPositiveButton("â–¶ï¸ Xem ngay", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                Log.d(TAG, "âœ… User chose to watch next episode");
                dialog.dismiss();
                navigateToNextEpisode();
            }
        });
        
        builder.setNegativeButton("âŒ ThoÃ¡t", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                Log.d(TAG, "âŒ User chose not to watch next episode");
                dialog.dismiss();
        ...
        builder.setNeutralButton("ðŸ“‹ Chá»n táº­p khÃ¡c", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                Log.d(TAG, "ðŸ“‹ User chose to select different episode");
                dialog.dismiss();
        ...
        builder.setOnCancelListener(new DialogInterface.OnCancelListener() {
            ...
            public void onCancel(DialogInterface dialog) {
                Log.d(TAG, "ðŸš« Next episode dialog cancelled");
                finish();
            }
        });
        ...
        try {
            ...
            dialog.show();
            Log.d(TAG, "ðŸ“º Next episode dialog shown");
            ...
            new Handler().postDelayed(new Runnable() {
                ...
                public void run() {
                    if (dialog.isShowing()) {
                        Log.d(TAG, "â° Auto-dismissing next episode dialog after 30s");
                        dialog.dismiss();
                        finish();
                    }
                }
            ...
            Log.e(TAG, "âŒ Error showing next episode dialog: " + e.getMessage());
    ...
    private void loadExternalSubtitle(String subtitleUrl, String language) {
        ...
        if (subtitleUrl != null && subtitleUrl.contains("xxxx.qphim.xyz")) {
            ...
            // Log.d(TAG, "ðŸŽ¬ Fixed subtitle URL hostname: " + subtitleUrl);
        }

        // Log.d(TAG, "ðŸŽ¬ Loading external subtitle: " + subtitleUrl);
        
        try {
            if (player == null) {
                Log.e(TAG, "ðŸŽ¬ Player is null, cannot load subtitle");
                return;
            }
            ...
            // Log.d(TAG, "ðŸŽ¬ Preparing player with subtitle...");
            ...
            // Log.d(TAG, "ðŸŽ¬ âœ… External subtitle loaded successfully: " + language);
            
        } catch (Exception e) {
            Log.e(TAG, "ðŸŽ¬ âŒ Error loading external subtitle: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void fetchCompleteMovieDataForWatchHistory(String movieId, String movieType) {
        ...
        Log.d(TAG, "ðŸ” Fetching complete movie data for enhanced watch history - ID: " + movieId + ", Type: " + movieType);
        ...
        call.enqueue(new Callback<MovieSingleDetails>() {
            ...
            public void onResponse(Call<MovieSingleDetails> call, Response<MovieSingleDetails> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    Log.d(TAG, "âœ… API Response received for ID: " + movieId);
                    Log.d(TAG, "ðŸ” API Response debug:");
                    ...
                    if (completeMovieData.getSeason() != null && !completeMovieData.getSeason().isEmpty()) {
                        Log.d(TAG, "ðŸ“º TV Series data - Seasons: " + completeMovieData.getSeason().size());
                    }
                    
                    Log.d(TAG, "ðŸ”„ CompleteMovieData updated from API, ready for enhanced watch history");
                    
                    // If we were waiting for this data, save watch history now
                    if (isWaitingForCompleteData && lastSavePosition >= 0 && lastSaveDuration > 0) {
                        Log.d(TAG, "ðŸ”„ API data received, retrying watch history save");
                    ...
                    Log.e(TAG, "âŒ Failed to fetch complete movie data - Response Code: " + response.code());
                    if (response.errorBody() != null) {
                        try {
                            Log.e(TAG, "âŒ Error body: " + response.errorBody().string());
                        } catch (Exception e) {
                            Log.e(TAG, "âŒ Error reading error body", e);
                        }
                    }
                    
                    // If we were waiting for this data and it failed, save with basic data
                    if (isWaitingForCompleteData) {
                        // Log.w(TAG, "âš ï¸ API fetch failed, falling back to basic save");
                        isWaitingForCompleteData = false;
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
            ...
            @Override
            public void onFailure(Call<MovieSingleDetails> call, Throwable t) {
                // Log.e(TAG, "âŒ API call failed for complete movie data", t);
