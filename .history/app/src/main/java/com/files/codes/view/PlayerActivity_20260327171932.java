    ...
    private void seekForward(long seekTimeMs) {
        if (player != null && player.isCurrentWindowSeekable()) {
            ...
            if (duration != C.TIME_UNSET) {
                ...
                // FIX: Safely seek without interrupting FFmpeg decoder
                // Step 1: PAUSE playback to let FFmpeg finish current frame decode
                boolean wasPlaying = player.getPlayWhenReady();
                Log.d("PlayerActivity", "⏸️ Pausing before seek (was playing: " + wasPlaying + ")");
                player.setPlayWhenReady(false);
                
                // Step 2: WAIT briefly for decoder buffers to flush (150ms is enough for EAC3)
                new Handler().postDelayed(() -> {
                    Log.d("PlayerActivity", "🔄 Decoder ready, performing seek to: " + (newPosition / 1000) + "s");
                    
                    // Step 3: NOW perform the actual seek (decoder is idle)
                    player.seekTo(newPosition);
                    
                    // Step 4: RESUME playback if it was playing before
                    if (wasPlaying) {
                        Log.d("PlayerActivity", "▶️ Resuming playback after seek");
                        player.setPlayWhenReady(true);
                    }
                    
                    // Clear seeking flag
                    new Handler().postDelayed(() -> isUserSeeking = false, 500);
                }, 150); // Wait 150ms for decoder to flush
            }
