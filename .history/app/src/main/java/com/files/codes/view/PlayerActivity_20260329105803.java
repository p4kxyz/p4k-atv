            ...
            if (mappedTrackInfo.getRendererType(rendererIndex) == C.TRACK_TYPE_AUDIO) {
                ...
                for (int groupIndex = 0; groupIndex < trackGroups.length; groupIndex++) {
                    ...
                    for (int trackIndex = 0; trackIndex < group.length; trackIndex++) {
                        ...
                        int channelScore = format.channelCount <= 2 ? 5 : 0;
                        int totalScore = languageScore + codecScore + channelScore;

                        Log.d("PlayerActivity", "🔊 Candidate audio track: mime=" + format.sampleMimeType
                                + ", lang=" + format.language
                                + ", label=" + format.label
                                + ", score=" + totalScore
                                + " (lang=" + languageScore + ", codec=" + codecScore + ")");
    ...
    private int getCompatibleAudioCodecScore(String mimeType) {
        ...
        switch (normalized) {
            ...
            case "audio/vnd.dts.hd":
                // We are in codec error recovery path, so avoid reselecting surround codecs
                // that are likely to trigger the same decoder failure again.
                return -1;
            default:
                return -1;
        }
