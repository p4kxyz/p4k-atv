...
//                }
        ...
        player.addListener(new Player.Listener() {
            ...
            public void onPlayerError(com.google.android.exoplayer2.PlaybackException error) {
                ...
                if (isVideoCodecError) {
                    ...
                    if (audioMode == 2) {
                        Log.e("PlayerActivity", "🔊 Passthrough mode: ignoring audio codec recovery and keeping passthrough as configured by user");
                        new ToastMsg(PlayerActivity.this).toastIconError("Passthrough: lỗi âm thanh đã được bỏ qua theo cài đặt.");
                        return;
                    }
                    ...
                    if (!hasHandledAudioCodecError) {
                        ...
                        Log.e("PlayerActivity", "🔊 Audio codec error already handled - attempting final track recovery before dialog");
                        boolean recovered = attemptFinalAudioTrackRecovery();
                        if (!recovered) {
                            Log.e("PlayerActivity", "🔊 Final recovery failed - forcing no-audio playback before dialog");
                            boolean continuedWithoutAudio = forceContinueWithoutAudioIfPossible();
                            if (!continuedWithoutAudio) {
                                Log.e("PlayerActivity", "🔊 Cannot continue playback without audio - showing user dialog");
    ...
    // Auto-select preferred audio/subtitle tracks when available (run once per session)
    private void applyPreferredLanguageTracks() {
        if (preferredTracksApplied || trackSelector == null) return;
        MappingTrackSelector.MappedTrackInfo mappedTrackInfo = trackSelector.getCurrentMappedTrackInfo();
        if (mappedTrackInfo == null) return;

        String preferredAudioLanguage = getPreferredAudioLanguageCode();
        String preferredSubtitleLanguage = getPreferredSubtitleLanguageCode();

        // Iterate renderers and pick tracks matching each configured preference.
        for (int rendererIndex = 0; rendererIndex < mappedTrackInfo.getRendererCount(); rendererIndex++) {
            ...
            if (rendererType == C.TRACK_TYPE_AUDIO) {
                targetLanguage = preferredAudioLanguage;
            } else if (rendererType == C.TRACK_TYPE_TEXT) {
                targetLanguage = preferredSubtitleLanguage;
            } else {
                continue;
            }
            ...
            for (int groupIndex = 0; groupIndex < trackGroups.length && !selected; groupIndex++) {
                TrackGroup group = trackGroups.get(groupIndex);
                for (int trackIndex = 0; trackIndex < group.length; trackIndex++) {
                    Format format = group.getFormat(trackIndex);
                    String normalized = normalizeAudioLanguage(format != null ? format.language : null,
                            format != null ? format.label : null);
                    if (targetLanguage.equals(normalized)) {
                        trackSelector.setParameters(
   ...
   /* @SuppressLint("StaticFieldLeak")
    ...
    private void renderSubtitleCuesWithBackgroundMode(List<Cue> cues) {
        ...
        if (!roundedSubtitleBackgroundEnabled) {
            subtitleView.setCues(cues);
            return;
        }
        subtitleView.setCues(createRoundedBackgroundCues(cues));
    }
    ...
    private List<Cue> createRoundedBackgroundCues(List<Cue> cues) {
        if (cues == null || cues.isEmpty()) {
            return cues;
        }

        List<Cue> roundedCues = new ArrayList<>(cues.size());
        for (Cue cue : cues) {
            ...
            if (cue.text instanceof Spanned) {
                ...
                RoundedBackgroundSpan[] existing = spanned.getSpans(0, spanned.length(), RoundedBackgroundSpan.class);
            ...
            CharSequence roundedText = applyRoundedBackgroundToText(cue.text);
            roundedCues.add(cue.buildUpon().setText(roundedText).build());
        }
        return roundedCues;
    }

    private CharSequence applyRoundedBackgroundToText(CharSequence text) {
    ...
    private static final class RoundedBackgroundSpan extends ReplacementSpan {
        ...
        private RoundedBackgroundSpan(int backgroundColor, float cornerRadiusPx, float horizontalPaddingPx, float verticalPaddingPx) {
        ...
        public void draw(Canvas canvas, CharSequence text, int start, int end, float x,
            ...
            canvas.drawText(text, start, end, x + horizontalPaddingPx, y, paint);
        }
    }
    ...
    */
    ...
    private void openUnifiedSettingsDialog() {
        ...
        // Audio language priority controls (used by automatic audio track selection)
        TextView audioLanguageLabel = new TextView(this);
        audioLanguageLabel.setTextColor(0xFFFFFFFF);
        audioLanguageLabel.setText("Ưu tiên ngôn ngữ âm thanh:");
        audioLanguageLabel.setTextSize(16);
        audioLanguageLabel.setPadding(0, 20, 0, 10);

        LinearLayout audioLanguageLayout = new LinearLayout(this);
        audioLanguageLayout.setOrientation(LinearLayout.HORIZONTAL);

        Button audioLanguagePrevBtn = new Button(this);
        audioLanguagePrevBtn.setText("◀");
        audioLanguagePrevBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable audioLanguagePrevBtnBg = new android.graphics.drawable.GradientDrawable();
        audioLanguagePrevBtnBg.setColor(0xFF3A3A4C); audioLanguagePrevBtnBg.setCornerRadius(dp(8)); audioLanguagePrevBtn.setBackground(audioLanguagePrevBtnBg);

        Button audioLanguageNextBtn = new Button(this);
        audioLanguageNextBtn.setText("▶");
        audioLanguageNextBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable audioLanguageNextBtnBg = new android.graphics.drawable.GradientDrawable();
        audioLanguageNextBtnBg.setColor(0xFF3A3A4C); audioLanguageNextBtnBg.setCornerRadius(dp(8)); audioLanguageNextBtn.setBackground(audioLanguageNextBtnBg);

        TextView audioLanguageTV = new TextView(this);
        audioLanguageTV.setTextColor(0xFFFFFFFF);
        String[] audioLanguageNames = {"Tiếng Việt", "English", "中文", "Theo hệ thống"};
        String[] audioLanguageValues = {"vi", "en", "zh", "device"};
        String currentAudioLanguagePref = prefs.getString("audio_language_priority", "vi");
        if (currentAudioLanguagePref == null || currentAudioLanguagePref.isEmpty()) {
            currentAudioLanguagePref = "vi";
            prefs.edit().putString("audio_language_priority", currentAudioLanguagePref).apply();
        }
        int currentAudioLanguageIndex = 0;
        for (int i = 0; i < audioLanguageValues.length; i++) {
            if (audioLanguageValues[i].equals(currentAudioLanguagePref)) {
                currentAudioLanguageIndex = i;
                break;
            }
        }
        audioLanguageTV.setText(audioLanguageNames[currentAudioLanguageIndex]);
        audioLanguageTV.setGravity(android.view.Gravity.CENTER);
        audioLanguageTV.setPadding(20, 0, 20, 0);

        audioLanguageLayout.addView(audioLanguagePrevBtn);
        audioLanguageLayout.addView(audioLanguageTV);
        audioLanguageLayout.addView(audioLanguageNextBtn);

        // Subtitle language priority controls (used by automatic subtitle track selection)
        TextView subtitleLanguageLabel = new TextView(this);
        subtitleLanguageLabel.setTextColor(0xFFFFFFFF);
        subtitleLanguageLabel.setText("Ưu tiên ngôn ngữ phụ đề:");
        subtitleLanguageLabel.setTextSize(16);
        subtitleLanguageLabel.setPadding(0, 20, 0, 10);

        LinearLayout subtitleLanguageLayout = new LinearLayout(this);
        subtitleLanguageLayout.setOrientation(LinearLayout.HORIZONTAL);

        Button subtitleLanguagePrevBtn = new Button(this);
        subtitleLanguagePrevBtn.setText("◀");
        subtitleLanguagePrevBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable subtitleLanguagePrevBtnBg = new android.graphics.drawable.GradientDrawable();
        subtitleLanguagePrevBtnBg.setColor(0xFF3A3A4C); subtitleLanguagePrevBtnBg.setCornerRadius(dp(8)); subtitleLanguagePrevBtn.setBackground(subtitleLanguagePrevBtnBg);

        Button subtitleLanguageNextBtn = new Button(this);
        subtitleLanguageNextBtn.setText("▶");
        subtitleLanguageNextBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable subtitleLanguageNextBtnBg = new android.graphics.drawable.GradientDrawable();
        subtitleLanguageNextBtnBg.setColor(0xFF3A3A4C); subtitleLanguageNextBtnBg.setCornerRadius(dp(8)); subtitleLanguageNextBtn.setBackground(subtitleLanguageNextBtnBg);

        TextView subtitleLanguageTV = new TextView(this);
        subtitleLanguageTV.setTextColor(0xFFFFFFFF);
        String[] subtitleLanguageNames = {"Tiếng Việt", "English", "中文", "Theo hệ thống"};
        String[] subtitleLanguageValues = {"vi", "en", "zh", "device"};
        String currentSubtitleLanguagePref = prefs.getString("subtitle_language_priority", "vi");
        if (currentSubtitleLanguagePref == null || currentSubtitleLanguagePref.isEmpty()) {
            currentSubtitleLanguagePref = "vi";
            prefs.edit().putString("subtitle_language_priority", currentSubtitleLanguagePref).apply();
        }
        int currentSubtitleLanguageIndex = 0;
        for (int i = 0; i < subtitleLanguageValues.length; i++) {
            if (subtitleLanguageValues[i].equals(currentSubtitleLanguagePref)) {
                currentSubtitleLanguageIndex = i;
                break;
            }
        }
        subtitleLanguageTV.setText(subtitleLanguageNames[currentSubtitleLanguageIndex]);
        subtitleLanguageTV.setGravity(android.view.Gravity.CENTER);
        subtitleLanguageTV.setPadding(20, 0, 20, 0);

        subtitleLanguageLayout.addView(subtitleLanguagePrevBtn);
        subtitleLanguageLayout.addView(subtitleLanguageTV);
        subtitleLanguageLayout.addView(subtitleLanguageNextBtn);
        ...
        applyWhiteFocusBorder(audioLanguagePrevBtn);
        applyWhiteFocusBorder(audioLanguageNextBtn);
        applyWhiteFocusBorder(subtitleLanguagePrevBtn);
        applyWhiteFocusBorder(subtitleLanguageNextBtn);
        ...
        layout.addView(audioLanguageLabel);
        layout.addView(audioLanguageLayout);
        layout.addView(subtitleLanguageLabel);
        layout.addView(subtitleLanguageLayout);
        ...
        audioLanguagePrevBtn.setOnClickListener(v -> {
            String currentValue = prefs.getString("audio_language_priority", "vi");
            int index = 0;
            for (int i = 0; i < audioLanguageValues.length; i++) {
                if (audioLanguageValues[i].equals(currentValue)) {
                    index = i;
                    break;
                }
            }
            index = (index - 1 + audioLanguageValues.length) % audioLanguageValues.length;
            prefs.edit().putString("audio_language_priority", audioLanguageValues[index]).apply();
            audioLanguageTV.setText(audioLanguageNames[index]);
            preferredTracksApplied = false;
            applyPreferredLanguageTracks();
        });

        audioLanguageNextBtn.setOnClickListener(v -> {
            String currentValue = prefs.getString("audio_language_priority", "vi");
            int index = 0;
            for (int i = 0; i < audioLanguageValues.length; i++) {
                if (audioLanguageValues[i].equals(currentValue)) {
            ...
            index = (index + 1) % audioLanguageValues.length;
            prefs.edit().putString("audio_language_priority", audioLanguageValues[index]).apply();
            audioLanguageTV.setText(audioLanguageNames[index]);
            preferredTracksApplied = false;
            applyPreferredLanguageTracks();
        ...
        subtitleLanguagePrevBtn.setOnClickListener(v -> {
            String currentValue = prefs.getString("subtitle_language_priority", "vi");
            int index = 0;
            for (int i = 0; i < subtitleLanguageValues.length; i++) {
                if (subtitleLanguageValues[i].equals(currentValue)) {
                    index = i;
                    break;
                }
            }
            index = (index - 1 + subtitleLanguageValues.length) % subtitleLanguageValues.length;
            prefs.edit().putString("subtitle_language_priority", subtitleLanguageValues[index]).apply();
            subtitleLanguageTV.setText(subtitleLanguageNames[index]);
            preferredTracksApplied = false;
            applyPreferredLanguageTracks();
        });

        subtitleLanguageNextBtn.setOnClickListener(v -> {
            String currentValue = prefs.getString("subtitle_language_priority", "vi");
            int index = 0;
            for (int i = 0; i < subtitleLanguageValues.length; i++) {
                if (subtitleLanguageValues[i].equals(currentValue)) {
            ...
            index = (index + 1) % subtitleLanguageValues.length;
            prefs.edit().putString("subtitle_language_priority", subtitleLanguageValues[index]).apply();
            subtitleLanguageTV.setText(subtitleLanguageNames[index]);
            preferredTracksApplied = false;
            applyPreferredLanguageTracks();
        ...
        resetBtn.setOnClickListener(v -> {
            ...
            editor.putString("audio_language_priority", "vi"); // Vietnamese
            editor.putString("subtitle_language_priority", "vi"); // Vietnamese
            ...
            audioLanguageTV.setText(audioLanguageNames[0]);
            subtitleLanguageTV.setText(subtitleLanguageNames[0]);
            preferredTracksApplied = false;
            applyPreferredLanguageTracks();
    ...
    private void handleAudioCodecError() {
        ...
        if (audioMode == 2) {
            Log.d("PlayerActivity", "🔊 Passthrough mode is locked - skipping audio error recovery flow");
            new ToastMsg(PlayerActivity.this).toastIconError("Passthrough: bỏ qua xử lý lỗi âm thanh.");
            return;
        }
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    if (!useSoftwareAudioDecoder) {
                        if (audioMode == 2) {
                            Log.d("PlayerActivity", "🔊 Passthrough mode is locked by user - skip auto switch to software decoder");
                        } else {
                    ...
                    // Strategy 2: Stay on current mode and try to find another compatible track.
                    Log.d("PlayerActivity", "🔊 Trying to recover by selecting a compatible audio track in current mode");
     ...
     */
        ...
        new ToastMsg(this).toastIconError("Không tìm thấy track âm thanh tương thích. Tiếp tục phát không tiếng.");
    }
    ...
    private boolean trySelectCompatibleAudioTrack(MappingTrackSelector.MappedTrackInfo mappedTrackInfo) {
        int bestRendererIndex = -1;
        TrackGroupArray bestTrackGroups = null;
        DefaultTrackSelector.SelectionOverride bestOverride = null;
        ...
        for (int rendererIndex = 0; rendererIndex < mappedTrackInfo.getRendererCount(); rendererIndex++) {
            if (mappedTrackInfo.getRendererType(rendererIndex) == C.TRACK_TYPE_AUDIO) {
                ...
                // Score all compatible tracks, then pick the best one.
                for (int groupIndex = 0; groupIndex < trackGroups.length; groupIndex++) {
                    TrackGroup group = trackGroups.get(groupIndex);
                    
                    for (int trackIndex = 0; trackIndex < group.length; trackIndex++) {
                        Format format = group.getFormat(trackIndex);
                        int codecScore = getCompatibleAudioCodecScore(format.sampleMimeType);
                        if (codecScore < 0) {
                            if (format.sampleMimeType != null) {
                                Log.d("PlayerActivity", "🔊 Skipping incompatible audio track: " + format.sampleMimeType);
                            }
                            continue;
                        }

                        int languageScore = getPreferredAudioLanguageScore(format);
                        int stereoScore = getStereoPreferenceScore(format);
                        int totalScore = languageScore + codecScore + stereoScore;

                        Log.d("PlayerActivity", "🔊 Candidate audio track: mime=" + format.sampleMimeType
                                + ", lang=" + format.language
                                + ", label=" + format.label
                                + ", score=" + totalScore
                                + " (lang=" + languageScore + ", codec=" + codecScore + ", stereo=" + stereoScore + ")");

                        if (totalScore > bestScore) {
                            bestScore = totalScore;
                            bestRendererIndex = rendererIndex;
                            bestTrackGroups = trackGroups;
                            bestOverride = new DefaultTrackSelector.SelectionOverride(groupIndex, trackIndex);
                        }
                    }
                }
            }
        ...
        if (bestOverride != null && bestTrackGroups != null && bestRendererIndex != -1) {
            trackSelector.setParameters(
                trackSelector.buildUponParameters()
                    .setSelectionOverride(bestRendererIndex, bestTrackGroups, bestOverride)
                    .setExceedRendererCapabilitiesIfNecessary(true)
                    .setTunnelingEnabled(false)
            );

            Format selectedFormat = bestTrackGroups.get(bestOverride.groupIndex).getFormat(bestOverride.tracks[0]);
            Log.d("PlayerActivity", "✅ Selected fallback audio track: mime=" + selectedFormat.sampleMimeType
                    + ", lang=" + selectedFormat.language
    ...
    private int getCompatibleAudioCodecScore(String mimeType) {
        if (mimeType == null) {
            return -1;
        }
        ...
        switch (normalized) {
            ...
            case "audio/eac3":
            case "audio/eac3-joc":
                // Keep AC3/EAC3 available in SW mode, but lower than AAC/MP3.
                return 20;
            case "audio/true-hd":
            case "audio/vnd.dts":
    ...
    private int getStereoPreferenceScore(Format format) {
    ...
    private boolean attemptFinalAudioTrackRecovery() {
        if (trackSelector == null || player == null) {
            return false;
        }
        ...
        boolean recovered = trySelectCompatibleAudioTrack(mappedTrackInfo);
        if (recovered) {
            showAudioTrackSwitchedNotification();
        }
        return recovered;
    ...
    private boolean forceContinueWithoutAudioIfPossible() {
        if (trackSelector == null || player == null) {
            return false;
        }
        ...
        try {
            ...
            Log.e("PlayerActivity", "Failed to continue without audio", e);
            return false;
        }
    }

    private int getPreferredAudioLanguageScore(Format format) {
        String normalizedLanguage = normalizeAudioLanguage(format != null ? format.language : null,
                format != null ? format.label : null);
        String preferredLanguage = getPreferredAudioLanguageCode();

        if (normalizedLanguage.isEmpty()) {
            return 10;
        }

        if (!preferredLanguage.isEmpty() && preferredLanguage.equals(normalizedLanguage)) {
            return 110;
        }
        ...
        if ("zh".equals(normalizedLanguage)) {
            return 60;
        }
    ...
    private String getPreferredAudioLanguageCode() {
        SharedPreferences prefs = getSharedPreferences("subtitle_settings", MODE_PRIVATE);
        String preference = prefs.getString("audio_language_priority", "vi");
        if (preference == null || preference.isEmpty()) {
            preference = "vi";
        }

        if ("device".equals(preference)) {
            String deviceLanguage = Locale.getDefault() != null ? Locale.getDefault().getLanguage() : "";
            return deviceLanguage != null ? deviceLanguage.toLowerCase(Locale.US) : "";
        }

        return preference.toLowerCase(Locale.US);
    }

    private String getPreferredSubtitleLanguageCode() {
        SharedPreferences prefs = getSharedPreferences("subtitle_settings", MODE_PRIVATE);
        String preference = prefs.getString("subtitle_language_priority", "vi");
        if (preference == null || preference.isEmpty()) {
            preference = "vi";
        }
    ...
    private String normalizeAudioLanguage(String language, String label) {
        String lang = language != null ? language.toLowerCase(Locale.US) : "";
        String lbl = label != null ? label.toLowerCase(Locale.US) : "";

        if (lang.startsWith("vi") || lang.equals("vie") || lang.contains("vietnam") || lbl.contains("viet")) {
            return "vi";
        }
        if (lang.startsWith("en") || lang.equals("eng") || lbl.contains("english")) {
            return "en";
        }
        if (lang.startsWith("zh") || lang.equals("chi") || lang.equals("zho") || lbl.contains("chinese")) {
            return "zh";
        }
     ...
     */
        ...
        new ToastMsg(this).toastIconSuccess("Đã tự động chuyển sang track âm thanh tương thích.");
    }
