...
public class PlayerActivity extends Activity {
    ...
    private boolean hasRetriedFromHttp416 = false;
    private boolean forceNoRangeHttpWorkaround = false;
...
//                }
    ...
    public void initVideoPlayer(String url, String type) {
        ...
        hasRetriedFromHttp416 = false;
        forceNoRangeHttpWorkaround = false;
        ...
        player.addListener(new Player.Listener() {
            ...
            public void onPlayerError(com.google.android.exoplayer2.PlaybackException error) {
                ...
                int httpResponseCode = extractHttpResponseCode(error);
                if (httpResponseCode == 416) {
                    Log.e("PlayerActivity", "🌐 HTTP 416 detected (Range Not Satisfiable). Retry from start.");
                    if (!hasRetriedFromHttp416) {
                        hasRetriedFromHttp416 = true;
                        forceNoRangeHttpWorkaround = true;
                        hasPlayerError = false;
                        if (recoverPlaybackFromStartForHttp416()) {
                            return;
                        }
                    }
                }
    ...
    private int extractHttpResponseCode(Throwable throwable) {
        Throwable current = throwable;
        while (current != null) {
            if (current instanceof com.google.android.exoplayer2.upstream.HttpDataSource.InvalidResponseCodeException) {
                return ((com.google.android.exoplayer2.upstream.HttpDataSource.InvalidResponseCodeException) current).responseCode;
            }
    ...
    private boolean recoverPlaybackFromStartForHttp416() {
        if (player == null) {
            return false;
        }

        final String retryBaseUrl = (originalVideoUrl != null && !originalVideoUrl.isEmpty()) ? originalVideoUrl : url;
        if (retryBaseUrl == null || retryBaseUrl.isEmpty()) {
            return false;
        }

        try {
            mStartingPosition = -1L;
            VideoTokenGenerator.resolveStreamUrlAsync(retryBaseUrl, new VideoTokenGenerator.StreamUrlCallback() {
                @Override
                public void onSuccess(String streamUrl) {
                    runOnUiThread(() -> {
                        try {
                            ...
                            String refreshedUrl = cleanVideoUrl(streamUrl);
                            if (refreshedUrl == null || refreshedUrl.isEmpty()) {
                                refreshedUrl = cleanVideoUrl(retryBaseUrl);
                            }

                            url = refreshedUrl;
                            Uri uri = Uri.parse(refreshedUrl);
                            MediaSource retrySource = createMediaSourceForUrl(uri, videoType);
                            if (retrySource == null) {
                                throw new IllegalStateException("retrySource is null");
                            }

                            player.stop();
                            player.clearMediaItems();
                            player.setMediaSource(retrySource);
                            player.prepare();
                            player.setPlayWhenReady(true);

                            Toast.makeText(PlayerActivity.this, "Đã làm mới link phát bảo mật, đang phát lại", Toast.LENGTH_SHORT).show();
                            Log.d("PlayerActivity", "✅ Recovered from HTTP 416 by refreshing secure stream URL");
                        } catch (Exception e) {
                            Log.e("PlayerActivity", "❌ Failed to apply refreshed secure URL", e);
                        }
                    });
                }
                ...
                @Override
                public void onError(Exception e) {
                    Log.e("PlayerActivity", "❌ Failed to refresh secure stream URL for HTTP 416", e);
                }
            });
            return true;
        } catch (Exception e) {
            Log.e("PlayerActivity", "❌ Failed recovering from HTTP 416", e);
            return false;
        }
    }
    ...
    private void seekToStartPosition() {
        ...
        if (mStartingPosition > 0L && !isUserSeeking) {
    ...
    DefaultHttpDataSource.Factory baseHttpFactory = new DefaultHttpDataSource.Factory()
    ...
    if (host != null && host.contains("phim4k.lol")) {
        defaultHeaders.put("Referer", "https://api.phim4k.lol/");
        defaultHeaders.put("Origin", "https://api.phim4k.lol");
    }
    baseHttpFactory.setDefaultRequestProperties(defaultHeaders);

    DataSource.Factory httpFactory = baseHttpFactory;
    if (shouldUseNoRangeHttpWorkaround(uri)) {
        Log.e("PlayerActivity", "🛠 Enabling no-range HTTP workaround for this source");
        httpFactory = () -> new NoRangeHttpDataSource(baseHttpFactory.createDataSource());
    }
    ...
    private boolean shouldUseNoRangeHttpWorkaround(Uri uri) {
        if (forceNoRangeHttpWorkaround) {
            return true;
        }

        String urlText = uri != null ? uri.toString() : "";
        if (containsAviHint(urlText)) {
            return true;
        }

        if (video != null) {
            if (containsAviHint(video.getLabel()) || containsAviHint(video.getFileType()) || containsAviHint(video.getFileUrl())) {
                return true;
            }
        }

        if (model != null) {
            if (containsAviHint(model.getTitle()) || containsAviHint(model.getVideoUrl())) {
                return true;
            }
        }
    ...
    private boolean containsAviHint(String value) {
        if (value == null || value.isEmpty()) {
            return false;
        }
        String lower = value.toLowerCase(Locale.US);
        return lower.contains(".avi") || lower.contains(" avi");
    }

    private static final class NoRangeHttpDataSource implements DataSource {
        private static final int SKIP_BUFFER_SIZE = 16 * 1024;
        private final DataSource upstream;
        private long bytesRemaining = C.LENGTH_UNSET;

        private NoRangeHttpDataSource(DataSource upstream) {
        ...
        public long open(DataSpec dataSpec) throws IOException {
            long targetPosition = Math.max(0L, dataSpec.position);
            DataSpec openSpec = dataSpec.buildUpon()
                    .setPosition(0L)
            ...
            // Always expose unknown length so ProgressiveMediaPeriod won't derive a negative
            // follow-up DataSpec length when reopening at later offsets.
        ...
        private void skipFully(long bytesToSkip) throws IOException {
            ...
            while (remaining > 0L) {
                ...
                if (read == C.RESULT_END_OF_INPUT) {
                    throw new IOException("Unable to skip to requested position: " + bytesToSkip);
                }
                remaining -= read;
            }
        }
        ...
        public Map<String, List<String>> getResponseHeaders() {
            if (upstream instanceof HttpDataSource) {
                return ((HttpDataSource) upstream).getResponseHeaders();
            }
    ...
    private List<Cue> createCustomOutlineCues(List<Cue> cues) {
        ...
        for (Cue cue : cues) {
            ...
            if (cue.text instanceof Spanned) {
                ...
                OutlineTextSpan[] existing = spanned.getSpans(0, spanned.length(), OutlineTextSpan.class);
            ...
            CharSequence outlinedText = applyOutlineToText(cue.text);
