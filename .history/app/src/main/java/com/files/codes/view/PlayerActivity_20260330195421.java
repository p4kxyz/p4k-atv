            ...
            public void buildAudioRenderers(Context context, int extensionRendererMode, com.google.android.exoplayer2.mediacodec.MediaCodecSelector mediaCodecSelector, boolean enableDecoderFallback, com.google.android.exoplayer2.audio.AudioSink audioSink, Handler eventHandler, com.google.android.exoplayer2.audio.AudioRendererEventListener eventListener, java.util.ArrayList<com.google.android.exoplayer2.Renderer> out) {
                ...
                if (audioMode == 1) { // Software Mode (FFmpeg PREFERRED)
                     ...
                     for (com.google.android.exoplayer2.Renderer r : out) {
                         ...
                         if (r instanceof com.google.android.exoplayer2.audio.AudioRenderer) {
                             audioRendererCount++;
                             Log.d("PlayerActivity", "   [Audio Renderer #" + audioRendererCount + "] " + rendererName);
                             
                             if (rendererName.contains("Ffmpeg") || rendererName.contains("Extension") || rendererName.toLowerCase().contains("codec")) {
                                 hasFFmpeg = true;
                     ...
                     if (!hasFFmpeg && audioRendererCount > 0) {
                         Log.w("PlayerActivity", "⚠️ SW MODE: FFmpeg not detected, but have " + audioRendererCount + " MediaCodec renderer(s). Will try to use them.");
                     } else if (audioRendererCount == 0) {
                         Log.e("PlayerActivity", "❌ SW MODE: NO AUDIO RENDERERS ADDED! Device may not support any audio decoders.");
                         // Force fallback to HW mode
                         audioMode = 0;
                         Log.d("PlayerActivity", "🔄 AUTO-FALLBACK: Switching to HW MODE due to no renderers in SW mode");
                         
                         // Clear existing renderers and rebuild in HW mode
                         out.clear();
                     ...
                     for (com.google.android.exoplayer2.Renderer r : out) {
                         if (r instanceof com.google.android.exoplayer2.audio.AudioRenderer) {
                             rendererCount++;
                             Log.d("PlayerActivity", "   [MediaCodec #" + rendererCount + "] " + r.getClass().getSimpleName());
                         }
                     }
                     
                     if (rendererCount == 0) {
                         Log.e("PlayerActivity", "❌ HW MODE: NO MEDIACODEC AUDIO RENDERERS! Device may lack audio codec support.");
                     }
