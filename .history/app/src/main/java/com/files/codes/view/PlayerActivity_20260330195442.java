...
//                }
    ...
    public void initVideoPlayer(String url, String type) {
        ...
        RenderersFactory renderersFactory = new DefaultRenderersFactory(this) {
            ...
            public void buildAudioRenderers(Context context, int extensionRendererMode, com.google.android.exoplayer2.mediacodec.MediaCodecSelector mediaCodecSelector, boolean enableDecoderFallback, com.google.android.exoplayer2.audio.AudioSink audioSink, Handler eventHandler, com.google.android.exoplayer2.audio.AudioRendererEventListener eventListener, java.util.ArrayList<com.google.android.exoplayer2.Renderer> out) {
                // Enhanced: Try FFmpeg first, fallback to MediaCodec if not available
                
                if (audioMode == 1) { // Software Mode (FFmpeg PREFERRED)
                     Log.d("PlayerActivity", "🔨 FORCE SW MODE: Requesting Extension Renderer Prefer...");
                     int initialSize = out.size();
                     
                     // Request FFmpeg-first priority
                     super.buildAudioRenderers(context, DefaultRenderersFactory.EXTENSION_RENDERER_MODE_PREFER, mediaCodecSelector, true, audioSink, eventHandler, eventListener, out);
                     
                     // Count renderers added
                     int renderersAdded = out.size() - initialSize;
                     Log.d("PlayerActivity", "📊 Renderers added: " + renderersAdded);
                     
                     // Detailed diagnostic: Check for FFmpeg
                     boolean hasFFmpeg = false;
                     int audioRendererCount = 0;
                     for (com.google.android.exoplayer2.Renderer r : out) {
                         String rendererName = r.getClass().getSimpleName();
                         String rendererPackage = r.getClass().getPackage().getName();
                         
                         // Check if this is an audio renderer (any renderer that can handle audio)
                         if (rendererName.contains("Audio") || rendererPackage.contains("audio")) {
                             audioRendererCount++;
                             Log.d("PlayerActivity", "   [Audio Renderer #" + audioRendererCount + "] " + rendererName);
                             
                             if (rendererName.contains("Ffmpeg") || rendererName.contains("Extension") || rendererName.toLowerCase().contains("encode")) {
                                 hasFFmpeg = true;
                                 Log.d("PlayerActivity", "      ✅ FFmpeg/Extension Renderer: " + rendererName);
                             }
                         }
                     }
                     
                     if (!hasFFmpeg && audioRendererCount > 0) {
                         Log.w("PlayerActivity", "⚠️ SW MODE: FFmpeg not detected, but have " + audioRendererCount + " audio renderer(s).");
                     } else if (audioRendererCount == 0) {
                         Log.e("PlayerActivity", "❌ SW MODE: NO AUDIO RENDERERS ADDED!");
                         // Force fallback to HW mode
                         audioMode = 0;
                         Log.d("PlayerActivity", "🔄 AUTO-FALLBACK: Switching to HW MODE due to no renderers");
                         
                         // Clear and rebuild in HW mode
                         out.clear();
                ...
                } else if (audioMode == 0) { // Hardware Mode (MediaCodec only)
                     ...
                     for (com.google.android.exoplayer2.Renderer r : out) {
                         String rendererName = r.getClass().getSimpleName();
                         if (rendererName.contains("Audio") || r.getClass().getPackage().getName().contains("audio")) {
                             rendererCount++;
                             Log.d("PlayerActivity", "   [MediaCodec #" + rendererCount + "] " + rendererName);
                         }
                     }
                     
                     if (rendererCount == 0) {
                         Log.e("PlayerActivity", "❌ HW MODE: NO AUDIO RENDERERS!");
                     }
    ...
    */
    ...
    private void handleAudioCodecError() {
        Log.d("PlayerActivity", "🔊 Handling audio codec error with smart fallback strategy");
        
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                try {
                    // Strategy 1: If in SW mode (FFmpeg) and failing, try HW mode (MediaCodec) as fallback
                    if (audioMode == 1) {
                        Log.d("PlayerActivity", "🔊 Currently in SW mode (FFmpeg) - trying HW fallback (MediaCodec)");
                        
                        // Clear saved selection to allow auto-selection for new renderer
                        savedAudioOverride = null;
                        savedAudioRendererIndex = -1;
                        Log.d("PlayerActivity", "🧹 Cleared saved audio selection for HW mode");
                        
                        useSoftwareAudioDecoder = false;
                        audioMode = 0; // Switch to HW mode
                        
                        // Reset error flag
                        hasHandledAudioCodecError = false;
                        
                        new ToastMsg(PlayerActivity.this).toastIconSuccess("FFmpeg gặp lỗi, chuyển sang phần cứng...");
                    ...
                    // Strategy 2: If in HW mode and still failing, try to find a compatible audio track
                    Log.d("PlayerActivity", "🔊 HW mode failed, trying to find compatible audio track");
