...
public class PlayerActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Diagnostic: Check FFmpeg extension availability
    ...
    private void checkFFmpegExtensionAvailability() {
        try {
            ...
            try {
                ...
                Log.w("PlayerActivity", "   FFmpeg extension AAR might not be properly included in APK");
            }
            
            // Check 2: Log device architecture (ABI)
            String[] abis = android.os.Build.SUPPORTED_ABIS;
            Log.d("PlayerActivity", "📱 Device Supported ABIs: " + android.Arrays.toString(abis));
            ...
            if (nativeLibDir.exists()) {
                ...
                if (libs != null && libs.length > 0) {
                    Log.d("PlayerActivity", "📦 Native libraries found: " + android.Arrays.toString(libs));
                    ...
                    if (!hasFFmpegLib) {
                        Log.w("PlayerActivity", "⚠️ No FFmpeg native libraries (*.so) found in: " + nativeLibDir.getAbsolutePath());
                    }
                } else {
                    Log.w("PlayerActivity", "⚠️ Native library directory is empty: " + nativeLibDir.getAbsolutePath());
                }
            } else {
                Log.w("PlayerActivity", "⚠️ Native library directory does not exist: " + nativeLibDir.getAbsolutePath());
            }
            
            // Check 4: Log compiler build info
            Log.d("PlayerActivity", "📋 Build Info:");
            Log.d("PlayerActivity", "   - Device: " + android.os.Build.DEVICE);
            Log.d("PlayerActivity", "   - SDK: " + android.os.Build.VERSION.SDK_INT);
            Log.d("PlayerActivity", "   - Product: " + android.os.Build.PRODUCT);
