...
=======
        int outlineWidth = prefs.getInt("outline_width", 2);
        if (outlineWidth < 0 || outlineWidth > 6) {
            outlineWidth = 2;
            prefs.edit().putInt("outline_width", outlineWidth).apply();
        }
>>>>>>> 361979c174e7feea0cd22935d5bd1ff89d8a2c9b
        ...
        subtitleOutlineWidthPx = dp(outlineWidth);
        useCustomSubtitleOutline = subtitleOutlineWidthPx > 0f && subtitleOutlineColor != Color.TRANSPARENT;
...
=======
                    useCustomSubtitleOutline ? CaptionStyleCompat.EDGE_TYPE_NONE : CaptionStyleCompat.EDGE_TYPE_OUTLINE,
>>>>>>> 361979c174e7feea0cd22935d5bd1ff89d8a2c9b
...
=======
        List<Cue> displayCues = normalizeCuePositionsForAppLayout(cues);
        if (useCustomSubtitleOutline) {
            displayCues = createCustomOutlineCues(displayCues);
        }
        if (!roundedSubtitleBackgroundEnabled) {
            subtitleView.setCues(displayCues);
            return;
        }
        subtitleView.setCues(createRoundedBackgroundCues(displayCues));
    }

    private List<Cue> createCustomOutlineCues(List<Cue> cues) {
        if (cues == null || cues.isEmpty()) {
            return cues;
        }

        List<Cue> outlinedCues = new ArrayList<>(cues.size());
        for (Cue cue : cues) {
            if (cue == null || cue.text == null || cue.text.length() == 0) {
                outlinedCues.add(cue);
                continue;
            }

            CharSequence outlineBaseText = removeOutlineSpans(cue.text);
            CharSequence outlinedText = applyOutlineToText(outlineBaseText);
            outlinedCues.add(cue.buildUpon().setText(outlinedText).build());
        }
        return outlinedCues;
    }

    private CharSequence removeOutlineSpans(CharSequence text) {
        if (!(text instanceof Spanned)) {
            return text;
        }

        SpannableString cleaned = SpannableString.valueOf(text);
        OutlineTextSpan[] existing = cleaned.getSpans(0, cleaned.length(), OutlineTextSpan.class);
        if (existing != null) {
            for (OutlineTextSpan span : existing) {
    ...
    private CharSequence applyOutlineToText(CharSequence text) {
        ...
        for (int i = 0; i <= textLength; i++) {
            ...
            if (i > lineStart) {
                spannable.setSpan(
                        new OutlineTextSpan(subtitleTextColor, subtitleOutlineColor, subtitleOutlineWidthPx),
                        lineStart,
                        i,
    ...
    private List<Cue> normalizeCuePositionsForAppLayout(List<Cue> cues) {
        if (cues == null || cues.isEmpty()) {
            return cues;
        }

        // External VTT cues often provide explicit line/position values.
        // When users adjust subtitle position in app settings, force cues back to default layout path.
        if (subtitleVerticalOffset == 0) {
            return cues;
        }

        List<Cue> normalized = new ArrayList<>(cues.size());
        for (Cue cue : cues) {
            if (cue == null) {
                normalized.add(null);
                continue;
...
=======
        ...
        TextView outlineWidthLabel = new TextView(this);
        outlineWidthLabel.setTextColor(0xFFFFFFFF);
        outlineWidthLabel.setText("Độ dày viền:");
        outlineWidthLabel.setTextSize(16);
        outlineWidthLabel.setPadding(0, 20, 0, 10);

        LinearLayout outlineWidthLayout = new LinearLayout(this);
        outlineWidthLayout.setOrientation(LinearLayout.HORIZONTAL);

        Button outlineWidthMinusBtn = new Button(this);
        outlineWidthMinusBtn.setText("-");
        outlineWidthMinusBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable outlineWidthMinusBtnBg = new android.graphics.drawable.GradientDrawable();
        outlineWidthMinusBtnBg.setColor(0xFF3A3A4C); outlineWidthMinusBtnBg.setCornerRadius(dp(8)); outlineWidthMinusBtn.setBackground(outlineWidthMinusBtnBg);

        Button outlineWidthPlusBtn = new Button(this);
        outlineWidthPlusBtn.setText("+");
        outlineWidthPlusBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable outlineWidthPlusBtnBg = new android.graphics.drawable.GradientDrawable();
        outlineWidthPlusBtnBg.setColor(0xFF3A3A4C); outlineWidthPlusBtnBg.setCornerRadius(dp(8)); outlineWidthPlusBtn.setBackground(outlineWidthPlusBtnBg);

        TextView outlineWidthTV = new TextView(this);
        outlineWidthTV.setTextColor(0xFFFFFFFF);
        int currentOutlineWidth = prefs.getInt("outline_width", 2);
        currentOutlineWidth = Math.max(0, Math.min(6, currentOutlineWidth));
        outlineWidthTV.setText(currentOutlineWidth == 0 ? "Tắt" : (currentOutlineWidth + "dp"));
        outlineWidthTV.setGravity(android.view.Gravity.CENTER);
        outlineWidthTV.setPadding(20, 0, 20, 0);

        outlineWidthLayout.addView(outlineWidthMinusBtn);
        outlineWidthLayout.addView(outlineWidthTV);
        outlineWidthLayout.addView(outlineWidthPlusBtn);
>>>>>>> 361979c174e7feea0cd22935d5bd1ff89d8a2c9b
...
=======
        applyWhiteFocusBorder(outlineWidthMinusBtn);
        applyWhiteFocusBorder(outlineWidthPlusBtn);
>>>>>>> 361979c174e7feea0cd22935d5bd1ff89d8a2c9b
...
=======
        layout.addView(outlineWidthLabel);
        layout.addView(outlineWidthLayout);
>>>>>>> 361979c174e7feea0cd22935d5bd1ff89d8a2c9b
...
<<<<<<< HEAD
        ...
        outlineThicknessNextBtn.setOnClickListener(v -> {
            ...
            applySubtitleSettings();
=======
        outlineWidthMinusBtn.setOnClickListener(v -> {
            int current = prefs.getInt("outline_width", 2);
            current = Math.max(0, current - 1);
            outlineWidthTV.setText(current == 0 ? "Tắt" : (current + "dp"));
            saveAndApplySubtitleSetting("outline_width", current);
        });

        outlineWidthPlusBtn.setOnClickListener(v -> {
            int current = prefs.getInt("outline_width", 2);
...
=======
            editor.putInt("outline_width", 2); // Medium
>>>>>>> 361979c174e7feea0cd22935d5bd1ff89d8a2c9b
