...
public class PlayerActivity extends Activity {
    ...
    private static final String PREF_SECONDARY_SUBTITLE_ENABLED = "secondary_subtitle_enabled";
    private static final String PREF_DUAL_SUBTITLE_ENABLED = "dual_subtitle_enabled";
    private static final String PREF_DUAL_SUBTITLE_SECONDARY_URL = "dual_subtitle_secondary_url";
    private static final String PREF_DUAL_SUBTITLE_SECONDARY_LABEL = "dual_subtitle_secondary_label";
    private static final String PREF_SECONDARY_VERTICAL_OFFSET = "secondary_vertical_offset";
    private boolean dualSubtitleRealEnabled = false;
    private String dualSubtitleSecondaryUrl = "";
    private String dualSubtitleSecondaryLabel = "";
    private String loadedDualSubtitleUrl = "";
    private String currentSecondarySubtitleText = null;
    private final List<DualSubtitleCueEntry> dualSubtitleCueEntries = new ArrayList<>();
    private ExoPlayer secondarySubtitlePlayer;
    private DefaultTrackSelector secondarySubtitleTrackSelector;
    private String pendingEmbeddedSecondaryDescriptor = "";
    private boolean embeddedSecondaryTrackApplied = false;
    private final Handler dualSubtitleHandler = new Handler();
    private final Runnable dualSubtitleTicker = new Runnable() {
        @Override
        public void run() {
            if (!dualSubtitleRealEnabled || player == null || secondarySubtitleView == null) {
                dualSubtitleHandler.removeCallbacks(this);
                return;
            }
            updateSecondarySubtitleAtCurrentPosition();
            dualSubtitleHandler.postDelayed(this, 220);
        }
    };
    private final Runnable secondaryEmbeddedSyncRunnable = new Runnable() {
        @Override
        public void run() {
            if (!isDualSubtitleEmbeddedMode() || player == null || secondarySubtitlePlayer == null) {
                dualSubtitleHandler.removeCallbacks(this);
                return;
            }

            try {
                if (!embeddedSecondaryTrackApplied) {
                    applyEmbeddedSecondaryTrackOverride();
                }

                boolean primaryPlayWhenReady = player.getPlayWhenReady();
                if (secondarySubtitlePlayer.getPlayWhenReady() != primaryPlayWhenReady) {
                    secondarySubtitlePlayer.setPlayWhenReady(primaryPlayWhenReady);
                }

                PlaybackParameters pp = player.getPlaybackParameters();
                if (pp != null) {
                    secondarySubtitlePlayer.setPlaybackParameters(pp);
                }

                long pPos = player.getCurrentPosition();
                long sPos = secondarySubtitlePlayer.getCurrentPosition();
                if (Math.abs(pPos - sPos) > 350) {
                    secondarySubtitlePlayer.seekTo(pPos);
                }
            } catch (Exception e) {
                Log.w(TAG, "Secondary embedded subtitle sync warning", e);
            }

            dualSubtitleHandler.postDelayed(this, 250);
        }
    };
    ...
    private void intiViews() {
        ...
        secondarySubtitleView = findViewById(R.id.subtitle_view_secondary);
...
//                }
    ...
    private void openSubtitleDialog() {
        ...
        TextView subtitleModeLabel = view.findViewById(R.id.subtitle_mode_label);
        Button primaryModeButton = view.findViewById(R.id.btn_primary_subtitle_mode);
        Button secondaryModeButton = view.findViewById(R.id.btn_secondary_subtitle_mode);
        final boolean[] selectingSecondarySubtitle = {false};

        Runnable refreshModeUI = () -> {
            if (subtitleModeLabel != null) {
                subtitleModeLabel.setText(selectingSecondarySubtitle[0]
                        ? "Đang chọn: Phụ đề thứ 2"
                        : "Đang chọn: Phụ đề chính");
            }
            if (primaryModeButton != null) {
                primaryModeButton.setAlpha(selectingSecondarySubtitle[0] ? 0.65f : 1.0f);
            }
            if (secondaryModeButton != null) {
                secondaryModeButton.setAlpha(selectingSecondarySubtitle[0] ? 1.0f : 0.65f);
            }
        };

        if (primaryModeButton != null) {
            primaryModeButton.setOnClickListener(v -> {
                selectingSecondarySubtitle[0] = false;
                refreshModeUI.run();
            });
        }
        if (secondaryModeButton != null) {
            secondaryModeButton.setOnClickListener(v -> {
                selectingSecondarySubtitle[0] = true;
                refreshModeUI.run();
            });
        }
        refreshModeUI.run();
        ...
        adapter.setListener(new SubtitleListAdapter.OnSubtitleItemClickListener() {
            ...
            public void onSubtitleItemClick(View view, Subtitle subtitle, int position, SubtitleListAdapter.SubtitleViewHolder holder) {
                ...
                if (selectingSecondarySubtitle[0]) {
                    if (url == null || url.isEmpty() || "off".equalsIgnoreCase(url)) {
                        SharedPreferences.Editor editor = getSharedPreferences("subtitle_settings", MODE_PRIVATE).edit();
                        editor.putBoolean(PREF_DUAL_SUBTITLE_ENABLED, false);
                        editor.remove(PREF_DUAL_SUBTITLE_SECONDARY_URL);
                        editor.remove(PREF_DUAL_SUBTITLE_SECONDARY_LABEL);
                        editor.apply();
                        applySubtitleSettings();
                        new ToastMsg(PlayerActivity.this).toastIconSuccess("Đã tắt phụ đề thứ 2");
                        dialog.dismiss();
                        return;
                    }

                    String label = subtitle.getLanguage() == null ? "Phụ đề thứ 2" : subtitle.getLanguage();
                    SharedPreferences.Editor editor = getSharedPreferences("subtitle_settings", MODE_PRIVATE).edit();
                    editor.putBoolean(PREF_DUAL_SUBTITLE_ENABLED, true);
                    editor.putString(PREF_DUAL_SUBTITLE_SECONDARY_URL, url);
                    editor.putString(PREF_DUAL_SUBTITLE_SECONDARY_LABEL, label);
                    editor.apply();

                    dualSubtitleSecondaryUrl = url;
                    dualSubtitleSecondaryLabel = label;
                    dualSubtitleRealEnabled = true;
                    if (url.startsWith("embedded:")) {
                        dualSubtitleCueEntries.clear();
                        loadedDualSubtitleUrl = "";
                        startSecondaryEmbeddedSubtitlePlayer(url);
                    } else {
                        stopSecondaryEmbeddedSubtitlePlayer();
                        loadDualSubtitleFromUrlAsync(url);
                    }
                    applySubtitleSettings();
                    new ToastMsg(PlayerActivity.this).toastIconSuccess("Đã chọn phụ đề thứ 2: " + label);
                    dialog.dismiss();
                    return;
                }
   ...
   /* @SuppressLint("StaticFieldLeak")
    ...
    private void releasePlayer() {
        dualSubtitleHandler.removeCallbacks(dualSubtitleTicker);
        dualSubtitleHandler.removeCallbacks(secondaryEmbeddedSyncRunnable);
        stopSecondaryEmbeddedSubtitlePlayer();
        currentSecondarySubtitleText = null;
        if (secondarySubtitleView != null) {
            secondarySubtitleView.setCues(null);
    ...
    private void applySubtitleSettings() {
        ...
        SharedPreferences prefs = getSharedPreferences("subtitle_settings", MODE_PRIVATE);
        dualSubtitleRealEnabled = prefs.getBoolean(
            PREF_DUAL_SUBTITLE_ENABLED,
            prefs.getBoolean(PREF_SECONDARY_SUBTITLE_ENABLED, false)
        );
        dualSubtitleSecondaryUrl = prefs.getString(PREF_DUAL_SUBTITLE_SECONDARY_URL, "");
        dualSubtitleSecondaryLabel = prefs.getString(PREF_DUAL_SUBTITLE_SECONDARY_LABEL, "");
        int fontSize = prefs.getInt("font_size", 20);
        int verticalOffset = prefs.getInt("vertical_offset", 0);
        int secondaryVerticalOffset = prefs.getInt(PREF_SECONDARY_VERTICAL_OFFSET, -6);
        ...
        if (subtitleView != null) {
            ...
            if (secondarySubtitleView != null) {
                CaptionStyleCompat secondaryStyle = new CaptionStyleCompat(
                        textColor,
                        Color.TRANSPARENT,
                        Color.TRANSPARENT,
                        CaptionStyleCompat.EDGE_TYPE_NONE,
                ...
                secondarySubtitleView.setStyle(secondaryStyle);
                secondarySubtitleView.setFixedTextSize(
                        Cue.TEXT_SIZE_TYPE_ABSOLUTE,
                        Math.max(14, fontSize - 4)
                );
                float secondaryBottomPadding = Math.max(
                    0.0f,
                    Math.min(0.85f, bottomPadding + (secondaryVerticalOffset * 0.01f))
                );
                secondarySubtitleView.setBottomPaddingFraction(secondaryBottomPadding);
                secondarySubtitleView.setVisibility(View.GONE);
            }
            ...
            Log.d(TAG, "Applied subtitle - colors: text=" + Integer.toHexString(textColor) + 
                      ...
                      ", primaryPosition: " + verticalOffset + "% offset, secondaryPosition: " + secondaryVerticalOffset + "% offset, bottomPadding: " + bottomPadding);

            if (dualSubtitleRealEnabled && !dualSubtitleSecondaryUrl.isEmpty()) {
                if (isDualSubtitleEmbeddedMode()) {
                    startSecondaryEmbeddedSubtitlePlayer(dualSubtitleSecondaryUrl);
                } else if (!dualSubtitleSecondaryUrl.equals(loadedDualSubtitleUrl)) {
                    stopSecondaryEmbeddedSubtitlePlayer();
                    loadDualSubtitleFromUrlAsync(dualSubtitleSecondaryUrl);
                }
            } else {
                stopSecondaryEmbeddedSubtitlePlayer();
            }
            updateDualSubtitleTickerState();
        }
    }
    ...
    private void renderSubtitleCuesWithBackgroundMode(List<Cue> cues) {
        ...
        List<Cue> styledCues = createStyledSubtitleCues(cues);
        subtitleView.setCues(styledCues);
        if (secondarySubtitleView != null && !dualSubtitleRealEnabled) {
            secondarySubtitleView.setCues(null);
            secondarySubtitleView.setVisibility(View.GONE);
        }
    }

    private void showDualSubtitleSecondaryPicker(android.widget.TextView selectedTextView, android.widget.Switch dualSubtitleSwitch) {
        List<Subtitle> combinedSubtitleList = new ArrayList<>();

        // 1) Add embedded subtitles from current playing media (MKV/HLS text tracks)
        if (trackSelector != null) {
            MappingTrackSelector.MappedTrackInfo mappedTrackInfo = trackSelector.getCurrentMappedTrackInfo();
            if (mappedTrackInfo != null) {
                for (int rendererIndex = 0; rendererIndex < mappedTrackInfo.getRendererCount(); rendererIndex++) {
                    ...
                    for (int groupIndex = 0; groupIndex < trackGroups.length; groupIndex++) {
                        ...
                        for (int trackIndex = 0; trackIndex < group.length; trackIndex++) {
                            ...
                            String label = format.label != null ? format.label : lang;
                            embeddedSub.setLanguage(label + " (Muxed)");
                            embeddedSub.setUrl("embedded:" + rendererIndex + ":" + groupIndex + ":" + trackIndex);
                            combinedSubtitleList.add(embeddedSub);
                        }
                    }
                }
            }
        ...
        // 2) Add external subtitles from API
        List<Subtitle> subtitleList = (video != null) ? video.getSubtitle() : null;
        if (subtitleList != null && !subtitleList.isEmpty()) {
            combinedSubtitleList.addAll(subtitleList);
        }

        if (combinedSubtitleList.isEmpty()) {
            Log.w(TAG, "Dual subtitle picker: no embedded/external subtitle candidates yet");
            new ToastMsg(this).toastIconError("Không có phụ đề để làm phụ đề thứ 2");
            return;
        }

        Log.d(TAG, "Dual subtitle picker: total candidates = " + combinedSubtitleList.size());

        List<String> labels = new ArrayList<>();
        List<String> urls = new ArrayList<>();
        int checkedIndex = -1;

        for (Subtitle subtitle : combinedSubtitleList) {
            if (subtitle == null) continue;
            String subUrl = subtitle.getUrl();
            if (subUrl == null || subUrl.trim().isEmpty() || "off".equalsIgnoreCase(subUrl.trim())) continue;

            String language = subtitle.getLanguage();
            String label = (language == null || language.trim().isEmpty()) ? subUrl : language;
            labels.add(label);
            urls.add(subUrl.trim());

            if (subUrl.trim().equalsIgnoreCase(dualSubtitleSecondaryUrl)) {
                checkedIndex = labels.size() - 1;
            }
        }

        if (labels.isEmpty()) {
            Log.w(TAG, "Dual subtitle picker: labels empty after filtering, candidates=" + combinedSubtitleList.size());
            new ToastMsg(this).toastIconError("Không tìm thấy phụ đề hợp lệ cho phụ đề thứ 2");
            return;
        }
        ...
        for (int i = 0; i < labels.size(); i++) {
            final int idx = i;
            actions[i] = () -> {
                String pickedUrl = urls.get(idx);
                String pickedLabel = labels.get(idx);

                SharedPreferences.Editor editor = getSharedPreferences("subtitle_settings", MODE_PRIVATE).edit();
                editor.putString(PREF_DUAL_SUBTITLE_SECONDARY_URL, pickedUrl);
                editor.putString(PREF_DUAL_SUBTITLE_SECONDARY_LABEL, pickedLabel);
                editor.putBoolean(PREF_DUAL_SUBTITLE_ENABLED, true);
                editor.apply();

                dualSubtitleSecondaryUrl = pickedUrl;
                dualSubtitleSecondaryLabel = pickedLabel;
                dualSubtitleRealEnabled = true;
                if (dualSubtitleSwitch != null) {
                    dualSubtitleSwitch.setChecked(true);
                }
                if (selectedTextView != null) {
                    selectedTextView.setText("Phụ đề thứ 2: " + pickedLabel);
                }

                if (pickedUrl.startsWith("embedded:")) {
                    dualSubtitleCueEntries.clear();
                    loadedDualSubtitleUrl = "";
                    startSecondaryEmbeddedSubtitlePlayer(pickedUrl);
                } else {
                    stopSecondaryEmbeddedSubtitlePlayer();
                    loadDualSubtitleFromUrlAsync(pickedUrl);
                }
                applySubtitleSettings();
                new ToastMsg(this).toastIconSuccess("Đã chọn phụ đề thứ 2: " + pickedLabel);
            };
        }

        buildCustomListDialog(
                "Chọn phụ đề thứ 2 (song ngữ)",
                labels.toArray(new String[0]),
                checkedIndex,
                actions,
                null
    ...
    private void clearDualSubtitleSelection(android.widget.TextView selectedTextView, android.widget.Switch dualSubtitleSwitch) {
        SharedPreferences.Editor editor = getSharedPreferences("subtitle_settings", MODE_PRIVATE).edit();
        editor.remove(PREF_DUAL_SUBTITLE_SECONDARY_URL);
        editor.remove(PREF_DUAL_SUBTITLE_SECONDARY_LABEL);
        editor.putBoolean(PREF_DUAL_SUBTITLE_ENABLED, false);
        ...
        dualSubtitleSecondaryUrl = "";
        dualSubtitleSecondaryLabel = "";
        dualSubtitleRealEnabled = false;
        loadedDualSubtitleUrl = "";
        currentSecondarySubtitleText = null;
        dualSubtitleCueEntries.clear();
        stopSecondaryEmbeddedSubtitlePlayer();
        ...
        if (dualSubtitleSwitch != null) {
            dualSubtitleSwitch.setChecked(false);
        }
        if (selectedTextView != null) {
            selectedTextView.setText("Phụ đề thứ 2: Chưa chọn");
        }
    ...
    private void loadDualSubtitleFromUrlAsync(String subtitleUrl) {
        if (subtitleUrl != null && subtitleUrl.startsWith("embedded:")) {
            dualSubtitleCueEntries.clear();
            loadedDualSubtitleUrl = "";
            startSecondaryEmbeddedSubtitlePlayer(subtitleUrl);
            return;
        }
        ...
        final String targetUrl = subtitleUrl.trim();
        stopSecondaryEmbeddedSubtitlePlayer();
        loadedDualSubtitleUrl = "";
        dualSubtitleCueEntries.clear();
        currentSecondarySubtitleText = null;
        ...
        new Thread(() -> {
            List<DualSubtitleCueEntry> parsedEntries = downloadAndParseSubtitleEntries(targetUrl);
            runOnUiThread(() -> {
                if (!targetUrl.equalsIgnoreCase(dualSubtitleSecondaryUrl)) {
                    return;
                }
                dualSubtitleCueEntries.clear();
                dualSubtitleCueEntries.addAll(parsedEntries);
                loadedDualSubtitleUrl = targetUrl;
                updateDualSubtitleTickerState();
            });
        }).start();
    }

    private void updateDualSubtitleTickerState() {
        if (secondarySubtitleView == null) {
            return;
        }
        ...
        if (isDualSubtitleEmbeddedMode()) {
            dualSubtitleHandler.removeCallbacks(dualSubtitleTicker);
            if (dualSubtitleRealEnabled && player != null && secondarySubtitlePlayer != null) {
                secondarySubtitleView.setVisibility(View.VISIBLE);
                startSecondaryEmbeddedSubtitleSync();
            } else if (!dualSubtitleRealEnabled) {
                secondarySubtitleView.setCues(null);
                secondarySubtitleView.setVisibility(View.GONE);
            }
            return;
        ...
        boolean canShow = dualSubtitleRealEnabled && player != null && !dualSubtitleCueEntries.isEmpty();
        if (!canShow) {
            dualSubtitleHandler.removeCallbacks(dualSubtitleTicker);
            currentSecondarySubtitleText = null;
            secondarySubtitleView.setCues(null);
        ...
        secondarySubtitleView.setVisibility(View.VISIBLE);
        dualSubtitleHandler.removeCallbacks(dualSubtitleTicker);
        dualSubtitleHandler.post(dualSubtitleTicker);
    }

    private void updateSecondarySubtitleAtCurrentPosition() {
        if (player == null || secondarySubtitleView == null) {
            return;
        }

        long positionMs = player.getCurrentPosition();
        String text = findDualSubtitleTextAt(positionMs);
        if (text != null && text.isEmpty()) {
            text = null;
        }

        if ((currentSecondarySubtitleText == null && text == null)
                || (currentSecondarySubtitleText != null && currentSecondarySubtitleText.equals(text))) {
            return;
        }

        currentSecondarySubtitleText = text;
        if (text == null) {
            secondarySubtitleView.setCues(Collections.emptyList());
            return;
        }

        List<Cue> cues = new ArrayList<>(1);
        cues.add(new Cue.Builder().setText(text).build());
        secondarySubtitleView.setCues(createStyledSubtitleCues(cues));
    }

    private boolean isDualSubtitleEmbeddedMode() {
        return dualSubtitleRealEnabled
                && dualSubtitleSecondaryUrl != null
                && dualSubtitleSecondaryUrl.startsWith("embedded:");
    }

    private void startSecondaryEmbeddedSubtitlePlayer(String embeddedDescriptor) {
        if (embeddedDescriptor == null || !embeddedDescriptor.startsWith("embedded:")) {
            stopSecondaryEmbeddedSubtitlePlayer();
            return;
        }

        if (url == null || url.trim().isEmpty()) {
            Log.w(TAG, "Cannot start secondary embedded subtitle player: missing video url");
            return;
        }

        if (secondarySubtitlePlayer != null && embeddedDescriptor.equals(pendingEmbeddedSecondaryDescriptor)) {
            startSecondaryEmbeddedSubtitleSync();
            return;
        }
        ...
        pendingEmbeddedSecondaryDescriptor = embeddedDescriptor;
        embeddedSecondaryTrackApplied = false;

        secondarySubtitleTrackSelector = new DefaultTrackSelector(this);
        secondarySubtitlePlayer = new ExoPlayer.Builder(this)
                .setTrackSelector(secondarySubtitleTrackSelector)
                .build();

        secondarySubtitlePlayer.setVolume(0f);
        secondarySubtitlePlayer.addListener(new Player.Listener() {
            @Override
            ...
            @Override
            public void onCues(List<Cue> cues) {
                if (secondarySubtitleView == null || !isDualSubtitleEmbeddedMode()) {
                    return;
                }
                secondarySubtitleView.setVisibility(View.VISIBLE);
                secondarySubtitleView.setCues(createStyledSubtitleCues(cues));
            ...
            public void onCues(CueGroup cueGroup) {
                ...
                List<Cue> cues = cueGroup != null ? cueGroup.cues : Collections.emptyList();
                secondarySubtitleView.setVisibility(View.VISIBLE);
                secondarySubtitleView.setCues(createStyledSubtitleCues(cues));
            }
        });
        ...
        try {
            String type = videoType != null ? videoType : detectVideoFormat(url);
            Uri uri = Uri.parse(url);
            com.google.android.exoplayer2.source.MediaSource source;
            if ("hls".equals(type)) {
                source = hlsMediaSource(uri, this);
            } else if ("rtmp".equals(type)) {
                source = rtmpMediaSource(uri);
            } else {
                source = mediaSource(uri, this);
            }
            secondarySubtitlePlayer.setMediaSource(source);
            secondarySubtitlePlayer.prepare();

            if (player != null) {
                secondarySubtitlePlayer.seekTo(player.getCurrentPosition());
                secondarySubtitlePlayer.setPlayWhenReady(player.getPlayWhenReady());
                secondarySubtitlePlayer.setPlaybackParameters(player.getPlaybackParameters());
            }

            startSecondaryEmbeddedSubtitleSync();
            updateDualSubtitleTickerState();
            Log.d(TAG, "Started secondary embedded subtitle player for descriptor: " + embeddedDescriptor);
        } catch (Exception e) {
            Log.e(TAG, "Failed to start secondary embedded subtitle player", e);
            stopSecondaryEmbeddedSubtitlePlayer();
        }
    }

    private void applyEmbeddedSecondaryTrackOverride() {
        if (secondarySubtitlePlayer == null
                || secondarySubtitleTrackSelector == null
                || pendingEmbeddedSecondaryDescriptor == null
                || pendingEmbeddedSecondaryDescriptor.isEmpty()) {
            return;
        }

        try {
            String[] parts = pendingEmbeddedSecondaryDescriptor.split(":");
            if (parts.length < 4) {
                return;
            }
            ...
            MappingTrackSelector.MappedTrackInfo mappedTrackInfo = secondarySubtitleTrackSelector.getCurrentMappedTrackInfo();
            if (mappedTrackInfo == null) {
                return;
            }
            ...
            if (textRendererIndex < 0) {
                Log.w(TAG, "Secondary subtitle player has no text renderer");
                return;
            }

            TrackGroupArray textGroups = mappedTrackInfo.getTrackGroups(textRendererIndex);
            if (textGroups == null || textGroups.length == 0) {
                Log.w(TAG, "Secondary subtitle player has empty text groups");
                return;
            }

            int resolvedGroupIndex = groupIndex;
            int resolvedTrackIndex = trackIndex;

            if (resolvedGroupIndex < 0 || resolvedGroupIndex >= textGroups.length) {
                Log.w(TAG, "Fallback secondary subtitle group index from " + groupIndex + " to 0");
                resolvedGroupIndex = 0;
            }

            TrackGroup selectedGroup = textGroups.get(resolvedGroupIndex);
            if (selectedGroup == null || selectedGroup.length == 0) {
                return;
            }

            if (resolvedTrackIndex < 0 || resolvedTrackIndex >= selectedGroup.length) {
                Log.w(TAG, "Fallback secondary subtitle track index from " + trackIndex + " to 0");
            ...
            DefaultTrackSelector.Parameters.Builder builder = secondarySubtitleTrackSelector.buildUponParameters();
            for (int i = 0; i < mappedTrackInfo.getRendererCount(); i++) {
            ...
            textRendererIndex,
                    textGroups,
            new DefaultTrackSelector.SelectionOverride(resolvedGroupIndex, resolvedTrackIndex)
            );
            secondarySubtitleTrackSelector.setParameters(builder);
            embeddedSecondaryTrackApplied = true;
        Log.d(TAG, "Applied secondary embedded subtitle override at textRenderer=" + textRendererIndex
            + ", group=" + resolvedGroupIndex + ", track=" + resolvedTrackIndex);
        } catch (Exception e) {
            Log.w(TAG, "Cannot apply embedded secondary subtitle override", e);
        }
    }

    private void startSecondaryEmbeddedSubtitleSync() {
        dualSubtitleHandler.removeCallbacks(secondaryEmbeddedSyncRunnable);
        if (isDualSubtitleEmbeddedMode() && player != null && secondarySubtitlePlayer != null) {
            dualSubtitleHandler.post(secondaryEmbeddedSyncRunnable);
        }
    }

    private void stopSecondaryEmbeddedSubtitlePlayer() {
        dualSubtitleHandler.removeCallbacks(secondaryEmbeddedSyncRunnable);
        pendingEmbeddedSecondaryDescriptor = "";
        embeddedSecondaryTrackApplied = false;

        if (secondarySubtitlePlayer != null) {
            try {
                secondarySubtitlePlayer.setPlayWhenReady(false);
                secondarySubtitlePlayer.stop();
                secondarySubtitlePlayer.release();
            } catch (Exception e) {
                Log.w(TAG, "Error releasing secondary subtitle player", e);
            }
            secondarySubtitlePlayer = null;
        }
        secondarySubtitleTrackSelector = null;
    }

    private String findDualSubtitleTextAt(long positionMs) {
        for (DualSubtitleCueEntry entry : dualSubtitleCueEntries) {
    ...
    private List<DualSubtitleCueEntry> downloadAndParseSubtitleEntries(String subtitleUrl) {
        List<DualSubtitleCueEntry> entries = new ArrayList<>();
        HttpURLConnection connection = null;
        BufferedReader reader = null;

        try {
            URL urlObj = new URL(subtitleUrl);
            ...
            entries.addAll(parseTimedSubtitleContent(content.toString()));
        } catch (Exception e) {
            Log.e(TAG, "Không thể tải phụ đề thứ 2: " + subtitleUrl, e);
        } finally {
            try {
    ...
    private List<DualSubtitleCueEntry> parseTimedSubtitleContent(String content) {
        List<DualSubtitleCueEntry> entries = new ArrayList<>();
        ...
        while (i < lines.length) {
            ...
            long startMs = parseSubtitleTimestampMs(timeParts[0]);
            long endMs = parseSubtitleTimestampMs(timeParts[1]);
            ...
            String text = textBuilder.toString().replaceAll("<[^>]+>", "").trim();
            if (startMs >= 0 && endMs > startMs && !text.isEmpty()) {
                entries.add(new DualSubtitleCueEntry(startMs, endMs, text));
            }
            i++;
        }
    ...
    private long parseSubtitleTimestampMs(String raw) {
        ...
        try {
            ...
            return (long) ((hours * 3600L + minutes * 60L + seconds) * 1000L);
        } catch (Exception e) {
            return -1;
        }
    }

    private static final class DualSubtitleCueEntry {
        final long startMs;
        final long endMs;
        final String text;

        DualSubtitleCueEntry(long startMs, long endMs, String text) {
    ...
    */
    ...
    private void openUnifiedSettingsDialog() {
        ...
        // Real dual subtitle controls
        TextView dualSubtitleSectionLabel = new TextView(this);
        dualSubtitleSectionLabel.setTextColor(0xFFFFFFFF);
        dualSubtitleSectionLabel.setText("Song ngữ thật (2 phụ đề độc lập):");
        dualSubtitleSectionLabel.setTextSize(16);
        dualSubtitleSectionLabel.setPadding(0, 0, 0, 10);

        LinearLayout dualSubtitleSwitchLayout = new LinearLayout(this);
        dualSubtitleSwitchLayout.setOrientation(LinearLayout.HORIZONTAL);
        dualSubtitleSwitchLayout.setPadding(0, 0, 0, 10);
        TextView dualSubtitleSwitchLabel = new TextView(this);
        dualSubtitleSwitchLabel.setTextColor(0xFFFFFFFF);
        dualSubtitleSwitchLabel.setText("Bật song ngữ");
        dualSubtitleSwitchLabel.setTextSize(15);
        android.widget.Switch dualSubtitleSwitch = new android.widget.Switch(this);
        boolean dualEnabledPref = prefs.getBoolean(PREF_DUAL_SUBTITLE_ENABLED, prefs.getBoolean(PREF_SECONDARY_SUBTITLE_ENABLED, false));
        dualSubtitleSwitch.setChecked(dualEnabledPref);
        dualSubtitleSwitchLayout.addView(dualSubtitleSwitchLabel);
        dualSubtitleSwitchLayout.addView(dualSubtitleSwitch);

        TextView dualSubtitleSelectedLabel = new TextView(this);
        dualSubtitleSelectedLabel.setTextColor(0xFFFFFFFF);
        String secondaryLabelPref = prefs.getString(PREF_DUAL_SUBTITLE_SECONDARY_LABEL, "");
        dualSubtitleSelectedLabel.setText("Phụ đề thứ 2: " + (secondaryLabelPref.isEmpty() ? "Chưa chọn" : secondaryLabelPref));
        dualSubtitleSelectedLabel.setTextSize(14);
        dualSubtitleSelectedLabel.setPadding(0, 0, 0, 10);

        LinearLayout dualSubtitleButtonLayout = new LinearLayout(this);
        dualSubtitleButtonLayout.setOrientation(LinearLayout.HORIZONTAL);
        dualSubtitleButtonLayout.setPadding(0, 0, 0, 14);

        Button dualSubtitlePickBtn = new Button(this);
        dualSubtitlePickBtn.setText("Chọn phụ đề thứ 2");
        dualSubtitlePickBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable dualSubtitlePickBtnBg = new android.graphics.drawable.GradientDrawable();
        dualSubtitlePickBtnBg.setColor(0xFF3A3A4C);
        dualSubtitlePickBtnBg.setCornerRadius(dp(8));
        dualSubtitlePickBtn.setBackground(dualSubtitlePickBtnBg);

        Button dualSubtitleClearBtn = new Button(this);
        dualSubtitleClearBtn.setText("Xóa");
        dualSubtitleClearBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable dualSubtitleClearBtnBg = new android.graphics.drawable.GradientDrawable();
        dualSubtitleClearBtnBg.setColor(0xFF5A2F2F);
        dualSubtitleClearBtnBg.setCornerRadius(dp(8));
        dualSubtitleClearBtn.setBackground(dualSubtitleClearBtnBg);

        LinearLayout.LayoutParams pickLp = new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f);
        LinearLayout.LayoutParams clearLp = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
        clearLp.leftMargin = dp(10);
        dualSubtitleButtonLayout.addView(dualSubtitlePickBtn, pickLp);
        dualSubtitleButtonLayout.addView(dualSubtitleClearBtn, clearLp);
        ...
        android.graphics.drawable.GradientDrawable secondaryPositionUpBtnBg = new android.graphics.drawable.GradientDrawable();
        secondaryPositionUpBtnBg.setColor(0xFF3A3A4C); secondaryPositionUpBtnBg.setCornerRadius(dp(8)); secondaryPositionUpBtn.setBackground(secondaryPositionUpBtnBg);
        ...
        android.graphics.drawable.GradientDrawable secondaryPositionDownBtnBg = new android.graphics.drawable.GradientDrawable();
        secondaryPositionDownBtnBg.setColor(0xFF3A3A4C); secondaryPositionDownBtnBg.setCornerRadius(dp(8)); secondaryPositionDownBtn.setBackground(secondaryPositionDownBtnBg);

        TextView secondaryPositionTV = new TextView(this);
        secondaryPositionTV.setTextColor(0xFFFFFFFF);
        int currentSecondaryOffset = prefs.getInt(PREF_SECONDARY_VERTICAL_OFFSET, -6);
        secondaryPositionTV.setText("Dịch chuyển (" + (currentSecondaryOffset > 0 ? "+" : "") + currentSecondaryOffset + "%)");
        secondaryPositionTV.setGravity(android.view.Gravity.CENTER);
        ...
        layout.addView(dualSubtitleSectionLabel);
        layout.addView(dualSubtitleSwitchLayout);
        layout.addView(dualSubtitleSelectedLabel);
        layout.addView(dualSubtitleButtonLayout);
        ...
        dualSubtitleSwitch.setOnCheckedChangeListener((buttonView, isChecked) -> {
            saveAndApplySubtitleSetting(PREF_DUAL_SUBTITLE_ENABLED, isChecked);
        });

        dualSubtitlePickBtn.setOnClickListener(v ->
                showDualSubtitleSecondaryPicker(dualSubtitleSelectedLabel, dualSubtitleSwitch)
        );

        dualSubtitleClearBtn.setOnClickListener(v -> {
            clearDualSubtitleSelection(dualSubtitleSelectedLabel, dualSubtitleSwitch);
            dualSubtitleSelectedLabel.setText("Phụ đề thứ 2: Chưa chọn");
        });
        ...
        secondaryPositionUpBtn.setOnClickListener(v -> {
            int offset = prefs.getInt(PREF_SECONDARY_VERTICAL_OFFSET, -6);
            offset += 1;
            offset = Math.min(offset, 30);
            secondaryPositionTV.setText("Dịch chuyển (" + (offset > 0 ? "+" : "") + offset + "%)");
        ...
        resetBtn.setOnClickListener(v -> {
            ...
            editor.putInt(PREF_SECONDARY_VERTICAL_OFFSET, -6);
            ...
            secondaryPositionTV.setText("Dịch chuyển (-6%)");
