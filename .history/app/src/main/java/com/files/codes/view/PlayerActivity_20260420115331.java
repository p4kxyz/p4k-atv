...
public class PlayerActivity extends Activity {
    ...
    private static final String SUBSOURCE_PREFS = "subsource_settings";
    private static final String PREF_SUBSOURCE_API_KEY = "subsource_api_key";
    private static final String SUBSOURCE_BASE_URL = "https://api.subsource.net/api/v1";
    ...
    private final Set<String> subsourceTempFiles = new HashSet<>();
...
//                }
    ...
    private void openSubtitleDialog() {
        ...
        // Always show the unified subtitle dialog so users can fetch from Subsource
        // even when API/embedded subtitles are unavailable.
        if (video == null) {
            video = new Video();
        }
        ...
        EditText subsourceApiKeyInput = view.findViewById(R.id.subsource_api_key_input);
        Button subsourceFetchButton = view.findViewById(R.id.subsource_fetch_button);

        // Fallback: if the inflated layout does not contain Subsource controls,
        // create them at runtime so API key input is always available.
        if (subsourceApiKeyInput == null || subsourceFetchButton == null) {
            View divider = view.findViewById(R.id.divider);
            RelativeLayout root = (view instanceof RelativeLayout) ? (RelativeLayout) view : null;
            if (root != null && divider != null) {
                LinearLayout dynamicControls = new LinearLayout(PlayerActivity.this);
                int controlsId = View.generateViewId();
                dynamicControls.setId(controlsId);
                dynamicControls.setOrientation(LinearLayout.VERTICAL);

                RelativeLayout.LayoutParams controlsLp = new RelativeLayout.LayoutParams(
                    RelativeLayout.LayoutParams.MATCH_PARENT,
                    RelativeLayout.LayoutParams.WRAP_CONTENT
                );
                controlsLp.addRule(RelativeLayout.BELOW, divider.getId());
                controlsLp.topMargin = dp(8);
                dynamicControls.setLayoutParams(controlsLp);

                TextView dynamicLabel = new TextView(PlayerActivity.this);
                dynamicLabel.setText(getString(R.string.subsource_api_key_label));
                dynamicLabel.setTextSize(TypedValue.COMPLEX_UNIT_SP, 14f);

                EditText dynamicInput = new EditText(PlayerActivity.this);
                LinearLayout.LayoutParams inputLp = new LinearLayout.LayoutParams(
                    LinearLayout.LayoutParams.MATCH_PARENT,
                    LinearLayout.LayoutParams.WRAP_CONTENT
                );
                inputLp.topMargin = dp(4);
                dynamicInput.setLayoutParams(inputLp);
                dynamicInput.setHint(getString(R.string.subsource_api_key_hint));
                dynamicInput.setImeOptions(android.view.inputmethod.EditorInfo.IME_ACTION_DONE);
                dynamicInput.setInputType(android.text.InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS);
                dynamicInput.setMaxLines(1);
                dynamicInput.setSingleLine(true);
                dynamicInput.setTextSize(TypedValue.COMPLEX_UNIT_SP, 14f);

                Button dynamicFetch = new Button(PlayerActivity.this);
                LinearLayout.LayoutParams fetchLp = new LinearLayout.LayoutParams(
                    LinearLayout.LayoutParams.WRAP_CONTENT,
                    LinearLayout.LayoutParams.WRAP_CONTENT
                );
                fetchLp.topMargin = dp(6);
                dynamicFetch.setLayoutParams(fetchLp);
                dynamicFetch.setBackgroundResource(R.drawable.button_focus);
                dynamicFetch.setText(getString(R.string.subsource_fetch_button));
                dynamicFetch.setTextColor(Color.WHITE);

                dynamicControls.addView(dynamicLabel);
                dynamicControls.addView(dynamicInput);
                dynamicControls.addView(dynamicFetch);
                root.addView(dynamicControls);

                RelativeLayout.LayoutParams serverLp = (RelativeLayout.LayoutParams) serverRv.getLayoutParams();
                serverLp.addRule(RelativeLayout.BELOW, controlsId);
                serverLp.topMargin = dp(5);
                serverRv.setLayoutParams(serverLp);

                subsourceApiKeyInput = dynamicInput;
                subsourceFetchButton = dynamicFetch;
            }
        }

        if (subsourceApiKeyInput == null || subsourceFetchButton == null) {
            new ToastMsg(PlayerActivity.this).toastIconError("Subsource UI not available");
        }

        String storedApiKey = getStoredSubsourceApiKey();
        if (subsourceApiKeyInput != null && !TextUtils.isEmpty(storedApiKey)) {
            subsourceApiKeyInput.setText(storedApiKey);
        }

        if (subsourceFetchButton != null && subsourceApiKeyInput != null) {
            EditText finalSubsourceApiKeyInput = subsourceApiKeyInput;
            Button finalSubsourceFetchButton = subsourceFetchButton;
            subsourceFetchButton.setOnClickListener(v -> {
            String apiKey = finalSubsourceApiKeyInput.getText() != null
                ? finalSubsourceApiKeyInput.getText().toString().trim()
                : "";

            if (TextUtils.isEmpty(apiKey)) {
                new ToastMsg(PlayerActivity.this).toastIconError(getString(R.string.subsource_missing_api_key));
                return;
            }

            saveSubsourceApiKey(apiKey);
            finalSubsourceFetchButton.setEnabled(false);
            fetchSubsourceSubtitles(apiKey, combinedSubtitles, adapter, finalSubsourceFetchButton);
            });
        }
        ...
        adapter.setListener(new SubtitleListAdapter.OnSubtitleItemClickListener() {
            ...
            public void onSubtitleItemClick(View view, Subtitle subtitle, int position, SubtitleListAdapter.SubtitleViewHolder holder) {
                ...
                if ("off".equals(url)) {
                    ...
                    dialog.dismiss();
                } else if (url != null && url.startsWith("subsource:")) {
                    try {
                        int subtitleId = Integer.parseInt(url.substring("subsource:".length()));
                        downloadAndLoadSubsourceSubtitle(subtitleId, subtitle.getLanguage());
                    } catch (Exception e) {
                        Log.e(TAG, "Invalid subsource subtitle id", e);
                        new ToastMsg(PlayerActivity.this).toastIconError(getString(R.string.subsource_download_failed));
                    }
                    dialog.dismiss();
                } else {
    ...
    private String getStoredSubsourceApiKey() {
        return getSharedPreferences(SUBSOURCE_PREFS, MODE_PRIVATE).getString(PREF_SUBSOURCE_API_KEY, "");
    }

    private void saveSubsourceApiKey(String apiKey) {
        getSharedPreferences(SUBSOURCE_PREFS, MODE_PRIVATE)
            .edit()
            .putString(PREF_SUBSOURCE_API_KEY, apiKey)
            .apply();
    }

    private void fetchSubsourceSubtitles(String apiKey,
                                         List<Subtitle> combinedSubtitles,
                                         SubtitleListAdapter adapter,
                                         Button fetchButton) {
        new ToastMsg(this).toastIconSuccess(getString(R.string.subsource_fetching));

        new Thread(() -> {
            try {
                List<Subtitle> fetchedSubtitles = fetchSubsourceSubtitleList(apiKey);
                runOnUiThread(() -> {
                    removeSubsourceEntries(combinedSubtitles);
                    combinedSubtitles.addAll(fetchedSubtitles);
                    adapter.notifyDataSetChanged();
                    fetchButton.setEnabled(true);

                    if (fetchedSubtitles.isEmpty()) {
                        new ToastMsg(PlayerActivity.this).toastIconError(getString(R.string.subsource_no_result));
                    } else {
                        new ToastMsg(PlayerActivity.this).toastIconSuccess("Subsource: " + fetchedSubtitles.size() + " phụ đề");
                    }
                });
            } catch (Exception e) {
                Log.e(TAG, "Subsource fetch failed", e);
                runOnUiThread(() -> {
                    fetchButton.setEnabled(true);
                    new ToastMsg(PlayerActivity.this).toastIconError(e.getMessage() != null
                        ? e.getMessage()
                        : getString(R.string.subsource_no_result));
                });
            }
        }).start();
    }

    private void removeSubsourceEntries(List<Subtitle> subtitles) {
        for (int i = subtitles.size() - 1; i >= 0; i--) {
            Subtitle subtitle = subtitles.get(i);
            String url = subtitle.getUrl();
            if (url != null && url.startsWith("subsource:")) {
                subtitles.remove(i);
            }
        }
    }

    private List<Subtitle> fetchSubsourceSubtitleList(String apiKey) throws Exception {
        List<Subtitle> result = new ArrayList<>();
        Integer movieId = resolveSubsourceMovieId(apiKey);
        if (movieId == null) {
            return result;
        }

        String subtitleEndpoint = SUBSOURCE_BASE_URL
            + "/subtitles?movieId=" + movieId
            + "&sort=rating"
            + "&limit=50";

        JSONObject subtitleJson = performSubsourceGet(subtitleEndpoint, apiKey);
        if (!subtitleJson.optBoolean("success", false)) {
            return result;
        }

        JSONArray data = subtitleJson.optJSONArray("data");
        if (data == null) {
            return result;
        }
        ...
        for (int i = 0; i < data.length(); i++) {
            ...
            int subtitleId = item.optInt("subtitleId", -1);
            if (subtitleId <= 0) continue;

            String language = item.optString("language", "unknown");
            String languageDisplay = formatLanguage(language);

            String releaseInfoText = "";
            JSONArray releaseInfoArray = item.optJSONArray("releaseInfo");
            if (releaseInfoArray != null && releaseInfoArray.length() > 0) {
                StringBuilder releaseInfoBuilder = new StringBuilder();
                for (int j = 0; j < Math.min(2, releaseInfoArray.length()); j++) {
                    String part = releaseInfoArray.optString(j, "").trim();
                    if (!part.isEmpty()) {
                        if (releaseInfoBuilder.length() > 0) {
                            releaseInfoBuilder.append(" ");
                        }
                        releaseInfoBuilder.append(part);
                    }
                }
                releaseInfoText = releaseInfoBuilder.toString();
            }

            Subtitle subtitle = new Subtitle();
            subtitle.setLanguage(
                releaseInfoText.isEmpty()
                    ? languageDisplay + " [Subsource]"
                    : languageDisplay + " - " + releaseInfoText + " [Subsource]"
            );
            subtitle.setSrclang(language);
            subtitle.setKind("subsource");
            subtitle.setUrl("subsource:" + subtitleId);
            result.add(subtitle);
        }
    ...
    private Integer resolveSubsourceMovieId(String apiKey) throws Exception {
        String tmdbCandidate = getTmdbIdCandidate();
        String imdbCandidate = getImdbIdCandidate();
        String titleCandidate = getTitleCandidate();
        Integer yearCandidate = getYearCandidate();
        boolean isSeries = isTvSeriesContent();
        int seasonCandidate = model != null ? model.getCurrentSeasonIndex() + 1 : -1;

        if (!TextUtils.isEmpty(imdbCandidate)) {
            String imdbQuery = encodeQueryParam(imdbCandidate);
            String endpoint = SUBSOURCE_BASE_URL + "/movies/search?searchType=imdb&imdb=" + imdbQuery;
            JSONArray imdbData = getSubsourceSearchData(endpoint, apiKey);
            Integer imdbPicked = pickSubsourceMovieId(imdbData, tmdbCandidate, yearCandidate, seasonCandidate);
            if (imdbPicked != null) {
                return imdbPicked;
            }
        }

        if (TextUtils.isEmpty(titleCandidate)) {
            return null;
        }

        StringBuilder textEndpoint = new StringBuilder();
        textEndpoint.append(SUBSOURCE_BASE_URL)
            .append("/movies/search?searchType=text&q=")
            .append(encodeQueryParam(titleCandidate));

        textEndpoint.append("&type=").append(isSeries ? "series" : "movie");

        if (yearCandidate != null) {
            textEndpoint.append("&year=").append(yearCandidate);
        }

        if (isSeries && seasonCandidate > 0) {
            textEndpoint.append("&season=").append(seasonCandidate);
        }

        JSONArray textData = getSubsourceSearchData(textEndpoint.toString(), apiKey);
        return pickSubsourceMovieId(textData, tmdbCandidate, yearCandidate, seasonCandidate);
    }

    private JSONArray getSubsourceSearchData(String endpoint, String apiKey) throws Exception {
        JSONObject json = performSubsourceGet(endpoint, apiKey);
    ...
    private Integer pickSubsourceMovieId(JSONArray data,
                                         String tmdbCandidate,
                                         Integer yearCandidate,
                                         int seasonCandidate) {
        ...
        for (int i = 0; i < data.length(); i++) {
            ...
            String tmdbId = item.optString("tmdbId", "");
            if (!TextUtils.isEmpty(tmdbCandidate) && tmdbCandidate.equals(tmdbId)) {
                int movieId = item.optInt("movieId", -1);
        ...
        for (int i = 0; i < data.length(); i++) {
            ...
            int releaseYear = item.optInt("releaseYear", -1);
            Integer season = item.isNull("season") ? null : item.optInt("season", -1);
            int movieId = item.optInt("movieId", -1);
            if (movieId <= 0) continue;

            boolean yearMatch = yearCandidate == null || releaseYear == yearCandidate;
            boolean seasonMatch = seasonCandidate <= 0 || season == null || season == seasonCandidate;
        ...
        if (first != null) {
            int movieId = first.optInt("movieId", -1);
            if (movieId > 0) {
                return movieId;
            }
        }
    ...
    private JSONObject performSubsourceGet(String endpoint, String apiKey) throws Exception {
        ...
        try {
            ...
            String body = readStreamAsString(stream);
            if (responseCode < 200 || responseCode >= 300) {
                throw new IOException("Subsource HTTP " + responseCode + (TextUtils.isEmpty(body) ? "" : " - " + body));
            }
    ...
    private String getTmdbIdCandidate() {
        Intent intent = getIntent();
        if (intent == null) return "";

        String[] keys = new String[]{"tmdb_id", "tmdbId", "tmdb"};
        for (String key : keys) {
    ...
    private String getImdbIdCandidate() {
        Intent intent = getIntent();
        if (intent != null) {
            String[] keys = new String[]{"imdb_id", "imdbId", "imdb"};
            for (String key : keys) {
                String value = intent.getStringExtra(key);
                if (!TextUtils.isEmpty(value)) {
                    Matcher matcher = Pattern.compile("tt\\d+").matcher(value);
                    if (matcher.find()) {
        ...
        if (model != null && !TextUtils.isEmpty(model.getMovieId())) {
            Matcher matcher = Pattern.compile("tt\\d+").matcher(model.getMovieId());
            if (matcher.find()) {
                return matcher.group();
            }
        }
    ...
    private String getTitleCandidate() {
        if (completeMovieData != null && !TextUtils.isEmpty(completeMovieData.getTitle())) {
            return completeMovieData.getTitle().trim();
        }
        if (model != null && !TextUtils.isEmpty(model.getTitle())) {
            return model.getTitle().trim();
        }
        return "";
    }

    private Integer getYearCandidate() {
        if (completeMovieData != null && !TextUtils.isEmpty(completeMovieData.getRelease())) {
            Integer year = extractYearFromString(completeMovieData.getRelease());
            if (year != null) return year;
        }

        if (model != null && !TextUtils.isEmpty(model.getReleaseDate())) {
            Integer year = extractYearFromString(model.getReleaseDate());
            if (year != null) return year;
        }
        ...
        if (intent != null) {
            ...
            if (!TextUtils.isEmpty(releaseDate)) {
                return extractYearFromString(releaseDate);
            }
        }
    ...
    private Integer extractYearFromString(String raw) {
        if (TextUtils.isEmpty(raw)) return null;
        Matcher matcher = Pattern.compile("(19|20)\\d{2}").matcher(raw);
        if (matcher.find()) {
            try {
    ...
    private boolean isTvSeriesContent() {
        if (model != null) {
            if (!TextUtils.isEmpty(model.getIsTvSeries()) && "1".equals(model.getIsTvSeries())) {
                return true;
            }
            if (!TextUtils.isEmpty(model.getVideoType()) && model.getVideoType().toLowerCase(Locale.US).contains("series")) {
                return true;
            }
            if (!TextUtils.isEmpty(model.getCategory()) && model.getCategory().toLowerCase(Locale.US).contains("series")) {
                return true;
            }
        }
        return false;
    ...
    private String formatLanguage(String language) {
        if (TextUtils.isEmpty(language)) return "Unknown";
        String normalized = language.replace("_", " ").replace("-", " ").trim();
        if (normalized.isEmpty()) return "Unknown";
        return normalized.substring(0, 1).toUpperCase(Locale.US) + normalized.substring(1);
    }

    private void downloadAndLoadSubsourceSubtitle(int subtitleId, String language) {
        String apiKey = getStoredSubsourceApiKey();
        if (TextUtils.isEmpty(apiKey)) {
            new ToastMsg(this).toastIconError(getString(R.string.subsource_missing_api_key));
            return;
        }

        new ToastMsg(this).toastIconSuccess(getString(R.string.subsource_downloading));

        new Thread(() -> {
            try {
                File localSubtitleFile = downloadSubsourceSubtitleToLocalFile(apiKey, subtitleId);
                if (localSubtitleFile == null || !localSubtitleFile.exists()) {
                    throw new IOException("Không có file phụ đề hợp lệ");
                }

                runOnUiThread(() -> {
                    markTrackSwitchTransition("subsource-subtitle-load");
                    String subtitleUrl = Uri.fromFile(localSubtitleFile).toString();
                    loadExternalSubtitle(subtitleUrl, language);
                });
            } catch (Exception e) {
                Log.e(TAG, "Subsource download failed", e);
                runOnUiThread(() -> new ToastMsg(PlayerActivity.this).toastIconError(getString(R.string.subsource_download_failed)));
            }
        }).start();
    }

    private File downloadSubsourceSubtitleToLocalFile(String apiKey, int subtitleId) throws Exception {
        File tempDir = new File(getCacheDir(), "subsource_subtitles");
        if (!tempDir.exists() && !tempDir.mkdirs()) {
            throw new IOException("Không tạo được thư mục phụ đề tạm");
        }

        String downloadEndpoint = SUBSOURCE_BASE_URL + "/subtitles/" + subtitleId + "/download";
        HttpURLConnection connection = null;
        InputStream inputStream = null;
        BufferedOutputStream outputStream = null;
        File zipFile = new File(tempDir, "subsource_" + subtitleId + "_" + System.currentTimeMillis() + ".zip");

        try {
            URL urlObj = new URL(downloadEndpoint);
            connection = (HttpURLConnection) urlObj.openConnection();
            connection.setRequestMethod("GET");
            connection.setConnectTimeout(12000);
            ...
            if (code < 200 || code >= 300) {
                String errorText = readStreamAsString(connection.getErrorStream());
                throw new IOException("Subsource HTTP " + code + (TextUtils.isEmpty(errorText) ? "" : " - " + errorText));
            }

            inputStream = new BufferedInputStream(connection.getInputStream());
            outputStream = new BufferedOutputStream(new FileOutputStream(zipFile));
        ...
        trackSubsourceTempFile(zipFile);
        return extractFirstSupportedSubtitleFromZip(zipFile, tempDir, subtitleId);
    }

    private File extractFirstSupportedSubtitleFromZip(File zipFile, File outputDir, int subtitleId) throws Exception {
        ...
        try (ZipInputStream zis = new ZipInputStream(new BufferedInputStream(new java.io.FileInputStream(zipFile)))) {
            ...
            while ((entry = zis.getNextEntry()) != null) {
                ...
                File outFile = new File(outputDir, subtitleId + "_" + System.currentTimeMillis() + "_" + safeName);
                ...
                trackSubsourceTempFile(outFile);

                String lower = safeName.toLowerCase(Locale.US);
                if (selectedSubtitle == null && (lower.endsWith(".vtt")
                    || lower.endsWith(".srt")
                    || lower.endsWith(".ssa")
                    || lower.endsWith(".ass")
                    || lower.endsWith(".ttml")
                    || lower.endsWith(".xml"))) {
                    selectedSubtitle = outFile;
                }
        ...
        if (selectedSubtitle == null) {
            throw new IOException("ZIP không chứa file phụ đề hỗ trợ");
        }

        return selectedSubtitle;
    }

    private synchronized void trackSubsourceTempFile(File file) {
        if (file != null) {
            subsourceTempFiles.add(file.getAbsolutePath());
        }
    }

    private synchronized void cleanupSubsourceTempFiles() {
        for (String path : subsourceTempFiles) {
            try {
                File f = new File(path);
                if (f.exists()) {
                    boolean deleted = f.delete();
                    if (!deleted) {
                        Log.w(TAG, "Could not delete temp subtitle file: " + path);
                    }
                }
            } catch (Exception e) {
                Log.w(TAG, "Error deleting temp subtitle file: " + path, e);
            }
        }
        subsourceTempFiles.clear();
    }
    ...
    */
    ...
    private void showNoSubtitleMessage() {
        ...
        // Always open the unified subtitle dialog (contains Off + embedded + Subsource UI).
        if (!hasEmbeddedSub) {
            new ToastMsg(this).toastIconError("Không tìm thấy phụ đề từ server. Bạn có thể dùng Subsource.");
        }
        openSubtitleDialog();
    }
    ...
    private void loadExternalSubtitle(String subtitleUrl, String language) {
        ...
        try {
            ...
            String subtitleMimeType = inferSubtitleMimeType(subtitleUrl);
            String subtitleLang = TextUtils.isEmpty(language) ? "und" : language;
            ...
            if (trackSelector != null) {
                ...
                trackSelector.setParameters(
                    trackSelector.buildUponParameters()
                        .setPreferredTextLanguage(subtitleLang) // Try to match by language
                        .setRendererDisabled(C.TRACK_TYPE_TEXT, false)
                );
            }
    ...
    private String inferSubtitleMimeType(String subtitleUrl) {
        if (TextUtils.isEmpty(subtitleUrl)) {
            return MimeTypes.TEXT_VTT;
        }

        String lower = subtitleUrl.toLowerCase(Locale.US);
