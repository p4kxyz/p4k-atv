...
//                }
    ...
    */
    ...
    private String resolveCurrentVideoFileId(String fallbackId) {
        if (video != null && !isNullOrEmpty(video.getVideoFileId())) {
            return video.getVideoFileId();
        }

        if (model != null && model.getVideo() != null && !isNullOrEmpty(model.getVideo().getVideoFileId())) {
        ...
        if (model != null && model.getVideoList() != null && !model.getVideoList().isEmpty()) {
            ...
            for (Video item : model.getVideoList()) {
                if (item == null) continue;
                if (firstNonEmpty == null && !isNullOrEmpty(item.getVideoFileId())) {
                    firstNonEmpty = item.getVideoFileId();
                }
                if (!isNullOrEmpty(url) && url.equals(item.getFileUrl()) && !isNullOrEmpty(item.getVideoFileId())) {
                    return item.getVideoFileId();
                }
            }
            if (!isNullOrEmpty(firstNonEmpty)) {
