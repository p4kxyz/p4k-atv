...
import com.files.codes.utils.VideoTokenGenerator;
...
public class PlayerActivity extends Activity {
    ...
    private static final String SUBTITLE_WORKER_BASE_URL = "https://hehe.phim4k.lol";
        private static final String PREF_BILINGUAL_ENABLED = "bilingual_enabled";
        ...
        private static final int[] SUBTITLE_COLOR_VALUES = {Color.WHITE, Color.YELLOW, Color.RED, Color.GREEN,
            Color.BLUE, 0xFFFF8C00, 0xFFFFC0CB, Color.CYAN};
    ...
    private int visible;
    private ImageButton serverButton, subtitleButton, subtitleSettingsButton, secondarySubtitleSettingsButton, audioTrackButton, aspectRatioButton;
    ...
    private boolean isHoldingTimeBarSeekKey = false;
    private int activeTimeBarSeekKeyCode = KeyEvent.KEYCODE_UNKNOWN;
    private boolean safeSeekInProgress = false;
    private long pendingSeekDeltaMs = 0L;
    private boolean holdSeekPausedForStability = false;
    ...
    private final Runnable applyPendingSeekRunnable = new Runnable() {
        @Override
        public void run() {
            applyPendingSeek();
        }
    ...
    private boolean bilingualSubtitleEnabled = false;
    private String selectedSecondarySubtitleUrl = "";
    private String loadedSecondarySubtitleUrl = "";
    private final List<SubtitleCueEntry> secondarySubtitleEntries = new ArrayList<>();
    private final Handler bilingualSubtitleHandler = new Handler();
    private int currentSecondarySubtitleIndex = -1;
    private final Runnable secondarySubtitleTickRunnable = new Runnable() {
        @Override
        public void run() {
            updateSecondarySubtitleLine();
            if (bilingualSubtitleEnabled) {
                bilingualSubtitleHandler.postDelayed(this, SECONDARY_SUBTITLE_TICK_MS);
            }
        }
    };
    private final Player.Listener roundedSubtitleCueListener = new Player.Listener() {
        ...
        public void onCues(List<Cue> cues) {
            renderSubtitleCuesWithBackgroundMode(cues);
        }

        @Override
        public void onCues(CueGroup cueGroup) {
            renderSubtitleCuesWithBackgroundMode(cueGroup != null ? cueGroup.cues : null);
        }
    };
    ...
    private final Runnable timeBarFocusLockRunnable = new Runnable() {
        @Override
        public void run() {
            if (!isHoldingTimeBarSeekKey || exoPlayerView == null || !exoPlayerView.isControllerVisible()) {
                return;
            }
            ...
            timeBarFocusLockHandler.postDelayed(this, 60);
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        intiViews();
        VideoTokenGenerator.resolveStreamUrlAsync(url, new VideoTokenGenerator.StreamUrlCallback() {
            @Override
            public void onSuccess(String streamUrl) {
                runOnUiThread(() -> {
                    initVideoPlayer(streamUrl, videoType);
                });
            }
            ...
            public void onError(Exception e) {
                runOnUiThread(() -> {
                    Toast.makeText(PlayerActivity.this, "Network Error: " + e.getMessage(), Toast.LENGTH_SHORT).show();
    ...
    private void intiViews() {
        ...
        secondarySubtitleSettingsButton = findViewById(R.id.img_subtitle_secondary_settings);
        ...
        Log.d(TAG, "🎬 Secondary subtitle settings button setup - secondarySubtitleSettingsButton: " + (secondarySubtitleSettingsButton != null ? "FOUND ✅" : "NULL ❌"));
    ...
    protected void onStart() {
        ...
        if (secondarySubtitleSettingsButton != null) {
            secondarySubtitleSettingsButton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    openSecondarySubtitleSettingsDialog();
                }
            });
        }
            ...
            if (timeBar != null && timeBar.hasFocus()) {
                ...
                if (action == KeyEvent.ACTION_DOWN) {
                    if (keyCode == KeyEvent.KEYCODE_DPAD_RIGHT) {
                        seekForward(30000);
                        isHoldingTimeBarSeekKey = true;
                        activeTimeBarSeekKeyCode = KeyEvent.KEYCODE_DPAD_RIGHT;
                        startTimeBarFocusLock();
                        timeBar.requestFocus();
                        return true;
                    } else if (keyCode == KeyEvent.KEYCODE_DPAD_LEFT) {
                        seekBackward(30000);
                        isHoldingTimeBarSeekKey = true;
                        activeTimeBarSeekKeyCode = KeyEvent.KEYCODE_DPAD_LEFT;
                        startTimeBarFocusLock();
                        timeBar.requestFocus();
                        return true;
                    } else if ((keyCode == KeyEvent.KEYCODE_DPAD_DOWN || keyCode == KeyEvent.KEYCODE_DPAD_UP)
                            && isHoldingTimeBarSeekKey) {
                        // Block vertical navigation only while user is holding seek key
                        return true;
                    }
                } else if (action == KeyEvent.ACTION_UP) {
                    if (keyCode == activeTimeBarSeekKeyCode) {
                        isHoldingTimeBarSeekKey = false;
                        activeTimeBarSeekKeyCode = KeyEvent.KEYCODE_UNKNOWN;
                        stopTimeBarFocusLock();
                        resumeAfterHoldSeekIfNeeded();
                        return true;
                    }
                }
            }
...
//                }
    ...
    private void launchExternalPlayer() {
        try {
            ...
            resolveAndLaunchExternalPlayer(selectedPlayer, videoUrl);
    ...
    private void resolveAndLaunchExternalPlayer(String selectedPlayer, String videoUrl) {
        VideoTokenGenerator.resolveStreamUrlAsync(videoUrl, new VideoTokenGenerator.StreamUrlCallback() {
            @Override
            public void onSuccess(String streamUrl) {
                runOnUiThread(() -> {
                    String resolvedUrl = cleanVideoUrl(streamUrl);
                    model.setVideoUrl(resolvedUrl);
                    Log.d(TAG, "🔐 External player using resolved URL: " + resolvedUrl);
                    launchSelectedExternalPlayer(selectedPlayer, resolvedUrl);
                });
            }
            ...
            public void onError(Exception e) {
                runOnUiThread(() -> {
                    Log.e(TAG, "⚠️ Failed to resolve secure URL for external player, using fallback", e);
                    String fallbackUrl = cleanVideoUrl(videoUrl);
                    model.setVideoUrl(fallbackUrl);
                    launchSelectedExternalPlayer(selectedPlayer, fallbackUrl);
                });
            }
        });
    }

    private void launchSelectedExternalPlayer(String selectedPlayer, String videoUrl) {
    ...
    private void continueExternalPlayerLaunch() {
        ...
        resolveAndLaunchExternalPlayer(selectedPlayer, cleanVideoUrl(videoUrl));
    }
    ...
    public void initVideoPlayer(String url, String type) {
        ...
        // Optimize LoadControl for faster seeking and lower RAM usage
        // DEFAULT_MAX_BUFFER_MS = 50s — too large for 4K (uses ~125MB RAM)
        // Reduced to 30s max which is plenty for stable TV streaming
        DefaultLoadControl loadControl = new DefaultLoadControl.Builder()
                .setBufferDurationsMs(
                        15_000,  // minBufferMs: 15s (was DEFAULT = 50s)
                        30_000,  // maxBufferMs: 30s (was DEFAULT = 50s) — saves ~80MB RAM on 4K
                        1500,    // bufferForPlaybackMs: fast start
        ...
        attachRoundedSubtitleCueRenderer();
            ...
            if (timeBar != null && timeBar instanceof com.google.android.exoplayer2.ui.DefaultTimeBar) {
                ...
                timeBar.setOnFocusChangeListener(new View.OnFocusChangeListener() {
                    ...
                    public void onFocusChange(View v, boolean hasFocus) {
                        ...
                        } else if (isHoldingTimeBarSeekKey) {
                            // If focus slips while user is holding seek key, immediately pull it back.
                            v.post(v::requestFocus);
                        }
                    }
                });

                timeBar.setOnKeyListener((v, keyCode, event) -> {
    ...
    private void startTimeBarFocusLock() {
        timeBarFocusLockHandler.removeCallbacks(timeBarFocusLockRunnable);
        timeBarFocusLockHandler.post(timeBarFocusLockRunnable);
    }

    private void stopTimeBarFocusLock() {
        timeBarFocusLockHandler.removeCallbacks(timeBarFocusLockRunnable);
    }
    ...
    private void enqueueSeekDelta(long deltaMs) {
        ...
        // While key is held, new deltas are appended and consumed in the next batch.
    ...
    private void applyPendingSeek() {
        ...
        long deltaMs = pendingSeekDeltaMs;
        pendingSeekDeltaMs = 0L;

        if (deltaMs == 0L) {
            safeSeekHandler.postDelayed(clearSeekingFlagRunnable, 300);
            return;
        }
        ...
        if (duration == C.TIME_UNSET) {
            targetPosition = Math.max(currentPosition + deltaMs, 0);
        } else {
            targetPosition = Math.max(0, Math.min(currentPosition + deltaMs, duration));
        }

        final boolean isHoldSeekSnapshot = isHoldingTimeBarSeekKey;
        final boolean wasPlaying = player.getPlayWhenReady();
        safeSeekInProgress = true;

        if (isHoldSeekSnapshot) {
            if (!holdSeekPausedForStability && wasPlaying) {
                player.setPlayWhenReady(false);
                holdSeekPausedForStability = true;
                holdSeekResumePending = true;
            }
        } else if (wasPlaying) {
            player.setPlayWhenReady(false);
        }
        ...
        safeSeekHandler.postDelayed(() -> {
            if (player != null) {
                player.seekTo(finalTargetPosition);
                if (!isHoldSeekSnapshot && wasPlaying) {
                    player.setPlayWhenReady(true);
                }
                Log.i("PlayerActivity", "🔄 Safe seek applied to: " + (finalTargetPosition / 1000) + "s (delta=" + (deltaMs / 1000) + "s)");
            }

            safeSeekInProgress = false;
            if (pendingSeekDeltaMs != 0L) {
                safeSeekHandler.postDelayed(applyPendingSeekRunnable, SEEK_BATCH_INTERVAL_MS);
            } else {
                safeSeekHandler.postDelayed(clearSeekingFlagRunnable, 350);
            }
        }, SEEK_DECODER_SETTLE_MS);
    }

    private void resumeAfterHoldSeekIfNeeded() {
        if (!holdSeekResumePending) {
            return;
        }
        ...
        holdSeekResumePending = false;
        holdSeekPausedForStability = false;
    }
   ...
   /* @SuppressLint("StaticFieldLeak")
    ...
    private void releasePlayer() {
        ...
        if (player != null) {
            if (roundedSubtitleCueRendererAttached) {
                player.removeListener(roundedSubtitleCueListener);
                roundedSubtitleCueRendererAttached = false;
            }
            player.setPlayWhenReady(false);
            player.stop();
            player.release();
    ...
    private void showAudioTrackSelectionDialog() {
        ...
        try {
            ...
            for (int i = 0; i < selectionPairs.size(); i++) {
                ...
                if (override != null) {
                    if (override.groupIndex == groupIndex && override.containsTrack(trackIndex)) {
                        checkedIndex = i;
                        break;
                    }
                    continue;
                ...
                if (currentSelections != null && rendererIndex < currentSelections.length) {
                    TrackSelection selected = currentSelections.get(rendererIndex);
                    if (selected == null) {
                        continue;
                    }
                    TrackGroup selectedGroup = selected.getTrackGroup();
                    TrackGroup candidateGroup = groups.get(groupIndex);
                    if (selectedGroup == candidateGroup && selected.indexOf(trackIndex) != C.INDEX_UNSET) {
                        checkedIndex = i;
                        break;
                    }
                }
            ...
            Log.w("PlayerActivity", "Could not resolve current audio track for checked state", e);
        }
    ...
    private void applySubtitleSettings() {
        ...
        int textColor = SUBTITLE_COLOR_VALUES[textColorIndex];
        int outlineColor;

        if (outlineThicknessPt < 0) {
            // Migrate legacy 4-level value to pt-based value.
            int legacyThickness = prefs.getInt("outline_thickness", 2);
            outlineThicknessPt = Math.max(0, Math.min(8, legacyThickness));
            prefs.edit().putInt("outline_thickness_pt", outlineThicknessPt).apply();
        }
        if (outlineThicknessPt < 0 || outlineThicknessPt > 8) {
            outlineThicknessPt = 2;
        ...
        if (subtitleView != null) {
            CaptionStyleCompat style = new CaptionStyleCompat(
                    ...
                    roundedSubtitleBackgroundEnabled ? Color.TRANSPARENT : (background ? Color.BLACK : Color.TRANSPARENT),
            ...
            if (player != null) {
                try {
                    renderSubtitleCuesWithBackgroundMode(player.getCurrentCues().cues);
                } catch (Exception ignored) {
                    // Some streams may not provide cues immediately.
                }
            }
            ...
            Log.d(TAG, "Applied subtitle - colors: text=" + Integer.toHexString(textColor) + 
                      ...
                      ", thicknessPt=" + outlineThicknessPt +
        ...
        bilingualSubtitleEnabled = prefs.getBoolean(PREF_BILINGUAL_ENABLED, false);
        selectedSecondarySubtitleUrl = prefs.getString(PREF_SECONDARY_SUBTITLE_URL, "");
        applySecondarySubtitleStyle(prefs);
        if (bilingualSubtitleEnabled && !isNullOrEmpty(selectedSecondarySubtitleUrl)) {
            ensureSecondarySubtitleLoaded(false);
        } else {
            clearSecondarySubtitleDisplay();
            stopSecondarySubtitleTicker();
        }
    ...
    private void renderSubtitleCuesWithBackgroundMode(List<Cue> cues) {
        if (subtitleView == null) {
            return;
        }
        subtitleView.setCues(createStyledSubtitleCues(cues));
    }

    private void ensureSecondarySubtitleView() {
        if (secondarySubtitleView != null || rootLayout == null) {
            return;
        }
        ...
        params.setMargins(dp(28), dp(8), dp(28), dp(96));
        rootLayout.addView(secondarySubtitleView, params);
    }

    private void applySecondarySubtitleStyle(SharedPreferences prefs) {
        ensureSecondarySubtitleView();
        ...
        if (secondaryColorIndex < 0 || secondaryColorIndex >= SUBTITLE_COLOR_VALUES.length) {
            ...
            prefs.edit().putInt(PREF_SECONDARY_TEXT_COLOR, secondaryColorIndex).apply();
        }
        ...
        int primaryOffset = prefs.getInt("vertical_offset", 0);
        float combinedFraction = 0.04f + ((primaryOffset + secondaryOffset) * 0.01f);
        combinedFraction = Math.max(0.01f, Math.min(0.85f, combinedFraction));
        int bottomMargin = (int) (getResources().getDisplayMetrics().heightPixels * combinedFraction);
    ...
    private void ensureSecondarySubtitleLoaded(boolean showLoadingToast) {
        ...
        String normalizedUrl = normalizeWorkerSubtitleUrl(selectedSecondarySubtitleUrl);
        if (isNullOrEmpty(normalizedUrl)) {
            clearSecondarySubtitleDisplay();
            stopSecondarySubtitleTicker();
            return;
        }
        selectedSecondarySubtitleUrl = normalizedUrl;

        if (normalizedUrl.equals(loadedSecondarySubtitleUrl) && !secondarySubtitleEntries.isEmpty()) {
            startSecondarySubtitleTicker();
            return;
        }

        if (showLoadingToast) {
            new ToastMsg(this).toastIconSuccess("Đang tải phụ đề thứ 2...");
        }

        new Thread(() -> {
            String content = downloadTextFromUrl(normalizedUrl);
            List<SubtitleCueEntry> parsed = parseSrtContent(content);

            runOnUiThread(() -> {
                if (!bilingualSubtitleEnabled || !normalizedUrl.equals(selectedSecondarySubtitleUrl)) {
                    return;
                }

                secondarySubtitleEntries.clear();
                secondarySubtitleEntries.addAll(parsed);
                loadedSecondarySubtitleUrl = normalizedUrl;
                currentSecondarySubtitleIndex = -1;

                if (secondarySubtitleEntries.isEmpty()) {
                    clearSecondarySubtitleDisplay();
                    stopSecondarySubtitleTicker();
                    if (showLoadingToast) {
                        new ToastMsg(this).toastIconError("Phụ đề thứ 2 rỗng hoặc không hợp lệ");
                    }
                } else {
                    startSecondarySubtitleTicker();
                    if (showLoadingToast) {
                        new ToastMsg(this).toastIconSuccess("Đã bật phụ đề song ngữ");
                    }
                }
            });
        }).start();
    ...
    private void startSecondarySubtitleTicker() {
        if (!bilingualSubtitleEnabled || secondarySubtitleEntries.isEmpty()) {
            return;
        }
        bilingualSubtitleHandler.removeCallbacks(secondarySubtitleTickRunnable);
        bilingualSubtitleHandler.post(secondarySubtitleTickRunnable);
    }

    private void stopSecondarySubtitleTicker() {
        bilingualSubtitleHandler.removeCallbacks(secondarySubtitleTickRunnable);
    }

    private void updateSecondarySubtitleLine() {
        ensureSecondarySubtitleView();
        if (secondarySubtitleView == null) {
            return;
        }

        if (!bilingualSubtitleEnabled || player == null || secondarySubtitleEntries.isEmpty()) {
            clearSecondarySubtitleDisplay();
            return;
        }

        long positionMs = Math.max(0L, player.getCurrentPosition());
        int nextIndex = findSecondaryCueIndex(positionMs, currentSecondarySubtitleIndex);
        currentSecondarySubtitleIndex = nextIndex;

        if (nextIndex < 0 || nextIndex >= secondarySubtitleEntries.size()) {
            secondarySubtitleView.setText("");
            secondarySubtitleView.setVisibility(View.GONE);
            return;
        }

        SubtitleCueEntry cue = secondarySubtitleEntries.get(nextIndex);
        if (cue == null || isNullOrEmpty(cue.text)) {
            secondarySubtitleView.setText("");
            secondarySubtitleView.setVisibility(View.GONE);
            return;
        }

        secondarySubtitleView.setText(cue.text);
        secondarySubtitleView.setVisibility(View.VISIBLE);
        secondarySubtitleView.bringToFront();
    }

    private int findSecondaryCueIndex(long positionMs, int lastIndex) {
        if (secondarySubtitleEntries.isEmpty()) {
            return -1;
        }

        if (lastIndex >= 0 && lastIndex < secondarySubtitleEntries.size()) {
            SubtitleCueEntry current = secondarySubtitleEntries.get(lastIndex);
            if (positionMs >= current.startMs && positionMs <= current.endMs) {
                return lastIndex;
            }
            if (lastIndex + 1 < secondarySubtitleEntries.size()) {
                SubtitleCueEntry next = secondarySubtitleEntries.get(lastIndex + 1);
        ...
        int low = 0;
        int high = secondarySubtitleEntries.size() - 1;
        while (low <= high) {
            int mid = (low + high) >>> 1;
            SubtitleCueEntry cue = secondarySubtitleEntries.get(mid);
    ...
    private void clearSecondarySubtitleDisplay() {
        ensureSecondarySubtitleView();
        if (secondarySubtitleView != null) {
            secondarySubtitleView.setText("");
            secondarySubtitleView.setVisibility(View.GONE);
    ...
    private String normalizeWorkerSubtitleUrl(String subtitleUrl) {
        if (isNullOrEmpty(subtitleUrl)) {
            return "";
        }
        String trimmed = subtitleUrl.trim();
        ...
        return SUBTITLE_WORKER_BASE_URL + trimmed;
    }

    private String downloadTextFromUrl(String subtitleUrl) {
        HttpURLConnection connection = null;
        try {
            URL target = new URL(subtitleUrl);
            ...
            Log.e(TAG, "Failed to download secondary subtitle", e);
            return "";
        } finally {
            if (connection != null) {
    ...
    private List<SubtitleCueEntry> parseSrtContent(String srtText) {
        List<SubtitleCueEntry> entries = new ArrayList<>();
        if (isNullOrEmpty(srtText)) {
            return entries;
        }
        ...
        Collections.sort(entries, Comparator.comparingLong(a -> a.startMs));
        return entries;
    }

    private void parseSrtBlock(List<String> block, List<SubtitleCueEntry> out) {
        ...
        String[] timeParts = block.get(timeLineIndex).split("-->");
        if (timeParts.length < 2) {
            return;
        }

        long startMs = parseSrtTimeToMs(timeParts[0].trim());
        long endMs = parseSrtTimeToMs(timeParts[1].trim());
        if (startMs < 0 || endMs < startMs) {
            return;
        }

        StringBuilder textBuilder = new StringBuilder();
        for (int i = timeLineIndex + 1; i < block.size(); i++) {
            if (textBuilder.length() > 0) {
                textBuilder.append('\n');
            }
            textBuilder.append(block.get(i));
        }

        String plain = htmlToPlainText(textBuilder.toString()).trim();
        if (plain.isEmpty()) {
            return;
        }

        out.add(new SubtitleCueEntry(startMs, endMs, plain));
    }

    private long parseSrtTimeToMs(String timeText) {
        if (isNullOrEmpty(timeText)) {
            return -1L;
        }

        String[] parts = timeText.replace(',', ':').split(":");
        if (parts.length < 4) {
            return -1L;
        }
    ...
    private void attachRoundedSubtitleCueRenderer() {
        if (player == null || roundedSubtitleCueRendererAttached) {
            return;
        }
        player.addListener(roundedSubtitleCueListener);
        roundedSubtitleCueRendererAttached = true;
    }

    private List<Cue> createStyledSubtitleCues(List<Cue> cues) {
        if (cues == null || cues.isEmpty()) {
            return cues;
        }

        List<Cue> styledCues = new ArrayList<>(cues.size());
        for (Cue cue : cues) {
    ...
    private CharSequence applyStyledSubtitleToText(CharSequence text) {
        ...
        for (int i = 0; i <= textLength; i++) {
            boolean isLineBreak = i < textLength && rawText.charAt(i) == '\n';
            boolean isLastChar = i == textLength;
            if (!isLineBreak && !isLastChar) {
                continue;
            }

            if (i > lineStart) {
                boolean drawBackground = roundedSubtitleBackgroundEnabled;
                spannable.setSpan(
                    new RoundedBackgroundSpan(
                        drawBackground ? roundedSubtitleBackgroundColor : Color.TRANSPARENT,
                        drawBackground ? dp(8) : 0f,
                        drawBackground ? dp(6) : 0f,
                        drawBackground ? dp(2) : 0f,
                        subtitleStrokeTextColor,
                        subtitleStrokeOutlineColor,
                        subtitleStrokeWidthPx
                    ),
                        lineStart,
                        i,
    ...
    private static final class RoundedBackgroundSpan extends ReplacementSpan {
        ...
        public int getSize(Paint paint, CharSequence text, int start, int end, Paint.FontMetricsInt fm) {
            return (int) Math.ceil(paint.measureText(text, start, end) + (horizontalPaddingPx * 2f));
        }
        ...
        public void draw(Canvas canvas, CharSequence text, int start, int end, float x,
            ...
            float rectTop = top + verticalPaddingPx;
            float rectBottom = bottom - verticalPaddingPx;
            rect.set(x, rectTop, x + textWidth + (horizontalPaddingPx * 2f), rectBottom);
            ...
            float previousMiter = paint.getStrokeMiter();
            ...
            float textX = x + horizontalPaddingPx;
    ...
    */
    ...
    private void openUnifiedSettingsDialog() {
        ...
        fontSizeMinusBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable fontSizeMinusBtnBg = new android.graphics.drawable.GradientDrawable();
        fontSizeMinusBtnBg.setColor(0xFF3A3A4C); fontSizeMinusBtnBg.setCornerRadius(dp(8)); fontSizeMinusBtn.setBackground(fontSizeMinusBtnBg);
        Button fontSizePlusBtn = new Button(this);
        fontSizePlusBtn.setText("+");
        fontSizePlusBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable fontSizePlusBtnBg = new android.graphics.drawable.GradientDrawable();
        fontSizePlusBtnBg.setColor(0xFF3A3A4C); fontSizePlusBtnBg.setCornerRadius(dp(8)); fontSizePlusBtn.setBackground(fontSizePlusBtnBg);
        TextView fontSizeTV = new TextView(this);
        fontSizeTV.setTextColor(0xFFFFFFFF);
        ...
        fontTypeLabel.setTextColor(0xFFFFFFFF);
        ...
        fontTypePrevBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable fontTypePrevBtnBg = new android.graphics.drawable.GradientDrawable();
        fontTypePrevBtnBg.setColor(0xFF3A3A4C); fontTypePrevBtnBg.setCornerRadius(dp(8)); fontTypePrevBtn.setBackground(fontTypePrevBtnBg);
        Button fontTypeNextBtn = new Button(this);
        fontTypeNextBtn.setText("▶");
        fontTypeNextBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable fontTypeNextBtnBg = new android.graphics.drawable.GradientDrawable();
        fontTypeNextBtnBg.setColor(0xFF3A3A4C); fontTypeNextBtnBg.setCornerRadius(dp(8)); fontTypeNextBtn.setBackground(fontTypeNextBtnBg);
        ...
        positionLabel.setTextColor(0xFFFFFFFF);
        ...
        positionUpBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable positionUpBtnBg = new android.graphics.drawable.GradientDrawable();
        positionUpBtnBg.setColor(0xFF3A3A4C); positionUpBtnBg.setCornerRadius(dp(8)); positionUpBtn.setBackground(positionUpBtnBg);
        Button positionDownBtn = new Button(this);
        positionDownBtn.setText("Xuống");
        positionDownBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable positionDownBtnBg = new android.graphics.drawable.GradientDrawable();
        positionDownBtnBg.setColor(0xFF3A3A4C); positionDownBtnBg.setCornerRadius(dp(8)); positionDownBtn.setBackground(positionDownBtnBg);
        TextView positionTV = new TextView(this);
        positionTV.setTextColor(0xFFFFFFFF);
        ...
        // Bilingual subtitle controls
        LinearLayout bilingualLayout = new LinearLayout(this);
        bilingualLayout.setOrientation(LinearLayout.HORIZONTAL);
        bilingualLayout.setPadding(0, 20, 0, 10);

        TextView bilingualLabel = new TextView(this);
        bilingualLabel.setTextColor(0xFFFFFFFF);
        bilingualLabel.setText("Song ngữ: ");
        bilingualLabel.setTextSize(16);

        Switch bilingualSwitch = new Switch(this);
        bilingualSwitch.setTextColor(0xFFFFFFFF);
        bilingualSwitch.setChecked(prefs.getBoolean(PREF_BILINGUAL_ENABLED, false));

        bilingualLayout.addView(bilingualLabel);
        bilingualLayout.addView(bilingualSwitch);

        TextView secondarySubtitleLabel = new TextView(this);
        secondarySubtitleLabel.setTextColor(0xFFFFFFFF);
        secondarySubtitleLabel.setText("Phụ đề thứ 2:");
        secondarySubtitleLabel.setTextSize(16);
        secondarySubtitleLabel.setPadding(0, 10, 0, 8);

        LinearLayout secondarySubtitlePickerLayout = new LinearLayout(this);
        secondarySubtitlePickerLayout.setOrientation(LinearLayout.HORIZONTAL);
        secondarySubtitlePickerLayout.setGravity(Gravity.CENTER_VERTICAL);

        Button chooseSecondarySubtitleBtn = new Button(this);
        chooseSecondarySubtitleBtn.setText("Chọn");
        chooseSecondarySubtitleBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable chooseSecondarySubtitleBg = new android.graphics.drawable.GradientDrawable();
        chooseSecondarySubtitleBg.setColor(0xFF3A3A4C);
        chooseSecondarySubtitleBg.setCornerRadius(dp(8));
        chooseSecondarySubtitleBtn.setBackground(chooseSecondarySubtitleBg);

        TextView secondarySubtitleTV = new TextView(this);
        secondarySubtitleTV.setTextColor(0xFFFFFFFF);
        String selectedSecondaryLabel = prefs.getString(PREF_SECONDARY_SUBTITLE_LABEL, "Chưa chọn");
        secondarySubtitleTV.setText(isNullOrEmpty(selectedSecondaryLabel) ? "Chưa chọn" : selectedSecondaryLabel);
        secondarySubtitleTV.setPadding(dp(12), 0, 0, 0);

        secondarySubtitlePickerLayout.addView(chooseSecondarySubtitleBtn);
        secondarySubtitlePickerLayout.addView(secondarySubtitleTV);

        TextView secondaryFontSizeLabel = new TextView(this);
        secondaryFontSizeLabel.setTextColor(0xFFFFFFFF);
        secondaryFontSizeLabel.setText("Cỡ chữ phụ đề 2:");
        secondaryFontSizeLabel.setTextSize(16);
        secondaryFontSizeLabel.setPadding(0, 20, 0, 10);

        LinearLayout secondaryFontSizeLayout = new LinearLayout(this);
        secondaryFontSizeLayout.setOrientation(LinearLayout.HORIZONTAL);

        Button secondaryFontSizeMinusBtn = new Button(this);
        secondaryFontSizeMinusBtn.setText("-");
        secondaryFontSizeMinusBtn.setTextColor(0xFFFFFFFF);
        secondaryFontSizeMinusBtn.setBackground(createDialogButtonBackground(0xFF3A3A4C));

        Button secondaryFontSizePlusBtn = new Button(this);
        secondaryFontSizePlusBtn.setText("+");
        secondaryFontSizePlusBtn.setTextColor(0xFFFFFFFF);
        secondaryFontSizePlusBtn.setBackground(createDialogButtonBackground(0xFF3A3A4C));

        TextView secondaryFontSizeTV = new TextView(this);
        secondaryFontSizeTV.setTextColor(0xFFFFFFFF);
        secondaryFontSizeTV.setGravity(Gravity.CENTER);
        secondaryFontSizeTV.setPadding(20, 0, 20, 0);
        secondaryFontSizeTV.setText(prefs.getInt(PREF_SECONDARY_FONT_SIZE, 18) + "sp");

        secondaryFontSizeLayout.addView(secondaryFontSizeMinusBtn);
        secondaryFontSizeLayout.addView(secondaryFontSizeTV);
        secondaryFontSizeLayout.addView(secondaryFontSizePlusBtn);

        TextView secondaryTextColorLabel = new TextView(this);
        secondaryTextColorLabel.setTextColor(0xFFFFFFFF);
        secondaryTextColorLabel.setText("Màu chữ phụ đề 2:");
        secondaryTextColorLabel.setTextSize(16);
        secondaryTextColorLabel.setPadding(0, 20, 0, 10);

        LinearLayout secondaryTextColorLayout = new LinearLayout(this);
        secondaryTextColorLayout.setOrientation(LinearLayout.HORIZONTAL);

        Button secondaryColorPrevBtn = new Button(this);
        secondaryColorPrevBtn.setText("◀");
        secondaryColorPrevBtn.setTextColor(0xFFFFFFFF);
        secondaryColorPrevBtn.setBackground(createDialogButtonBackground(0xFF3A3A4C));

        Button secondaryColorNextBtn = new Button(this);
        secondaryColorNextBtn.setText("▶");
        secondaryColorNextBtn.setTextColor(0xFFFFFFFF);
        secondaryColorNextBtn.setBackground(createDialogButtonBackground(0xFF3A3A4C));

        TextView secondaryColorTV = new TextView(this);
        int currentSecondaryColor = prefs.getInt(PREF_SECONDARY_TEXT_COLOR, 1);
        if (currentSecondaryColor < 0 || currentSecondaryColor >= SUBTITLE_COLOR_NAMES.length) {
            currentSecondaryColor = 1;
            prefs.edit().putInt(PREF_SECONDARY_TEXT_COLOR, currentSecondaryColor).apply();
        }
        secondaryColorTV.setText(SUBTITLE_COLOR_NAMES[currentSecondaryColor]);
        secondaryColorTV.setTextColor(SUBTITLE_COLOR_VALUES[currentSecondaryColor]);
        secondaryColorTV.setGravity(Gravity.CENTER);
        secondaryColorTV.setPadding(20, 0, 20, 0);

        secondaryTextColorLayout.addView(secondaryColorPrevBtn);
        secondaryTextColorLayout.addView(secondaryColorTV);
        secondaryTextColorLayout.addView(secondaryColorNextBtn);

        TextView secondaryPositionLabel = new TextView(this);
        secondaryPositionLabel.setTextColor(0xFFFFFFFF);
        secondaryPositionLabel.setText("Vị trí phụ đề 2:");
        secondaryPositionLabel.setTextSize(16);
        secondaryPositionLabel.setPadding(0, 20, 0, 10);

        LinearLayout secondaryPositionLayout = new LinearLayout(this);
        secondaryPositionLayout.setOrientation(LinearLayout.HORIZONTAL);

        Button secondaryPositionUpBtn = new Button(this);
        secondaryPositionUpBtn.setText("Lên");
        secondaryPositionUpBtn.setTextColor(0xFFFFFFFF);
        secondaryPositionUpBtn.setBackground(createDialogButtonBackground(0xFF3A3A4C));

        Button secondaryPositionDownBtn = new Button(this);
        secondaryPositionDownBtn.setText("Xuống");
        secondaryPositionDownBtn.setTextColor(0xFFFFFFFF);
        secondaryPositionDownBtn.setBackground(createDialogButtonBackground(0xFF3A3A4C));

        TextView secondaryPositionTV = new TextView(this);
        int currentSecondaryOffset = prefs.getInt(PREF_SECONDARY_VERTICAL_OFFSET, -8);
        secondaryPositionTV.setText((currentSecondaryOffset > 0 ? "+" : "") + currentSecondaryOffset + "%");
        secondaryPositionTV.setTextColor(0xFFFFFFFF);
        secondaryPositionTV.setGravity(Gravity.CENTER);
        secondaryPositionTV.setPadding(20, 0, 20, 0);

        secondaryPositionLayout.addView(secondaryPositionUpBtn);
        ...
        textColorLabel.setTextColor(0xFFFFFFFF);
        ...
        textColorPrevBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable textColorPrevBtnBg = new android.graphics.drawable.GradientDrawable();
        textColorPrevBtnBg.setColor(0xFF3A3A4C); textColorPrevBtnBg.setCornerRadius(dp(8)); textColorPrevBtn.setBackground(textColorPrevBtnBg);
        Button textColorNextBtn = new Button(this);
        textColorNextBtn.setText("▶");
        textColorNextBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable textColorNextBtnBg = new android.graphics.drawable.GradientDrawable();
        textColorNextBtnBg.setColor(0xFF3A3A4C); textColorNextBtnBg.setCornerRadius(dp(8)); textColorNextBtn.setBackground(textColorNextBtnBg);
        TextView textColorTV = new TextView(this);
        textColorTV.setTextColor(0xFFFFFFFF);
        
        int currentTextColor = prefs.getInt("text_color", 0);
        // Bounds checking to prevent crash
        if (currentTextColor >= SUBTITLE_COLOR_NAMES.length) {
            currentTextColor = 0; // Default to White
            prefs.edit().putInt("text_color", currentTextColor).apply();
        }
        textColorTV.setText(SUBTITLE_COLOR_NAMES[currentTextColor]);
        textColorTV.setTextColor(SUBTITLE_COLOR_VALUES[currentTextColor]);
        ...
        outlineColorLabel.setTextColor(0xFFFFFFFF);
        ...
        outlineColorPrevBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable outlineColorPrevBtnBg = new android.graphics.drawable.GradientDrawable();
        outlineColorPrevBtnBg.setColor(0xFF3A3A4C); outlineColorPrevBtnBg.setCornerRadius(dp(8)); outlineColorPrevBtn.setBackground(outlineColorPrevBtnBg);
        Button outlineColorNextBtn = new Button(this);
        outlineColorNextBtn.setText("▶");
        outlineColorNextBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable outlineColorNextBtnBg = new android.graphics.drawable.GradientDrawable();
        outlineColorNextBtnBg.setColor(0xFF3A3A4C); outlineColorNextBtnBg.setCornerRadius(dp(8)); outlineColorNextBtn.setBackground(outlineColorNextBtnBg);
        TextView outlineColorTV = new TextView(this);
        outlineColorTV.setTextColor(0xFFFFFFFF);
        ...
        TextView outlineThicknessLabel = new TextView(this);
        outlineThicknessLabel.setTextColor(0xFFFFFFFF);
        outlineThicknessLabel.setText("Độ dày viền:");
        outlineThicknessLabel.setTextSize(16);
        outlineThicknessLabel.setPadding(0, 20, 0, 10);

        LinearLayout outlineThicknessLayout = new LinearLayout(this);
        outlineThicknessLayout.setOrientation(LinearLayout.HORIZONTAL);

        Button outlineThicknessPrevBtn = new Button(this);
        outlineThicknessPrevBtn.setText("◀");
        outlineThicknessPrevBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable outlineThicknessPrevBtnBg = new android.graphics.drawable.GradientDrawable();
        outlineThicknessPrevBtnBg.setColor(0xFF3A3A4C); outlineThicknessPrevBtnBg.setCornerRadius(dp(8)); outlineThicknessPrevBtn.setBackground(outlineThicknessPrevBtnBg);

        Button outlineThicknessNextBtn = new Button(this);
        outlineThicknessNextBtn.setText("▶");
        outlineThicknessNextBtn.setTextColor(0xFFFFFFFF);
        android.graphics.drawable.GradientDrawable outlineThicknessNextBtnBg = new android.graphics.drawable.GradientDrawable();
        outlineThicknessNextBtnBg.setColor(0xFF3A3A4C); outlineThicknessNextBtnBg.setCornerRadius(dp(8)); outlineThicknessNextBtn.setBackground(outlineThicknessNextBtnBg);

        TextView outlineThicknessTV = new TextView(this);
        outlineThicknessTV.setTextColor(0xFFFFFFFF);
        int currentOutlineThicknessPt = prefs.getInt("outline_thickness_pt", -1);
        if (currentOutlineThicknessPt < 0) {
            int legacyThickness = prefs.getInt("outline_thickness", 2);
            currentOutlineThicknessPt = Math.max(0, Math.min(8, legacyThickness));
            prefs.edit().putInt("outline_thickness_pt", currentOutlineThicknessPt).apply();
        }
        if (currentOutlineThicknessPt < 0 || currentOutlineThicknessPt > 8) {
            currentOutlineThicknessPt = 2;
            prefs.edit().putInt("outline_thickness_pt", currentOutlineThicknessPt).apply();
        }
        outlineThicknessTV.setText(currentOutlineThicknessPt + "pt");
        outlineThicknessTV.setGravity(android.view.Gravity.CENTER);
        outlineThicknessTV.setPadding(20, 0, 20, 0);

        outlineThicknessLayout.addView(outlineThicknessPrevBtn);
        outlineThicknessLayout.addView(outlineThicknessTV);
        outlineThicknessLayout.addView(outlineThicknessNextBtn);
        
        // Playback Speed controls
        TextView playbackSpeedLabel = new TextView(this);
        playbackSpeedLabel.setTextColor(0xFFFFFFFF);
        ...
        // Add clear TV focus ring for subtitle settings controls.
        applyWhiteFocusBorder(fontSizeMinusBtn);
        applyWhiteFocusBorder(fontSizePlusBtn);
        applyWhiteFocusBorder(fontTypePrevBtn);
        ...
        applyWhiteFocusBorder(outlineThicknessPrevBtn);
        applyWhiteFocusBorder(outlineThicknessNextBtn);
        ...
        layout.addView(outlineThicknessLabel);
        layout.addView(outlineThicknessLayout);
        ...
        Runnable updateSecondaryControlsState = () -> {
            boolean enabled = bilingualSwitch.isChecked();
        ...
        bilingualSwitch.setOnCheckedChangeListener((buttonView, isChecked) -> {
            saveAndApplySubtitleSetting(PREF_BILINGUAL_ENABLED, isChecked);
            updateSecondaryControlsState.run();
            if (isChecked && isNullOrEmpty(prefs.getString(PREF_SECONDARY_SUBTITLE_URL, ""))) {
                openSecondarySubtitleSelectionDialog(selectedLabel -> {
                    secondarySubtitleTV.setText(selectedLabel);
                });
            }
        });

        chooseSecondarySubtitleBtn.setOnClickListener(v -> {
            openSecondarySubtitleSelectionDialog(selectedLabel -> {
                secondarySubtitleTV.setText(selectedLabel);
            });
        });

        secondaryFontSizeMinusBtn.setOnClickListener(v -> {
            int currentSize = prefs.getInt(PREF_SECONDARY_FONT_SIZE, 18);
            if (currentSize > 12) {
                currentSize -= 2;
                secondaryFontSizeTV.setText(currentSize + "sp");
                saveAndApplySubtitleSetting(PREF_SECONDARY_FONT_SIZE, currentSize);
            }
        });

        secondaryFontSizePlusBtn.setOnClickListener(v -> {
            int currentSize = prefs.getInt(PREF_SECONDARY_FONT_SIZE, 18);
            if (currentSize < 40) {
                currentSize += 2;
                secondaryFontSizeTV.setText(currentSize + "sp");
        ...
        secondaryColorPrevBtn.setOnClickListener(v -> {
            ...
            secondaryColorTV.setText(SUBTITLE_COLOR_NAMES[index]);
            secondaryColorTV.setTextColor(SUBTITLE_COLOR_VALUES[index]);
            applySubtitleSettings();
        });

        secondaryColorNextBtn.setOnClickListener(v -> {
            int index = (prefs.getInt(PREF_SECONDARY_TEXT_COLOR, 1) + 1) % SUBTITLE_COLOR_NAMES.length;
            prefs.edit().putInt(PREF_SECONDARY_TEXT_COLOR, index).apply();
            secondaryColorTV.setText(SUBTITLE_COLOR_NAMES[index]);
            secondaryColorTV.setTextColor(SUBTITLE_COLOR_VALUES[index]);
            applySubtitleSettings();
        ...
        secondaryPositionUpBtn.setOnClickListener(v -> {
            int offset = prefs.getInt(PREF_SECONDARY_VERTICAL_OFFSET, -8);
            offset = Math.min(80, offset + 1);
            secondaryPositionTV.setText((offset > 0 ? "+" : "") + offset + "%");
            saveAndApplySubtitleSetting(PREF_SECONDARY_VERTICAL_OFFSET, offset);
        });

        secondaryPositionDownBtn.setOnClickListener(v -> {
            int offset = prefs.getInt(PREF_SECONDARY_VERTICAL_OFFSET, -8);
            offset = Math.max(-10, offset - 1);
            secondaryPositionTV.setText((offset > 0 ? "+" : "") + offset + "%");
            saveAndApplySubtitleSetting(PREF_SECONDARY_VERTICAL_OFFSET, offset);
        });
        ...
        textColorPrevBtn.setOnClickListener(v -> {
            int index = (prefs.getInt("text_color", 0) - 1 + SUBTITLE_COLOR_NAMES.length) % SUBTITLE_COLOR_NAMES.length;
            prefs.edit().putInt("text_color", index).apply();
            textColorTV.setText(SUBTITLE_COLOR_NAMES[index]);
            textColorTV.setTextColor(SUBTITLE_COLOR_VALUES[index]);
            applySubtitleSettings();
        });
        
        textColorNextBtn.setOnClickListener(v -> {
            int index = (prefs.getInt("text_color", 0) + 1) % SUBTITLE_COLOR_NAMES.length;
            prefs.edit().putInt("text_color", index).apply();
            textColorTV.setText(SUBTITLE_COLOR_NAMES[index]);
            textColorTV.setTextColor(SUBTITLE_COLOR_VALUES[index]);
            applySubtitleSettings();
        ...
        // Outline thickness controls
        outlineThicknessPrevBtn.setOnClickListener(v -> {
            int pt = prefs.getInt("outline_thickness_pt", -1);
            if (pt < 0) {
                pt = Math.max(0, Math.min(8, prefs.getInt("outline_thickness", 2)));
            }
            pt = Math.max(0, pt - 1);
            prefs.edit().putInt("outline_thickness_pt", pt).apply();
            outlineThicknessTV.setText(pt + "pt");
            applySubtitleSettings();
        });

        outlineThicknessNextBtn.setOnClickListener(v -> {
            int pt = prefs.getInt("outline_thickness_pt", -1);
            if (pt < 0) {
                pt = Math.max(0, Math.min(8, prefs.getInt("outline_thickness", 2)));
            }
        ...
        resetBtn.setOnClickListener(v -> {
            ...
            editor.putInt("outline_thickness", 2); // Keep legacy key for compatibility
            editor.putInt("outline_thickness_pt", 2);
            ...
            textColorTV.setText(SUBTITLE_COLOR_NAMES[0]);
            textColorTV.setTextColor(SUBTITLE_COLOR_VALUES[0]);
            outlineColorTV.setText(outlineColorNames[1]);
            outlineColorTV.setTextColor(outlineColorValues[1]); // Show black color for reset
            outlineThicknessTV.setText("2pt");
        ...
        applyWhiteFocusBorder(closeBtn);
    ...
    private void openSecondarySubtitleSettingsDialog() {
        android.app.Dialog dialog = new android.app.Dialog(this);
        dialog.requestWindowFeature(android.view.Window.FEATURE_NO_TITLE);
        dialog.setCancelable(true);
        ...
        titleView.setText("Cài đặt phụ đề thứ 2");
        titleView.setTextColor(0xFFFFFFFF);
        ...
        LinearLayout bilingualLayout = new LinearLayout(this);
        bilingualLayout.setOrientation(LinearLayout.HORIZONTAL);
        bilingualLayout.setPadding(0, 10, 0, 10);

        TextView bilingualLabel = new TextView(this);
        bilingualLabel.setTextColor(0xFFFFFFFF);
        bilingualLabel.setText("Bật song ngữ:");
        bilingualLabel.setTextSize(16);
        ...
        TextView secondarySubtitleLabel = new TextView(this);
        secondarySubtitleLabel.setTextColor(0xFFFFFFFF);
        secondarySubtitleLabel.setText("Chọn phụ đề thứ 2:");
        secondarySubtitleLabel.setTextSize(16);
        secondarySubtitleLabel.setPadding(0, 16, 0, 8);
        ...
        chooseSecondarySubtitleBtn.setText("Chọn");
        chooseSecondarySubtitleBtn.setTextColor(0xFFFFFFFF);
        chooseSecondarySubtitleBtn.setBackground(createDialogButtonBackground(0xFF3A3A4C));
        ...
        resetSecondaryBtn.setBackground(createDialogButtonBackground(0xFF4A4A5C));
        ...
        layout.addView(secondarySubtitleLabel);
        ...
        bilingualSwitch.setOnCheckedChangeListener((buttonView, isChecked) -> {
            ...
            updateSecondaryControlsState.run();
            if (isChecked && isNullOrEmpty(prefs.getString(PREF_SECONDARY_SUBTITLE_URL, ""))) {
                openSecondarySubtitleSelectionDialog(selectedLabel -> secondarySubtitleTV.setText(selectedLabel));
            }
        });
        ...
        closeBtn.setText("Đóng");
        closeBtn.setTextColor(0xFFFFFFFF);
        closeBtn.setBackground(createDialogButtonBackground(0xFF3A3A4C));
        applyWhiteFocusBorder(closeBtn);
        closeBtn.setOnClickListener(v -> dialog.dismiss());
    ...
    private void saveAndApplySubtitleSetting(String key, String value) {
        SharedPreferences.Editor editor = getSharedPreferences("subtitle_settings", MODE_PRIVATE).edit();
    ...
    private interface SecondarySubtitleSelectionListener {
        void onSelected(String selectedLabel);
    }

    private void openSecondarySubtitleSelectionDialog(SecondarySubtitleSelectionListener listener) {
        String fileId = resolveCurrentVideoFileId(model != null ? model.getMovieId() : null);
        if (isNullOrEmpty(fileId)) {
            new ToastMsg(this).toastIconError("Không tìm thấy video_file_id để tải phụ đề 2");
            return;
        }

        new ToastMsg(this).toastIconSuccess("Đang tải danh sách phụ đề thứ 2...");
        new Thread(() -> {
            WorkerSubtitleResult result = fetchWorkerSubtitles(fileId);
            runOnUiThread(() -> {
                List<String> labels = new ArrayList<>();
                List<String> urls = new ArrayList<>();

                labels.add("Tắt phụ đề thứ 2");
                urls.add("");

                if (result != null && result.subtitles != null) {
                    for (Subtitle subtitle : result.subtitles) {
                        if (subtitle == null || isNullOrEmpty(subtitle.getUrl())) {
                            continue;
                        }
                        String label = subtitle.getLanguage();
                        if (isNullOrEmpty(label)) {
                            label = buildWorkerSubtitleDisplayLabel(
                        ...
                        labels.add(label);
                        urls.add(normalizeWorkerSubtitleUrl(subtitle.getUrl()));
                    }
                }
                ...
                SharedPreferences prefs = getSharedPreferences("subtitle_settings", MODE_PRIVATE);
                String currentSelectedUrl = normalizeWorkerSubtitleUrl(prefs.getString(PREF_SECONDARY_SUBTITLE_URL, ""));
                int checkedIndex = 0;
                for (int i = 0; i < urls.size(); i++) {
                    if (!isNullOrEmpty(currentSelectedUrl) && currentSelectedUrl.equals(urls.get(i))) {
                        checkedIndex = i;
                        break;
                    }
                }
                ...
                for (int i = 0; i < labels.size(); i++) {
                    ...
                    actions[i] = () -> {
                        ...
                        String selectedLabel = labels.get(index);
                        saveAndApplySubtitleSetting(PREF_SECONDARY_SUBTITLE_URL, selectedUrl);
                        saveAndApplySubtitleSetting(PREF_SECONDARY_SUBTITLE_LABEL, selectedLabel);

                        if (listener != null) {
                            listener.onSelected(selectedLabel);
                        }

                        if (isNullOrEmpty(selectedUrl)) {
                            secondarySubtitleEntries.clear();
                            loadedSecondarySubtitleUrl = "";
                            clearSecondarySubtitleDisplay();
                        } else {
                            ensureSecondarySubtitleLoaded(true);
                        }
                    };
                }
    ...
    private void applyWhiteFocusBorder(android.widget.Button button) {
        if (button == null) {
            return;
        }
        ...
        if (!(background instanceof android.graphics.drawable.GradientDrawable)) {
            ...
            fallback.setColor(0xFF3A3A4C);
            fallback.setStroke(dp(1), 0x00FFFFFF);
            button.setBackground(fallback);
            background = fallback;
        }

        if (background instanceof android.graphics.drawable.GradientDrawable) {
            ((android.graphics.drawable.GradientDrawable) background).setStroke(dp(1), 0x00FFFFFF);
        }
                ...
                public void onResponse(Call<MovieSingleDetails> call, Response<MovieSingleDetails> response) {
                    if (response.isSuccessful() && response.body() != null) {
                        ...
                        if (movieDetails.getSeason() != null && !movieDetails.getSeason().isEmpty()) {
                            ...
                            Episode firstWithSubs = null;
                            // Match current episode by file URL, fallback to first with subtitles
                            outer:
                            for (Season season : movieDetails.getSeason()) {
                                if (season.getEpisodes() != null && !season.getEpisodes().isEmpty()) {
                                    for (Episode episode : season.getEpisodes()) {
                                        if (url != null && url.equals(episode.getFileUrl()) &&
                                                episode.getSubtitle() != null && !episode.getSubtitle().isEmpty()) {
                                            matchedEpisode = episode;
                                            break outer;
                                        }
                                        if (firstWithSubs == null && episode.getSubtitle() != null && !episode.getSubtitle().isEmpty()) {
                                            firstWithSubs = episode;
                                        }
                                    }
                                }
                            }
                            Episode episodeToUse = matchedEpisode != null ? matchedEpisode : firstWithSubs;
                            if (episodeToUse != null) {
                                Video videoWithSubs = new Video();
                                videoWithSubs.setSubtitle(episodeToUse.getSubtitle());
                                videoWithSubs.setVideoFileId(findVideoFileIdByFileUrl(episodeToUse.getFileUrl()));
                                video = videoWithSubs;
                                Log.d(TAG, "✅ Found subtitles in episode " + episodeToUse.getEpisodesId() + ": " + episodeToUse.getSubtitle().size() + " tracks" + (matchedEpisode != null ? " (matched by URL)" : " (fallback)"));
                                openSubtitleDialog();
                                return;
                            }
                        }
                        ...
                        Log.d(TAG, "❌ No subtitles found in API response, trying worker subtitles");
                        tryLoadWorkerSubtitles(resolveCurrentVideoFileId(videoId));
                    } else {
    ...
    private void tryLoadWorkerSubtitles(String fileId) {
        if (isNullOrEmpty(fileId)) {
            runOnUiThread(this::showNoSubtitleMessage);
            return;
        }

        new Thread(() -> {
            WorkerSubtitleResult result = fetchWorkerSubtitles(fileId);
            if (result != null && result.subtitles != null && !result.subtitles.isEmpty()) {
                if (video == null) {
                    video = new Video();
                }
                video.setVideoFileId(fileId);
                video.setSubtitle(result.subtitles);
                runOnUiThread(this::openSubtitleDialog);
                return;
            }

            runOnUiThread(() -> {
                if (result != null && !isNullOrEmpty(result.queuedReason)) {
                    new ToastMsg(this).toastIconSuccess("Phụ đề đang được tạo, vui lòng thử lại sau 1-2 phút");
                } else {
                    showNoSubtitleMessage();
                }
            });
    ...
    private WorkerSubtitleResult fetchWorkerSubtitles(String fileId) {
        WorkerSubtitleResult result = new WorkerSubtitleResult();
        HttpURLConnection connection = null;
        try {
            String endpoint = SUBTITLE_WORKER_BASE_URL + "/subtitles/" + fileId;
            ...
            JSONArray subtitles = json.optJSONArray("subtitles");
            if (subtitles != null) {
                for (int i = 0; i < subtitles.length(); i++) {
                    JSONObject item = subtitles.optJSONObject(i);
                    if (item == null) continue;

                    String subUrl = item.optString("url", null);
                    if (isNullOrEmpty(subUrl)) continue;

                    Subtitle subtitle = new Subtitle();
                    String subtitleId = item.optString("id", "");
                    subtitle.setSubtitleId(subtitleId);
                    subtitle.setUrl(subUrl);

                    String lang = item.optString("lang", "");
                    subtitle.setSrclang(lang);
                    subtitle.setLanguage(buildWorkerSubtitleDisplayLabel(subtitleId, lang, subUrl));
                    subtitle.setKind("captions");
                    result.subtitles.add(subtitle);
                }
            }

            JSONObject queued = json.optJSONObject("queued");
            if (queued != null) {
                result.queuedReason = queued.optString("reason", "queued");
            }
        } catch (Exception e) {
            result.error = e.getMessage();
            Log.e(TAG, "Failed to load worker subtitles", e);
        } finally {
            if (connection != null) {
                connection.disconnect();
            }
    ...
    private String resolveCurrentVideoFileId(String fallbackId) {
        String fromCurrentUrl = extractFileIdFromStreamUrl(url);
        if (!isNullOrEmpty(fromCurrentUrl)) {
            return fromCurrentUrl;
        }

        if (video != null) {
            if (!isNullOrEmpty(video.getVideoFileId())) {
                return video.getVideoFileId();
            }
            String fromVideoUrl = extractFileIdFromStreamUrl(video.getFileUrl());
            if (!isNullOrEmpty(fromVideoUrl)) {
                return fromVideoUrl;
            }
        }

        if (model != null && model.getVideo() != null) {
            if (!isNullOrEmpty(model.getVideo().getVideoFileId())) {
                return model.getVideo().getVideoFileId();
            }
            String fromModelVideoUrl = extractFileIdFromStreamUrl(model.getVideo().getFileUrl());
            if (!isNullOrEmpty(fromModelVideoUrl)) {
                return fromModelVideoUrl;
            }
        }

        if (model != null && model.getVideoList() != null && !model.getVideoList().isEmpty()) {
            for (Video item : model.getVideoList()) {
                if (item == null) continue;

                if (!isNullOrEmpty(url) && url.equals(item.getFileUrl())) {
                    if (!isNullOrEmpty(item.getVideoFileId())) {
                        return item.getVideoFileId();
                    }
                    String fromMatchedUrl = extractFileIdFromStreamUrl(item.getFileUrl());
                    if (!isNullOrEmpty(fromMatchedUrl)) {
                        return fromMatchedUrl;
                    }
                }
                ...
                String fromListUrl = extractFileIdFromStreamUrl(item.getFileUrl());
                if (!isNullOrEmpty(fromListUrl)) {
                    return fromListUrl;
                }
            }
        }
    ...
    private String findVideoFileIdByFileUrl(String fileUrl) {
        String extracted = extractFileIdFromStreamUrl(fileUrl);
        if (!isNullOrEmpty(extracted)) {
            return extracted;
        }

        if (isNullOrEmpty(fileUrl) || model == null || model.getVideoList() == null) {
            return null;
        }
        for (Video item : model.getVideoList()) {
            if (item != null && fileUrl.equals(item.getFileUrl()) && !isNullOrEmpty(item.getVideoFileId())) {
                return item.getVideoFileId();
            }
        }
        return null;
    ...
    private String extractFileIdFromStreamUrl(String streamUrl) {
        ...
        try {
            ...
            // Normalize common stream path endings if any appear.
            if ("master.m3u8".equalsIgnoreCase(last) && segments.size() > 1) {
            ...
            return isNullOrEmpty(last) ? null : last;
        } catch (Exception e) {
            Log.w(TAG, "Unable to extract file_id from stream URL: " + streamUrl);
            return null;
        }
    }

    private String toSubtitleLanguageLabel(String lang) {
        if (isNullOrEmpty(lang)) {
            return "Subtitle";
        }
        String normalized = lang.trim().toLowerCase(Locale.US);
        if ("vie".equals(normalized) || "vi".equals(normalized)) {
            return "Vietnamese (SRT)";
        }
        if ("eng".equals(normalized) || "en".equals(normalized)) {
            return "English (SRT)";
        }
        return normalized.toUpperCase(Locale.US) + " (SRT)";
    }

    private String buildWorkerSubtitleDisplayLabel(String subtitleId, String lang, String subtitleUrl) {
        String effectiveLang = lang;
        if (isNullOrEmpty(effectiveLang)) {
            effectiveLang = inferLangCodeFromSubtitleId(subtitleId);
        }

        String baseLabel = toSubtitleLanguageLabel(effectiveLang);
        String trackTag = extractWorkerTrackTag(subtitleId, subtitleUrl);
        if (!isNullOrEmpty(trackTag)) {
            return baseLabel + " [" + trackTag + "]";
        }
        return baseLabel;
    }

    private String inferLangCodeFromSubtitleId(String subtitleId) {
        if (isNullOrEmpty(subtitleId)) {
            return "";
        }

        String normalized = subtitleId.trim().toLowerCase(Locale.US);
        ...
        if (parts.length > 0 && !isNullOrEmpty(parts[0])) {
            return parts[0];
        }
        return normalized;
    }

    private String extractWorkerTrackTag(String subtitleId, String subtitleUrl) {
        String source = !isNullOrEmpty(subtitleId) ? subtitleId : subtitleUrl;
        ...
        if (underscore >= 0 && underscore + 1 < normalized.length()) {
            String suffix = normalized.substring(underscore + 1);
    ...
    private static class SubtitleCueEntry {
        final long startMs;
        final long endMs;
        final String text;

        SubtitleCueEntry(long startMs, long endMs, String text) {
    ...
    private static class WorkerSubtitleResult {
    ...
    private void loadExternalSubtitle(String subtitleUrl, String language) {
        ...
        try {
            ...
            String subtitleMimeType = lowerUrl.endsWith(".srt")
