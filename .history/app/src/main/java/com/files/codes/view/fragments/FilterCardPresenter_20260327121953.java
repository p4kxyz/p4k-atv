...
public class FilterCardPresenter extends Presenter {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent) {
        ...
        card.setOnFocusChangeListener((v, hasFocus) -> {
            ...
            // Always keep fully visible - prevent dimming
            card.setAlpha(1.0f);
            if (card.getParent() instanceof android.view.View) {
                android.view.View parent = (android.view.View) card.getParent();
                parent.setAlpha(1.0f);
                // When filter gets focus, clear dimming from entire parent hierarchy
                if (hasFocus) {
                    clearDimmingRecursively(parent, 5);
                }
            }
        });
        
        // Helper to clear dimming recursively up the hierarchy
        card.addOnAttachStateChangeListener(new android.view.View.OnAttachStateChangeListener() {
            @Override
            public void onViewAttachedToWindow(android.view.View v) {
                // Continuously monitor and clear dimming
                v.getViewTreeObserver().addOnGlobalFocusChangeListener((oldFocus, newFocus) -> {
                    if (newFocus == v) {
                        // When this card gets focus, clear all dimming
                        clearDimmingRecursively(v, 5);
                    }
                });
            }
            @Override
    ...
    private void clearDimmingRecursively(android.view.View v, int depth) {
        if (v == null || depth <= 0) return;
        v.setAlpha(1.0f);
        v.setBackground(null);
        if (v.getParent() instanceof android.view.View) {
            clearDimmingRecursively((android.view.View) v.getParent(), depth - 1);
        }
        ...
        // Disable Leanback dimming effect - aggressively
        card.addOnAttachStateChangeListener(new android.view.View.OnAttachStateChangeListener() {
            @Override
            public void onViewAttachedToWindow(android.view.View v) {
            ...
            private void clearDimmingRecursively(android.view.View v) {
                v.setAlpha(1.0f);
                if (v.getParent() instanceof android.view.View) {
                    ...
                    if (parent.getParent() instanceof android.view.View) {
                        android.view.View grandparent = (android.view.View) parent.getParent();
