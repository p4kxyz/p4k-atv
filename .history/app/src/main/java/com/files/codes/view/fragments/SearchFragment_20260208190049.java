...
public class SearchFragment extends SearchSupportFragment implements SearchSupportFragment.SearchResultProvider, LoaderManager.LoaderCallbacks<Cursor> {
    ...
    /**
     * Style the Leanback search bar with white focus border.
     * Does NOT interfere with Leanback's own focus management.
     */
    private void setupSearchBarFocusStyle(View rootView) {
        try {
            ...
            if (searchBar != null) {
                // Apply focus-aware border drawable to search bar container
                searchBar.setBackgroundResource(R.drawable.search_bar_focus_bg);
                searchBar.setPadding(16, 8, 16, 8);
                
                // Also watch focus on its children (EditText) to highlight the bar
                searchBar.setOnFocusChangeListener((v, hasFocus) -> {
                    v.setSelected(hasFocus);
                });
                
                // Listen for descendant focus too (when EditText inside gets focus)
                searchBar.getViewTreeObserver().addOnGlobalFocusChangeListener(
                    (oldFocus, newFocus) -> {
                        boolean childHasFocus = newFocus != null && isDescendantOf(newFocus, searchBar);
                        searchBar.setSelected(childHasFocus);
                    });
            }
        } catch (Exception e) {
