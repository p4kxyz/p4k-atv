...
import com.files.codes.view.presenter.SearchHistoryPresenter;
...
public class SearchFragment extends SearchSupportFragment implements SearchSupportFragment.SearchResultProvider, LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private ArrayObjectAdapter recentSearchesAdapter;
    ...
    private Call<SearchModel> activeSearchCall;
    private SearchHistoryManager searchHistoryManager;
    private static final HeaderItem RECENT_SEARCH_HEADER = new HeaderItem("Tìm kiếm gần đây");
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mRowsAdapter = new ArrayObjectAdapter(new ListRowPresenter(androidx.leanback.widget.FocusHighlight.ZOOM_FACTOR_NONE, false));
        searchHistoryManager = SearchHistoryManager.getInstance(requireContext());
        recentSearchesAdapter = new ArrayObjectAdapter(new SearchHistoryPresenter(new SearchHistoryPresenter.ActionListener() {
            @Override
    ...
    private OnItemViewClickedListener getDefaultItemViewClickedListener() {
        return new OnItemViewClickedListener() {
            ...
            public void onItemClicked(Presenter.ViewHolder viewHolder, Object o,
                ...
                if (o instanceof SearchHistoryManager.SearchHistoryEntry) {
                    SearchHistoryManager.SearchHistoryEntry historyEntry = (SearchHistoryManager.SearchHistoryEntry) o;
                    if (!TextUtils.isEmpty(historyEntry.getQuery())) {
                        setSearchQuery(historyEntry.getQuery());
                    }
                    return;
                }
    ...
    public boolean onQueryTextChange(String newQuery) {
        // Submit-only search: just keep the latest query while typing.
    ...
    private void getQueryData() {
        ...
        if (activeSearchCall != null && !activeSearchCall.isCanceled()) {
        ...
        call.enqueue(new Callback<SearchModel>() {
            ...
            public void onResponse(Call<SearchModel> call,  Response<SearchModel> response) {
                if (call.isCanceled() || !TextUtils.equals(query, mQuery)) {
                    return;
                }
                ...
                if (response.code() == 200 && response.body() != null) {
                    ...
                    if (response.body().getMovie() != null && response.body().getMovie().size() != 0) {
                        ...
                        for (Movie video : movieResult) {
                            ...
                            SearchContent searchContent = new SearchContent(id, title, description, type, streamUrl, streamFrom, thumbnailUrl);
                            searchContent.setVideoQuality(video.getVideoQuality());
                            searchContent.setReleaseYear(video.getRelease());
                            searchContent.setRuntime(video.getRuntime());
                            searchContent.setImdbRating(video.getImdbRating());
                            mItems.add(searchContent);
                        }
                    }
                    ...
                    showErrorToast("LÃ¡Â»â€”i tÃƒÂ¬m kiÃ¡ÂºÂ¿m: " + response.code());
                }
            ...
            public void onFailure(Call<SearchModel> call, Throwable t) {
                ...
                showErrorToast("LÃ¡Â»â€”i mÃ¡ÂºÂ¡ng: " + t.getLocalizedMessage());
            }
        });
            ...
            public void onSuccess(List<VideoContent> videoContents) {
                if (videoContents != null && !videoContents.isEmpty()) {
                    ...
                    for (VideoContent videoContent : videoContents) {
                        ...
                        SearchContent searchContent = new SearchContent(id, title, description, type, streamUrl, streamFrom, thumbnailUrl);
                        searchContent.setVideoQuality(videoContent.getVideoQuality());
                        searchContent.setReleaseYear(videoContent.getRelease());
                        searchContent.setRuntime(videoContent.getRuntime());
                        searchContent.setImdbRating(videoContent.getImdbRating());
                        mItems.add(searchContent);
                    }
    ...
    private void loadRows(final List<Movie> movieResult, final List<Movie> tvSeriesResult, final List<TvModel> tvResult) {
        ...
        new AsyncTask<String, Void, List<ListRow>>() {
            ...
            protected void onPostExecute(List<ListRow> listRow) {
                ...
                renderRowsWithHistory(listRow);
                
                // Ã¢Å“â€¦ Auto-move focus to results so user can navigate with down arrow
    ...
    private void loadChannelRows(List<Movie> movieResult, List<Movie> tvSeriesResult, List<TvModel> tvResult) {
        ...
        ArrayObjectAdapter rowsAdapter = new ArrayObjectAdapter(new ListRowPresenter(androidx.leanback.widget.FocusHighlight.ZOOM_FACTOR_NONE, false));
    ...
    private void recordSearchQuery(String query) {
        if (searchHistoryManager == null || TextUtils.isEmpty(query)) {
            return;
        }
        searchHistoryManager.addSearchQuery(query);
        refreshRecentSearchAdapter();
    }
    ...
    private void showRecentSearchHistory() {
        ...
        refreshRecentSearchAdapter();
        mRowsAdapter.clear();
        if (recentSearchesAdapter != null && recentSearchesAdapter.size() > 0) {
            mRowsAdapter.add(new ListRow(RECENT_SEARCH_HEADER, recentSearchesAdapter));
        }
        mRowsAdapter.notifyArrayItemRangeChanged(0, mRowsAdapter.size());
    }

    private void refreshRecentSearchAdapter() {
        if (recentSearchesAdapter == null || searchHistoryManager == null) {
            return;
        }

        recentSearchesAdapter.clear();
        List<SearchHistoryManager.SearchHistoryEntry> recentSearches = searchHistoryManager.getRecentSearches();
        for (SearchHistoryManager.SearchHistoryEntry entry : recentSearches) {
            recentSearchesAdapter.add(entry);
        }
    }

    private void renderRowsWithHistory(List<ListRow> listRows) {
        if (mRowsAdapter == null) {
            return;
        }
