...
public class HeroThumbnailPresenter extends Presenter {
    ...
    public void onBindViewHolder(ViewHolder viewHolder, Object item) {
        ...
        if (thumbnailUrl != null && !thumbnailUrl.isEmpty()) {
            ...
            // Convert dp to px for proper sizing (Reduced 30%: 134dp x 76dp)
            android.content.Context context = holder.view.getContext();
            float density = context.getResources().getDisplayMetrics().density;
            int widthPx = (int) (134 * density);
            int heightPx = (int) (76 * density);
        ...
        if (holder.thumbnailTitle != null) {
            ...
            if (rawTitle != null && !rawTitle.isEmpty()) {
                ...
                if (matcher.find()) {
                    holder.thumbnailTitle.setText(matcher.group(1).trim());
                } else {
                    holder.thumbnailTitle.setText(rawTitle);
                }
    ...
    private String getOptimizedThumbnailUrl(String imageUrl) {
        try {
            ...
            // Thumbnail dimensions for xxhdpi (3x): 134dp → 402px, 76dp → 228px
            int thumbnailWidth = 402;
            int thumbnailHeight = 228;
