...
// Test các endpoints có thể có
$endpoints = [
    // API v1.3.0
    "/api/v1.3.0/movies?api_key={$api_key}&limit=2",
    "/api/v1.3.0/movies?api_key={$api_key}&q=strange&limit=2",
    "/api/v1.3.0/movies?api_key={$api_key}&q=duong%20trieu&limit=2",
    
    // API v1.0.0  
    "/api/v1.0.0/movies?api_key={$api_key}&limit=2",
    "/api/v1.0.0/movies?api_key={$api_key}&q=strange&limit=2", 
    "/api/v1.0.0/movies?api_key={$api_key}&q=duong%20trieu&limit=2",
    
    // Có thể có endpoints khác
    "/api/movies?api_key={$api_key}&limit=2",
    "/api/search?api_key={$api_key}&q=strange&limit=2",
    
    // Test TV series
    "/api/v1.3.0/tvseries?api_key={$api_key}&limit=2",
    "/api/v1.3.0/tvseries?api_key={$api_key}&q=strange&limit=2",
];
...
foreach($search_terms as $term) {
    ...
    $url = "{$base_url}/api/v1.3.0/movies?api_key={$api_key}&q={$encoded_term}&limit=5";
    
    echo "<div style='background:#e3f2fd; padding:10px; margin:5px 0; border-radius:5px;'>";
    echo "<strong>Search term:</strong> '{$term}'<br>";
    
    $result = testAPI($url);
    if($result['http_code'] == 200 && $result['json'] && isset($result['json']['movies'])) {
        ...
        echo "<span style='color:red'>No results</span><br>";
    }
    echo "</div>";
}
?>