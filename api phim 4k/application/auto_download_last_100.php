...
try {
    ...
    $start_id = $config['start_from_id'];
    if ($start_id === null) {
        // Tự động lấy ID cao nhất
        $max_stmt = $pdo->query("SELECT MAX(videos_id) as max_id FROM videos WHERE (tmdbid IS NOT NULL AND tmdbid != '' AND tmdbid != '0') AND publication = 1");
        $max_result = $max_stmt->fetch();
        $start_id = $max_result['max_id'];
        logMessage("🔍 ID cao nhất tìm thấy: {$start_id}");
    }
    
    // Lấy 100 phim từ start_id trở xuống (sử dụng string concatenation cho LIMIT)
    logMessage("Đang lấy {$config['limit']} phim từ ID {$start_id} trở xuống...");
    $sql = "
        SELECT videos_id, tmdbid, title, is_tvseries
        FROM videos 
        WHERE (tmdbid IS NOT NULL AND tmdbid != '' AND tmdbid != '0')
        AND publication = 1
        AND videos_id <= " . (int)$start_id . "
