    ...
    *****/
    ...
    public function search_get() {
        ...
        if(strpos($type, "movie")!==false):
            // Get movies by title
            $movies_by_title = $this->api_v100_model->get_movie_search_result($q,$page,$genre_id,$country_id,$range_to,$range_from);
            // Get movies by actor name
            $movies_by_actor = array();
            if(!empty($q) && $q !='' && $q !=NULL):
                $movies_by_actor = $this->api_v100_model->search_by_actor_name_by_type($q,$page,'0',$genre_id,$country_id,$range_to,$range_from);
            endif;
        ...
        if(strpos($type, "series")!==false):
            // Get series by title
            $series_by_title = $this->api_v100_model->get_tvseries_search_result($q,$page,$genre_id,$country_id,$range_to,$range_from);
            // Get series by actor name
            $series_by_actor = array();
            if(!empty($q) && $q !='' && $q !=NULL):
                $series_by_actor = $this->api_v100_model->search_by_actor_name_by_type($q,$page,'1',$genre_id,$country_id,$range_to,$range_from);
            endif;
            // Merge and remove duplicates
    ...
    public function search_by_actor_get() {
        ...
        if(!empty($q) && $q !='' && $q !=NULL):
            $page               =   $this->input->get('page');
            $response           =   $this->api_v100_model->search_by_actor_name($q, $page);
        else:
            $response['status']     = 'error';
