...
class Api_v100_model extends CI_Model {
    ...
    *****/
    ...
    public function get_single_movie_details_by_id($id=''){
        ...
        $response['imdb_rating']                = isset($movie->imdb_rating) ? $movie->imdb_rating : null;
    ...
    **********************/
    ...
    public function get_movie_search_result($q='',$page='',$genre_id='',$country_id='',$range_to='',$range_from=''){
        ...
        if(!empty($q) && $q !='' && $q !=NULL):
            // Enhanced Vietnamese search - split query into keywords
            $keywords = get_search_keywords($q);
            
            if(!empty($keywords)):
                $this->db->group_start();
                
                // First priority: Exact match in Vietnamese title (with accents)
                $this->db->like('title_vietnamese', $q, 'both');
                
                // Second priority: Vietnamese title without accents
                $q_normalized = removeVietnameseAccents($q);
                if(!empty($q_normalized)):
                    $this->db->or_like('title_vietnamese_no_accent', $q_normalized, 'both');
                endif;
                
                // Third priority: Original title (English/Asian)
                $this->db->or_like('title_original', $q, 'both');
                
                // Fourth priority: Legacy title field (fallback) - search with accents
                $this->db->or_like('title', $q, 'both');
                
                // Fifth priority: Legacy title field - search without accents (for old data)
                if(!empty($q_normalized) && $q !== $q_normalized):
                    $this->db->or_like("LOWER(REPLACE(REPLACE(REPLACE(REPLACE(title, ':', ' '), '-', ' '), '.', ' '), ',', ' '))", $q_normalized, 'both');
                endif;
                
                $this->db->group_end();
                
                // Fifth priority: Search each keyword separately BUT require ALL keywords (AND logic)
                // This ensures "nguoi nhen" returns movies with BOTH words, not just one
                if(count($keywords) > 1):
                    // Multiple keywords - require ALL of them (AND)
                    foreach($keywords as $keyword):
                        $this->db->group_start();
                            // Search in Vietnamese columns first
                            $this->db->like('title_vietnamese', $keyword, 'both');
                            $this->db->or_like('title_vietnamese_no_accent', removeVietnameseAccents($keyword), 'both');
                            $this->db->or_like('title_original', $keyword, 'both');
                            
                            // Fallback to legacy title column
                ...
                elseif(strlen($keywords[0]) >= 3):
                    ...
                    $this->db->group_start();
                        ...
                        if($keywords[0] !== $keyword_normalized):
                            $this->db->or_like('LOWER(title)', $keyword_normalized, 'both');
                        endif;
                    $this->db->group_end();
                endif;
            endif;
    ...
    public function get_tvseries_search_result($q='',$page='',$genre_id='',$country_id='',$range_to='',$range_from=''){
        ...
        if(!empty($q) && $q !='' && $q !=NULL):
            ...
            if(!empty($keywords)):
                ...
                // Fourth priority: Legacy title field (fallback)
                $this->db->or_like('title', $q, 'both');
                ...
                if(count($keywords) > 1):
                    ...
                    foreach($keywords as $keyword):
                        $this->db->group_start();
                            ...
                            // Also search in title with punctuation removed
                            $this->db->or_like("REPLACE(REPLACE(REPLACE(REPLACE(title, ':', ' '), '-', ' '), '.', ' '), ',', ' ')", $keyword, 'both');
                            
                            $keyword_normalized = remove_vietnamese_accents($keyword);
                            if($keyword !== $keyword_normalized):
                                $this->db->or_like('LOWER(title)', $keyword_normalized, 'both');
                                $this->db->or_like("LOWER(REPLACE(REPLACE(REPLACE(REPLACE(title, ':', ' '), '-', ' '), '.', ' '), ',', ' '))", $keyword_normalized, 'both');
                            endif;
                        $this->db->group_end();
                    endforeach;
                elseif(strlen($keywords[0]) >= 3):
                ...
                if(count($keywords) > 1):
                    ...
                    foreach($keywords as $keyword):
                        $this->db->group_start();
                            ...
                            // Also search in tv_name with punctuation removed
                            $this->db->or_like("REPLACE(REPLACE(REPLACE(REPLACE(tv_name, ':', ' '), '-', ' '), '.', ' '), ',', ' ')", $keyword, 'both');
                            
                            $keyword_normalized = remove_vietnamese_accents($keyword);
                            if($keyword !== $keyword_normalized):
                                $this->db->or_like('LOWER(tv_name)', $keyword_normalized, 'both');
                                $this->db->or_like("LOWER(REPLACE(REPLACE(REPLACE(REPLACE(tv_name, ':', ' '), '-', ' '), '.', ' '), ',', ' '))", $keyword_normalized, 'both');
                            endif;
                        $this->db->group_end();
                    endforeach;
                elseif(strlen($keywords[0]) >= 3):
