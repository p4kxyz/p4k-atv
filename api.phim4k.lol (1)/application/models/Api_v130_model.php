...
class Api_v130_model extends CI_Model {
    ...
    public function get_home_content(){
       ...
       $response[$i]['id']                      = $i;
        ...
        foreach ($genres as $genre):
            $genre_movies = $this->get_movie_tvseries_by_genre_id_for_home($genre['genre_id']);
            if(!empty($genre_movies)):
                ...
                $response[$i]['url']                    = '';
                $response[$i]['content']                = $genre_movies;
                $i++;
            endif;
        endforeach;
        return $response;
    ...
    public function get_movies_for_slider_for_home($limit=''){
        ...
        $video_ids = array();
        foreach ($latest_movies as $video) {
            $video_ids[] = $video['videos_id'];
        }
        $preview_urls = $this->prefetch_video_preview_urls($video_ids);
        ...
        foreach ($latest_movies as $video):
            ...
            $response[$i]['stream_url']                 = isset($preview_urls[$video['videos_id']]) ? $preview_urls[$video['videos_id']] : $blank_video_file;
            $i++;
        endforeach;
        return $response;
    }
    ...
    public function get_latest_movies_for_home($limit=''){
        ...
        foreach ($latest_movies as $video):
            ...
            $response[$i]['stream_from']                = '';
            $response[$i]['stream_label']               = '';
            $response[$i]['stream_url']                 = ''; // Optimized to reduce CPU load
            $i++;
        endforeach;
        return $response;
    }
    ...
    *****/
    ...
    // get latest movie
    public function get_latest_movies($limit='', $genre_id='', $country_id='', $range_from='', $range_to='', $page=''){
        $response       = array();
        $actual_limit = (!empty($limit) && is_numeric($limit)) ? (int)$limit : $this->default_limit;
        $offset = (!empty($page) && is_numeric($page) && (int)$page > 1) ? ((int)$page * $actual_limit - $actual_limit) : 0;
        $this->db->limit($actual_limit, $offset);
        ...
        if(!empty($range_from) && is_numeric($range_from) && strlen($range_from)==4 && !empty($range_to) && is_numeric($range_to) && strlen($range_to)==4):
            $this->db->where('release >=', date('Y-m-d', strtotime($range_from.'-01-01')));
            $this->db->where('release <=', date('Y-m-d', strtotime($range_to.'-12-31')));
        endif;
        ...
        foreach ($latest_movies as $video):
            ...
            $response[$i]['slug']                       = $video['slug'];
            $response[$i]['is_paid']                    = $video['is_paid'];
            $response[$i]['release']                    = (!empty($video['release']) ? date('Y', strtotime($video['release'])) : null);
            $response[$i]['runtime']                    = $video['runtime'];
            $response[$i]['video_quality']              = $video['video_quality'];
            $response[$i]['imdb_rating']                = isset($video['imdb_rating']) ? $video['imdb_rating'] : null;
            $response[$i]['thumbnail_url']              = $this->common_model->get_video_thumb_url($video['videos_id']);
...
//                    $response[$i]['file_url']           = str_replace('.com/','.com/video/',$response[$i]['file_url']);
    ...
    *****/
    ...
    // get latest movies
    public function get_latest_tvseries($limit='', $genre_id='', $country_id='', $range_from='', $range_to='', $page=''){
        $response       = array();
        $actual_limit = (!empty($limit) && is_numeric($limit)) ? (int)$limit : $this->default_limit;
        $offset = (!empty($page) && is_numeric($page) && (int)$page > 1) ? ((int)$page * $actual_limit - $actual_limit) : 0;
        $this->db->limit($actual_limit, $offset);
    ...
    **********************/
    ...
    public function get_movie_search_result($q='',$page='',$genre_id='',$country_id='',$range_to='',$range_from=''){
        ...
        if(!empty($q) && $q !='' && $q !=NULL):
            ...
            $q_normalized = removeVietnameseAccents($q);
            
            $this->db->group_start();
                // Search Vietnamese title with accents
                $this->db->like('title_vietnamese', $q, 'both');
                
                // Search Vietnamese title without accents
                $this->db->or_like('title_vietnamese_no_accent', $q_normalized, 'both');
                
                // Search original title
                $this->db->or_like('title_original', $q, 'both');
                
                // Search legacy title field
                $this->db->or_like('title', $q, 'both');
            $this->db->group_end();
        endif;
    ...
    public function get_tvseries_search_result($q='',$page='',$genre_id='',$country_id='',$range_to='',$range_from=''){
        ...
        if(!empty($q) && $q !='' && $q !=NULL):
            // Simple Vietnamese search for TV series
            $q_normalized = removeVietnameseAccents($q);
    ...
    public function get_tv_channel_search_result($q='',$page='',$tv_category_id=''){
        ...
        if(!empty($q) && $q !='' && $q !=NULL):
            // Enhanced fuzzy search - split query into keywords
            $keywords = get_search_keywords($q);
            
            if(!empty($keywords)):
                $this->db->group_start();
                
                // First priority: Search with original query
                $this->db->like('tv_name', $q, 'both');
                
                // Second priority: Search with normalized query
                $q_normalized = removeVietnameseAccents($q);
                if($q !== $q_normalized):
                    $this->db->or_like('LOWER(tv_name)', $q_normalized, 'both');
                endif;
                
                $this->db->group_end();
                
                // Third priority: Search each keyword separately BUT require ALL keywords (AND logic)
                if(count($keywords) > 1):
                    // Multiple keywords - require ALL of them (AND)
                    foreach($keywords as $keyword):
                        $this->db->group_start();
                            $this->db->like('tv_name', $keyword, 'both');
                            
                            $keyword_normalized = removeVietnameseAccents($keyword);
                            if($keyword !== $keyword_normalized):
                                $this->db->or_like('LOWER(tv_name)', $keyword_normalized, 'both');
                            endif;
                        $this->db->group_end();
                    endforeach;
                elseif(strlen($keywords[0]) >= 3):
                    // Single keyword with 3+ chars - allow partial match
                    $this->db->group_start();
                        $this->db->like('tv_name', $keywords[0], 'both');
                        
                        $keyword_normalized = removeVietnameseAccents($keywords[0]);
    ...
    // Search movies by actor/star name
    public function search_by_actor_name($q='', $page=''){
        $q = trim($q);
        $response = array();
        ...
        // First: Find star IDs matching the search query
        $keywords = get_search_keywords($q);
        $star_ids = array();
        
        if(!empty($keywords)):
            // Search in star table
            ...
            $this->db->group_start();
                ...
                // Search with normalized name (replace hyphens with spaces)
                $this->db->or_like("REPLACE(star_name, '-', ' ')", $q, 'both');
                
                // Normalized query (Vietnamese accents)
                $q_normalized = removeVietnameseAccents($q);
                if($q !== $q_normalized):
                    $this->db->or_like('LOWER(star_name)', $q_normalized, 'both');
                    $this->db->or_like("LOWER(REPLACE(star_name, '-', ' '))", $q_normalized, 'both');
                endif;
            $this->db->group_end();
            
            // Fuzzy search by keywords (AND logic)
            if(count($keywords) > 1):
                foreach($keywords as $keyword):
                    $this->db->group_start();
                        $this->db->like('star_name', $keyword, 'both');
                        $this->db->or_like("REPLACE(star_name, '-', ' ')", $keyword, 'both');
                        
                        $keyword_normalized = removeVietnameseAccents($keyword);
                        if($keyword !== $keyword_normalized):
                            $this->db->or_like('LOWER(star_name)', $keyword_normalized, 'both');
                            $this->db->or_like("LOWER(REPLACE(star_name, '-', ' '))", $keyword_normalized, 'both');
                        endif;
                    $this->db->group_end();
                endforeach;
            endif;
            ...
            foreach($stars as $star):
                $star_ids[] = $star['star_id'];
            endforeach;
        endif;
        
        // If no stars found, return empty
        if(empty($star_ids)):
            return $response;
        endif;
        
        // Second: Find movies with these star IDs
        if(!empty($page) && $page !='' && $page !=NULL && is_numeric($page)):
            $offset = ((int)$page * $this->default_limit) - $this->default_limit;
            $this->db->limit($this->default_limit, $offset);
        else:
        ...
        // Search in stars field (comma-separated IDs)
        $this->db->group_start();
        foreach($star_ids as $star_id):
            $this->db->or_where("find_in_set(".$star_id.",stars) >", 0);
        endforeach;
        $this->db->group_end();
    ...
    // Search movies by actor/star name - separated by type
    public function search_by_actor_name_by_type($q='', $page='', $is_tvseries='0', $genre_id='', $country_id='', $range_to='', $range_from=''){
        $q = trim($q);
        $genre_id = trim($genre_id);
        $country_id = trim($country_id);
        $range_to = trim($range_to);
    ...
    // Netflix-style reverse flow: TV generates code (no user_id yet)
    function generate_tv_code() {
        // Remove expired codes first to keep table clean
        $this->db->where('expire_time <', time());
        $this->db->delete('tv_connection');
        ...
        $data['status']      = 'pending';
        $data['create_time'] = time();
        $data['expire_time'] = strtotime("+5 minutes", time());
        $this->db->insert('tv_connection', $data);
        return $str;
    }

    // Phone confirms: links user_id to the TV code
    function confirm_tv_code($code = '', $user_id = 0) {
        $this->db->where('code', $code);
        ...
        if ($query->num_rows() > 0) {
            $this->db->where('code', $code);
            $this->db->where('status', 'pending');
            $this->db->update('tv_connection', array(
                'user_id' => (int)$user_id,
                'status'  => 'confirmed',
            ));
            return TRUE;
    ...
    // TV polls: check if phone has confirmed the code
    function check_tv_code_status($code = '') {
        $this->db->where('code', $code);
        $this->db->where('expire_time >', time());
        $query = $this->db->get('tv_connection');
        if ($query->num_rows() === 0) {
        ...
        $row = $query->row();
        if ($row->status === 'confirmed' && (int)$row->user_id > 0) {
            return array('status' => 'confirmed', 'user_id' => (int)$row->user_id);
        }
    ...
    function get_ads_config(){
        $response['ads_enable']                         =   '0';
        $response['mobile_ads_network']                 =   '';
        $response['admob_app_id']                       =   '';
        $response['admob_banner_ads_id']                =   ''; 
        $response['admob_interstitial_ads_id']          =   ''; 
        $response['admob_native_ads_id']                =   ''; 
        $response['fan_native_ads_placement_id']        =   ''; 
        $response['fan_banner_ads_placement_id']        =   ''; 
        $response['fan_interstitial_ads_placement_id']  =   ''; 
        $response['fan_native_ads_placement_id']        =   ''; 
        $response['startapp_app_id']                    =   '';
        return $response;
    }
