        ...
        try {
            ...
            try {
                ...
                if (video != null && video.getSubtitle() != null && !video.getSubtitle().isEmpty()) {
                    ...
                    String subtitleUrl = video.getSubtitle().get(0).getUrl();
                    if (subtitleUrl != null && !subtitleUrl.isEmpty()) {
                        intent.putExtra("subs", new Uri[] { Uri.parse(subtitleUrl) });
                        intent.putExtra("subs.name", new String[] { video.getSubtitle().get(0).getLanguage() });
                        Log.d(TAG, "📝 Adding subtitle: " + subtitleUrl);
                    }
                }
            } catch (Exception e) {
                Log.e(TAG, "Error adding subtitle: " + e.getMessage());
            }
