        ...
        switch ($_GET['action']) {
            case 'test':
                echo json_encode([
                    ...
                    'api_key_works' => true, // Simplified for now
            ...
            case 'process_batch':
                ...
                foreach ($movies as $movie) {
                    ...
                    // Giả lập download (để test)
                    $results[] = [
                        'message' => "🔍 Processing #{$videos_id}: " . substr($title, 0, 30) . "... (TMDB: {$tmdbid})",
                        'type' => 'info'
                    ];
                    $skipped++; // Tạm thời mark là skipped để test
...
<?php
// This file now only contains the HTML interface
// API endpoints are handled separately
?>