...
public function enhanced_movie_search($q='', $page='', $genre_id='', $country_id='', $range_to='', $range_from='') {
    ...
    if(!empty($q) && $q !='' && $q !=NULL):
        ...
        if(!empty($keywords)):
            ...
            // Priority 1: Exact match in title (highest priority)
            $this->db->like('title', $q, 'both');
            
            // Priority 2: Exact match in Vietnamese title
            $this->db->or_like('title_vietnamese', $q, 'both');
            
            // Priority 3: Exact match in search title (no accents)
            ...
            // Priority 4: Fuzzy search with all keywords (AND logic)
            if(count($keywords) > 1):
