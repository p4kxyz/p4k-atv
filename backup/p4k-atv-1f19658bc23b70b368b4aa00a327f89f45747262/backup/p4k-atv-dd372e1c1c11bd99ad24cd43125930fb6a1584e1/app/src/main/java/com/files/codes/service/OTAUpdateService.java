...
public class OTAUpdateService extends Service {
    ...
    public void initializeFirebase() {
        if (updateRef == null) {
            ...
            try {
                ...
                Log.d(TAG, "Firebase URL: https://website-19a7d-default-rtdb.asia-southeast1.firebasedatabase.app");
                Log.d(TAG, "Firebase path: " + FIREBASE_UPDATE_PATH);
            } catch (Exception e) {
    ...
    public void checkForUpdates() {
        ...
        Log.d(TAG, "🔄 Checking for updates with fresh data...");
        ...
        updateRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                try {
                    ...
                    Log.d(TAG, "📊 Firebase Path: " + FIREBASE_UPDATE_PATH);
                    Log.d(TAG, "🗄️ Snapshot Key: " + snapshot.getKey());
                    ...
                    if (snapshot.exists()) {
                        ...
                        if (isUpdateAvailable(updateInfo)) {
                            ...
                            Log.d(TAG, "❌ No update needed. Current >= Available");
                            if (updateListener != null) {
                                updateListener.onNoUpdateAvailable();
                            }
                        }
                    } else {
                        Log.w(TAG, "❌ No update data found in Firebase at path: " + FIREBASE_UPDATE_PATH);
                        Log.w(TAG, "🔍 DEBUG: Check if data exists at Firebase Console:");
                        Log.w(TAG, "🔗 URL: https://website-19a7d-default-rtdb.asia-southeast1.firebasedatabase.app/");
                        Log.w(TAG, "📍 Path: " + FIREBASE_UPDATE_PATH);
                        if (updateListener != null) {
                            updateListener.onNoUpdateAvailable();
                        }
                    }
    ...
    private boolean isUpdateAvailable(UpdateInfo updateInfo) {
        ...
        // Check if user skipped this version (unless force update)
        if (hasUpdate && !updateInfo.forceUpdate) {
            Context ctx = context != null ? context : this;
            String skippedVersion = PreferenceUtils.getString(ctx, "ota_skipped_version", "");
            Log.d(TAG, "Checking skipped version: " + skippedVersion + " vs " + updateInfo.version);
            if (updateInfo.version.equals(skippedVersion)) {
    ...
    public void skipVersion(String version) {
        PreferenceUtils.saveString(this, "ota_skipped_version", version);
