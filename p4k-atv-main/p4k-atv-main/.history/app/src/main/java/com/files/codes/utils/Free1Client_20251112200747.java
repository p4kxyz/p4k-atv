...
public class Free1Client {
    ...
    private VideoContent convertPhim4kMovieToVideoContent(Phim4kMovie movie) {
        ...
        video.setGenre(movie.getCasts()); // Use casts as genre for now
        video.setRuntime(movie.getTime());
        video.setVideoQuality(movie.getQuality());
        video.setStreamLabel(movie.getLanguage());
        video.setIsTvseries(String.valueOf(movie.getTotalEpisodes() > 1));
        
        // Set episodes if available
        if (movie.getEpisodes() != null && !movie.getEpisodes().isEmpty()) {
            List<String> episodeUrls = new ArrayList<>();
            for (Phim4kEpisodeServer episodeServer : movie.getEpisodes()) {
                if (episodeServer.getItems() != null && !episodeServer.getItems().isEmpty()) {
                    for (Phim4kEpisode episode : episodeServer.getItems()) {
                        if (episode.getLink() != null && !episode.getLink().isEmpty()) {
                            episodeUrls.add(episode.getLink());
                        }
                    }
                }
            }
