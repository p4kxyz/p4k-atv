...
public interface WatchHistorySyncService {
    ...
    /**
     * Ghi dữ liệu lịch sử xem lên server (old format)
     */
    ...
    /**
     * Ghi dữ liệu lịch sử xem lên server (full structure format)
     */
    @POST("user-json/{id}/write")
    Call<JsonObject> writeFullWatchHistory(
        @Path("id") String userId,
        @Body Map<String, Object> watchHistoryData
    );
