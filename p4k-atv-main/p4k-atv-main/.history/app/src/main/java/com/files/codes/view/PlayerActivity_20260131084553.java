...
//                }
    ...
    public void initVideoPlayer(String url, String type) {
        ...
        final int finalExtensionMode = useSoftwareAudioDecoder 
                ? DefaultRenderersFactory.EXTENSION_RENDERER_MODE_PREFER 
                : DefaultRenderersFactory.EXTENSION_RENDERER_MODE_ON;
        
        Log.d("PlayerActivity", "🔊 Extension Renderer Mode Target: " + finalExtensionMode);
        
        // Custom RenderersFactory to STRICTLY avoid multiple audio renderers
        RenderersFactory renderersFactory = new DefaultRenderersFactory(this) {
            ...
            public void buildAudioRenderers(Context context, int extensionRendererMode, com.google.android.exoplayer2.mediacodec.MediaCodecSelector mediaCodecSelector, boolean enableDecoderFallback, com.google.android.exoplayer2.audio.AudioSink audioSink, Handler eventHandler, com.google.android.exoplayer2.audio.AudioRendererEventListener eventListener, java.util.ArrayList<com.google.android.exoplayer2.Renderer> out) {
                // Determine if we should only include Extension Renderer (FFmpeg)
                // This happens if useSoftwareAudioDecoder is TRUE.
                boolean forceExtensionOnly = useSoftwareAudioDecoder;

                if (forceExtensionOnly) {
                    Log.d("PlayerActivity", "🔨 FORCE MODE: Building ONLY Extension Audio Renderer (FFmpeg)");
                    // Add ONLY extension renderers. Pass 'out' directly.
                    // We call super but with a trick: we want it to add extension renderers.
                    // But standard DefaultRenderersFactory adds MediaCodecAudioRenderer first.
                    
                    // Manual Construction of FFMpeg Renderer to guarantee it is the ONLY one.
                    try {
                        // Reflectively load FFMpegAudioRenderer to avoid compile-time dependency strictness if library missing
                        // But since we have it, we try to add it.
                        // Actually, DefaultRenderersFactory has a protected method addExtensionRenderers.
                        // But we can just use the super method with PREFER and then REMOVE the first element if it's MediaCodec.
                        
                        java.util.ArrayList<com.google.android.exoplayer2.Renderer> tempOut = new java.util.ArrayList<>();
                        super.buildAudioRenderers(context, DefaultRenderersFactory.EXTENSION_RENDERER_MODE_PREFER, mediaCodecSelector, enableDecoderFallback, audioSink, eventHandler, eventListener, tempOut);
                        
                        // Filter out MediaCodecAudioRenderer
                        for (com.google.android.exoplayer2.Renderer renderer : tempOut) {
                            String rendererName = renderer.getClass().getSimpleName();
                            Log.d("PlayerActivity", "   - Check built renderer: " + rendererName);
                            
                            // If it's the standard MediaCodec renderer, SKIP IT
                            if (rendererName.contains("MediaCodecAudioRenderer")) {
                                Log.d("PlayerActivity", "   - 🚫 REMOVING MediaCodecAudioRenderer to prevent conflict");
                                continue; 
                            }
                            
                            // Add others (likely FfmpegAudioRenderer)
                            out.add(renderer);
                            Log.d("PlayerActivity", "   - ✅ KEEPING " + rendererName);
                        }
                    } catch (Exception e) {
                         Log.e("PlayerActivity", "❌ Error building custom renderers", e);
                         // Fallback to standard
                         super.buildAudioRenderers(context, extensionRendererMode, mediaCodecSelector, enableDecoderFallback, audioSink, eventHandler, eventListener, out);
                    }
                } else {
                    // Standard build
                    Log.d("PlayerActivity", "🔨 STANDARD MODE: Building Audio Renderers");
                    super.buildAudioRenderers(context, extensionRendererMode, mediaCodecSelector, enableDecoderFallback, audioSink, eventHandler, eventListener, out);
                }
            }
