...
//                }
    ...
    public void initVideoPlayer(String url, String type) {
        ...
        else if (audioMode == 2) finalExtensionMode = DefaultRenderersFactory.EXTENSION_RENDERER_MODE_ON;
        else finalExtensionMode = DefaultRenderersFactory.EXTENSION_RENDERER_MODE_OFF; // HW Mode
        ...
        RenderersFactory renderersFactory = new DefaultRenderersFactory(this) {
            ...
            public void buildAudioRenderers(Context context, int extensionRendererMode, com.google.android.exoplayer2.mediacodec.MediaCodecSelector mediaCodecSelector, boolean enableDecoderFallback, com.google.android.exoplayer2.audio.AudioSink audioSink, Handler eventHandler, com.google.android.exoplayer2.audio.AudioRendererEventListener eventListener, java.util.ArrayList<com.google.android.exoplayer2.Renderer> out) {
                // If software decoder is forced, we want to AVOID standard MediaCodecAudioRenderer
                if (audioMode == 1) { // Software Mode
                     Log.d("PlayerActivity", "🔨 FORCE SW MODE: Initializing FFmpeg extraction...");
                     
                     // 1. Create a temporary list to let super build everything it wants (HW + SW)
                     java.util.ArrayList<com.google.android.exoplayer2.Renderer> tempRenderers = new java.util.ArrayList<>();
                     
                     // 2. Call super with EXTENSION_RENDERER_MODE_PREFER to ensure FFmpeg is included
                     super.buildAudioRenderers(context, DefaultRenderersFactory.EXTENSION_RENDERER_MODE_PREFER, mediaCodecSelector, enableDecoderFallback, audioSink, eventHandler, eventListener, tempRenderers);
                     
                     // 3. Filter the list: Only allow FFmpeg / Libopus / Libflac etc.
                     //    Strictly REJECT MediaCodecAudioRenderer
                     boolean loadedExtension = false;
                     
                     for (com.google.android.exoplayer2.Renderer r : tempRenderers) {
                         String className = r.getClass().getSimpleName();
                         if (className.contains("MediaCodecAudioRenderer")) {
                             Log.d("PlayerActivity", "🔥 DROPPED HARDWARE RENDERER: " + className);
                             continue; // Skip hardware renderer
                         }
                         
                         out.add(r);
                         loadedExtension = true;
                         Log.d("PlayerActivity", "✅ ADDED SOFTWARE RENDERER: " + className);
                     }
                     
                     // Check if FFmpeg Native Library is actually available using reflection
                     if (loadedExtension) {
                         try {
                             ...
                             if (!available) {
                                 Log.e("PlayerActivity", "⚠️ FFmpeg extension found but native library NOT available!");
                                 runOnUiThread(() -> new ToastMsg(PlayerActivity.this).toastIconError("Lỗi: FFmpeg thiếu thư viện Native!"));
                             } else {
                                 Log.d("PlayerActivity", "✅ FFmpeg Native Library is available.");
                             }
                         } catch (Exception e) {
                             Log.w("PlayerActivity", "⚠️ Could not verify FFmpeg availability: " + e.getMessage());
                         }
                     }
                     
                     // Fallback check: If no extension loaded (e.g. library missing), we MUST add the HW renderer back
                     // to prevent "No audio renderer" error, even if it might risk the clock crash.
                     if (!loadedExtension) {
                         Log.w("PlayerActivity", "⚠️ NO SOFTWARE RENDERER FOUND! Falling back to Hardware Renderer.");
                     ...
                     // Force OFF extensions
                     super.buildAudioRenderers(context, DefaultRenderersFactory.EXTENSION_RENDERER_MODE_OFF, mediaCodecSelector, enableDecoderFallback, audioSink, eventHandler, eventListener, out);
                     
                } else { // Passthrough Mode (2)
                     Log.d("PlayerActivity", "🔨 PASSTHROUGH/DEFAULT MODE: Building Renderers");
                     // Use ON (Default) allow both, but usually HW is first in list unless PREFER is set? 
                     // Actually Standard behavior is MediaCodec first, then Extensions.
                     super.buildAudioRenderers(context, DefaultRenderersFactory.EXTENSION_RENDERER_MODE_ON, mediaCodecSelector, enableDecoderFallback, audioSink, eventHandler, eventListener, out);
                }
            }
        ...
        player.addAnalyticsListener(new com.google.android.exoplayer2.analytics.AnalyticsListener() {
            ...
            public void onAudioDecoderInitialized(EventTime eventTime, String decoderName, long initializationDurationMs) {
                Log.d("PlayerActivity", "🎧 Audio Decoder Initialized: " + decoderName);
                runOnUiThread(() -> {
                    ...
                    if (!useSoftwareAudioDecoder && !isVideoCodecError) {
                        if (error.errorCode == com.google.android.exoplayer2.PlaybackException.ERROR_CODE_DECODER_INIT_FAILED || 
                            error.errorCode == com.google.android.exoplayer2.PlaybackException.ERROR_CODE_DECODING_FAILED) {
                             Log.w("PlayerActivity", "⚠️ Generic decoding error in HW mode - assuming Audio issue and attempting fallback.");
                             isAudioCodecError = true;
                        }
                    }
                }
