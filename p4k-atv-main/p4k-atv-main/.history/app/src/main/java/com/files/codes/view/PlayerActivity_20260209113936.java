...
//                }
    ...
    private void resolveAnimeStreamAndContinue() {
        ...
        // Show loading toast
        Toast.makeText(this, "Đang tải nguồn phát anime...", Toast.LENGTH_SHORT).show();

        // Resolve on background thread
        new Thread(() -> {
            AnimeStreamResponse streamResponse = AnimeClient.getInstance().getStreamSourceSync(
            ...
            runOnUiThread(() -> {
                if (streamResponse != null && streamResponse.getUrl() != null && !streamResponse.getUrl().isEmpty()) {
                    ...
                    Log.e(TAG, "Failed to resolve anime stream URL");
                    new ToastMsg(PlayerActivity.this).toastIconError("Không thể tải nguồn phát anime");
                    finish();
                }
            });
        }).start();
