...
public class PlayerActivity extends Activity {
    ...
    private java.util.Map<String, String> animeProxyHeaders = null; // Headers from anime stream API (Referer etc)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Handle anime stream URL resolution
        if (AnimeClient.isAnimeResolveUrl(url)) {
            resolveAnimeStreamAndContinue();
            return;
        }
...
//                }
    ...
    /**
     * Resolve anime stream URL and continue player initialization.
     * Called when URL is in "anime_resolve:{episodeData}|{provider}|{server}" format.
     */
    private void resolveAnimeStreamAndContinue() {
        String[] parts = AnimeClient.parseAnimeResolveUrl(url);
        if (parts == null) {
            new ToastMsg(this).toastIconError("Lỗi: URL anime không hợp lệ");
            finish();
            return;
        }

        final String episodeData = parts[0];
        final String provider = parts[1];
        final String server = parts[2];

        Log.d(TAG, "Resolving anime stream: episodeData=" + episodeData + ", provider=" + provider + ", server=" + server);

        // Show loading UI
        if (progressBar != null) progressBar.setVisibility(VISIBLE);
        Toast.makeText(this, "Đang tải nguồn phát anime... (có thể mất 15-30s)", Toast.LENGTH_LONG).show();

        // Resolve on background thread with retry
        final int MAX_RETRIES = 2;
        new Thread(() -> {
            AnimeStreamResponse streamResponse = null;
            for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                Log.d(TAG, "Anime stream resolve attempt " + attempt + "/" + MAX_RETRIES);
                streamResponse = AnimeClient.getInstance().getStreamSourceSync(
                    episodeData, provider, server.isEmpty() ? null : server);
                if (streamResponse != null && streamResponse.getUrl() != null && !streamResponse.getUrl().isEmpty()) {
                    break; // Success
                }
                if (attempt < MAX_RETRIES) {
                    final int att = attempt;
                    runOnUiThread(() -> Toast.makeText(PlayerActivity.this,
                        "Thử lại lần " + (att + 1) + "...", Toast.LENGTH_SHORT).show());
                    try { Thread.sleep(2000); } catch (InterruptedException ignored) {}
                }
            }

            final AnimeStreamResponse finalResponse = streamResponse;
            runOnUiThread(() -> {
                if (progressBar != null) progressBar.setVisibility(View.GONE);
                if (isFinishing()) return;

                if (finalResponse != null && finalResponse.getUrl() != null && !finalResponse.getUrl().isEmpty()) {
                    if (finalResponse.isPlayable()) {
                        url = streamResponse.getUrl();
                        videoType = "hls"; // Must be "hls" to match switch case in initVideoPlayer
                        model.setVideoUrl(url);
                        model.setVideoType("hls");

                        // Save proxy headers (Referer etc.) for ExoPlayer data source
                        animeProxyHeaders = streamResponse.getProxyHeaders();
                        // Always ensure Referer is set for anime streams (required by source sites)
                        if (animeProxyHeaders == null || animeProxyHeaders.isEmpty()) {
                            animeProxyHeaders = new java.util.HashMap<>();
                            // Default Referer based on provider
                            animeProxyHeaders.put("Referer", "https://animevietsub.show/");
                            animeProxyHeaders.put("Origin", "https://animevietsub.show");
                            Log.d(TAG, "Anime stream: using default headers (Referer: animevietsub.show)");
                        } else {
                            Log.d(TAG, "Anime stream headers from API: " + animeProxyHeaders.toString());
                        }

                        Log.d(TAG, "Anime stream resolved: " + url + " (type: " + streamResponse.getType() + ")");

                        // Continue normal player initialization
                        category = model.getCategory();
                        if (model.getVideo() != null)
                            video = model.getVideo();
                        ...
                        try {
                            tvRecommendationManager = new TvRecommendationManager(PlayerActivity.this);
                            addToTvRecommendations();
                        } catch (Exception e) {
                            Log.e(TAG, "Error initializing TV recommendations", e);
                        }
                    } else {
                        // EMBED type - not directly playable
                        Log.w(TAG, "Anime stream type EMBED not supported: " + streamResponse.getUrl());
                        new ToastMsg(PlayerActivity.this).toastIconError("Nguồn phát này không hỗ trợ (EMBED)");
                        finish();
                    }
                } else {
                    Log.e(TAG, "Failed to resolve anime stream URL after " + MAX_RETRIES + " attempts");
                    new ToastMsg(PlayerActivity.this).toastIconError("Không thể tải nguồn phát anime. Server có thể đang bận, thử lại sau.");
                    finish();
                }
            });
        }).start();
   ...
   /* @SuppressLint("StaticFieldLeak")
    ...
    private com.google.android.exoplayer2.source.MediaSource hlsMediaSource(Uri uri, Context context) {
        ...
        // If anime proxy headers are present (Referer etc.), use HttpDataSource with custom headers
        if (animeProxyHeaders != null && !animeProxyHeaders.isEmpty()) {
            Log.d(TAG, "HLS: Using custom headers for anime stream: " + animeProxyHeaders);
            ...
            // Clear after use so normal streams aren't affected
            animeProxyHeaders = null;
        } else {
