...
//                }
   ...
   /* @SuppressLint("StaticFieldLeak")
    ...
    /**
     * Resolve anime_resolve: URL to actual stream URL, then continue with initVideoPlayer
     * Retries up to 2 times with 2s delay between attempts
     */
    private void resolveAnimeStreamAndContinue(String resolveUrl, String type) {
        progressBar.setVisibility(VISIBLE);
        
        String[] parts = AnimeClient.parseAnimeResolveUrl(resolveUrl);
        if (parts == null) {
            Log.e(TAG, "Failed to parse anime_resolve URL: " + resolveUrl);
            Toast.makeText(this, "Lỗi phân tích URL anime", Toast.LENGTH_SHORT).show();
            return;
        }
        
        String episodeData = parts[0];
        String provider = parts[1];
        String server = parts[2];
        
        Log.d(TAG, "🎬 Resolving anime stream - episodeData: " + episodeData + ", provider: " + provider + ", server: " + server);
        Toast.makeText(this, "Đang tải nguồn anime...", Toast.LENGTH_SHORT).show();
        ...
        new Thread(() -> {
            ...
            for (int attempt = 1; attempt <= 2; attempt++) {
                Log.d(TAG, "🎬 Anime stream resolve attempt " + attempt);
                streamResponse = AnimeClient.getInstance().getStreamSourceSync(episodeData, provider, server.isEmpty() ? null : server);
            ...
            runOnUiThread(() -> {
                if (finalResponse != null && finalResponse.getUrl() != null && !finalResponse.getUrl().isEmpty()) {
                    String streamUrl = finalResponse.getUrl();
                    
                    // Fix relative URLs - prepend base URL if needed
                    if (streamUrl.startsWith("/")) {
                        streamUrl = "https://api.animapper.net" + streamUrl;
                    }
                    
                    Log.d(TAG, "🎬 Anime stream resolved: " + streamUrl);
                    
                    // Set proxy headers if required
                    animeProxyHeaders = null;
                    if (finalResponse.isCorsProxyRequired() && finalResponse.getProxyHeaders() != null) {
                        animeProxyHeaders = new HashMap<>();
                        for (Map.Entry<String, String> entry : finalResponse.getProxyHeaders().entrySet()) {
                            animeProxyHeaders.put(entry.getKey(), entry.getValue());
                        }
                        Log.d(TAG, "🎬 Anime proxy headers set: " + animeProxyHeaders);
                    } else {
                        // Default referer for anime streams
                        animeProxyHeaders = new HashMap<>();
                        animeProxyHeaders.put("Referer", "https://animevietsub.page");
                    }
                    
                    // Now play with HLS type
                    initVideoPlayer(streamUrl, "hls");
                } else {
                    Log.e(TAG, "Failed to resolve anime stream after retries");
                    Toast.makeText(PlayerActivity.this, "Không thể tải nguồn anime. Vui lòng thử lại.", Toast.LENGTH_LONG).show();
                    progressBar.setVisibility(View.GONE);
                }
            });
        }).start();
