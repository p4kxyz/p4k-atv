...
//                }
    ...
    private void launchExternalPlayer() {
        try {
            ...
            // Intercept Phim4k CDN and generate secure token URL
            if (videoUrl != null && videoUrl.contains("cdn.phim4k.lol")) {
                try {
                    String filename = VideoTokenGenerator.extractFilenameFromCdnUrl(videoUrl);
                    String secureUrl = VideoTokenGenerator.genVideoUrl(filename);
                    if (secureUrl != null && !secureUrl.isEmpty()) {
                        Log.d("PlayerActivity", "ðŸ”’ Generated secure URL for external player: " + secureUrl);
                        videoUrl = secureUrl;
                    }
                } catch (Exception e) {
                    Log.e("PlayerActivity", "âŒ Error generating secure video URL", e);
                }
            }
