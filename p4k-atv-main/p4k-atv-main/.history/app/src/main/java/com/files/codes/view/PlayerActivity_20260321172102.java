...
//                }
    ...
    private void launchExternalPlayer() {
        try {
            ...
            final String cleanedUrl = cleanVideoUrl(videoUrl);

            if (cleanedUrl != null && cleanedUrl.contains("cdn.phim4k.lol")) {
                if (progressBar != null) progressBar.setVisibility(View.VISIBLE);
                VideoTokenGenerator.resolveStreamUrlAsync(cleanedUrl, new VideoTokenGenerator.StreamUrlCallback() {
                    @Override
                    public void onSuccess(String streamUrl) {
                        Log.d("PlayerActivity", "ðŸ”’ Resolved secure URL for external player: " + streamUrl);
                        if (progressBar != null) progressBar.setVisibility(View.GONE);
                        continueLaunchExternalPlayer(streamUrl, videoUrl);
                    }

                    @Override
                    public void onError(Exception e) {
                        Log.e("PlayerActivity", "âŒ Error resolving secure video URL", e);
                        if (progressBar != null) progressBar.setVisibility(View.GONE);
                        continueLaunchExternalPlayer(cleanedUrl, videoUrl);
                    }
                });
            } else {
                continueLaunchExternalPlayer(cleanedUrl, videoUrl);
            ...
            Log.e(TAG, "Error in launchExternalPlayer", e);
        }
    }

    private void continueLaunchExternalPlayer(String videoUrl, String originalUrl) {
        try {
    ...
    public void initVideoPlayer(String url, String type) {
        if (url != null && url.contains("cdn.phim4k.lol")) {
            if (progressBar != null) progressBar.setVisibility(View.VISIBLE);
            VideoTokenGenerator.resolveStreamUrlAsync(url, new VideoTokenGenerator.StreamUrlCallback() {
                @Override
                public void onSuccess(String streamUrl) {
                    Log.d("PlayerActivity", "ðŸ”’ Resolved secure URL: " + streamUrl);
                    initVideoPlayerInternal(streamUrl, type);
                }

                @Override
                public void onError(Exception e) {
                    Log.e("PlayerActivity", "âŒ Error resolving secure video URL", e);
                    initVideoPlayerInternal(url, type);
                }
            });
            return;
        }
    ...
    private void initVideoPlayerInternal(String url, String type) {
    ...
    */
    ...
    private android.app.Dialog buildCustomAlertDialog(
        ...
        if (message != null && !message.isEmpty()) {
            ...
            msgView.setTextColor(0xFFDDDDDD);
