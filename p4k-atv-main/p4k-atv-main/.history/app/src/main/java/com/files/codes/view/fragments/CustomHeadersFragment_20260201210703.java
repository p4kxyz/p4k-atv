    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        setOnHeaderViewSelectedListener(new OnHeaderViewSelectedListener() {
            ...
            public void onHeaderSelected(RowHeaderPresenter.ViewHolder viewHolder, Row row) {
                ...
                // [FIX] Ensure frag replacement happens safely
                if (isResumed()) {
        ...
        // [FIX] Handle Mouse Clicks on Header Items
        // Creates explicit click handling for older boxes/mouse usage
        setOnHeaderClickedListener(new HeadersSupportFragment.OnHeaderClickedListener() {
            @Override
            public void onHeaderClicked(RowHeaderPresenter.ViewHolder viewHolder, Row row) {
                ...
                // 1. Force fragment change (in case selection didn't trigger)
                getFragmentManager().beginTransaction().replace(R.id.rows_container, (Fragment) obj).commitAllowingStateLoss();
                ((HomeActivity) getActivity()).updateCurrentRowsFragment((Fragment) obj);
                
                // 2. Close Menu (Smoothly transition to content)
                ((HomeActivity) getActivity()).toggleHeadersFragment(false);
                
                // 3. Request focus on the new content
                // We post this to ensure the view hierarchy has updated
                viewHolder.view.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        try {
                            VerticalGridView grid = ((HomeActivity) getActivity()).getVerticalGridView((Fragment) obj);
