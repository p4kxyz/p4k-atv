...
public class SearchFragment extends SearchSupportFragment implements SearchSupportFragment.SearchResultProvider, LoaderManager.LoaderCallbacks<Cursor> {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        // Style search bar: white focus border + click-to-activate typing
        setupSearchBarFocusStyle(view);
        
        // Move initial focus away from search EditText to filter panel
        // Delay to override Leanback's auto-focus
        view.postDelayed(() -> {
            if (getActivity() != null) {
                View filterBtn = getActivity().findViewById(R.id.btn_filter_genre);
                if (filterBtn != null) {
     ...
     * - EditText disabled by default (no auto-keyboard)
     * - Press ENTER/OK on search bar → activate EditText for typing
     * - Press BACK while typing → deactivate EditText, return to navigation
     */
    private void setupSearchBarFocusStyle(View rootView) {
        try {
            ...
            searchBar.setBackgroundResource(R.drawable.search_bar_focus_bg);
            ...
            if (searchEditTextView instanceof EditText) {
                ...
                // Disable EditText focus initially — search bar is just navigable
                rootView.postDelayed(() -> {
                    editText.setFocusable(false);
                    editText.setFocusableInTouchMode(false);
                }, 100);
                
                // ENTER/OK on search bar → activate EditText
                ...
                // BACK while in EditText → deactivate, return focus to search bar
            ...
            searchBar.getViewTreeObserver().addOnGlobalFocusChangeListener(
                (oldFocus, newFocus) -> {
                    boolean hasFocus = newFocus == searchBar || 
                        (newFocus != null && isDescendantOf(newFocus, searchBar));
                    searchBar.setSelected(hasFocus);
                });
