...
public class SearchFragment extends SearchSupportFragment implements SearchSupportFragment.SearchResultProvider, LoaderManager.LoaderCallbacks<Cursor> {
            ...
            public void onItemClicked(Presenter.ViewHolder viewHolder, Object o,
                ...
                switch (searchContent.getType()) {
                    ...
                    case "tvseries": {
                        ...
                        } else if ("Anime".equals(searchContent.getStreamFrom())) {
                            intent.putExtra("source", "anime");
                        }
                        startActivity(intent);
                        break;
                    }
    ...
    private void searchAnime(String query) {
        ...
        AnimeClient.getInstance().searchAnime(query, new AnimeClient.AnimeSearchCallback() {
            @Override
            public void onSuccess(List<VideoContent> videoContents) {
                if (videoContents != null && !videoContents.isEmpty()) {
                    animeResults.clear();
                    animeResults.addAll(videoContents);
                    ...
                    if (getActivity() != null) {
                        getActivity().runOnUiThread(() -> addAnimeRowToAdapter());
                    }
                }
            }
    ...
    private void addAnimeRowToAdapter() {
        if (animeResults.isEmpty()) return;
        
        for (int i = 0; i < mRowsAdapter.size(); i++) {
            Object row = mRowsAdapter.get(i);
            if (row instanceof ListRow) {
                ListRow listRow = (ListRow) row;
                if ("Nguồn Anime".equals(listRow.getHeaderItem().getName())) {
                    return;
                }
            }
        }
        
        ArrayObjectAdapter animeAdapter = new ArrayObjectAdapter(new SearchCardPresenter());
        
        for (SearchContent searchContent : mItems) {
            if ("Anime".equals(searchContent.getStreamFrom())) {
                animeAdapter.add(searchContent);
            }
        }
        
        if (animeAdapter.size() > 0) {
            HeaderItem animeHeaderItem = new HeaderItem("Nguồn Anime");
            ListRow animeRow = new ListRow(animeHeaderItem, animeAdapter);
            mRowsAdapter.add(animeRow);
        }
    }
