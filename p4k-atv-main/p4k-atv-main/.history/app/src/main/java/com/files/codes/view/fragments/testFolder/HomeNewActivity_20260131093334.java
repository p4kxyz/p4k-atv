...
public class HomeNewActivity extends FragmentActivity {
                ...
                public View onFocusSearch(View focused, int direction) {
                    ...
                    if (direction == View.FOCUS_UP) {
                        ...
                        if (focused == settingsOrb) {
                             // Try to find a view within the rows container to focus
                             // Ideally, we focus the fragment or let the system find the first focusable in container
    ...
    private void showPlayerSelectionDialog() {
       ...
       int selected = useExternal ? 1 : 0;
       String[] items = {"ExoPlayer (Mặc định)", "Trình phát ngoài (VLC/MX Player)"};
       
       new android.app.AlertDialog.Builder(this)
           .setTitle("Chọn Trình phát")
           .setSingleChoiceItems(items, selected, new android.content.DialogInterface.OnClickListener() {
               @Override
               public void onClick(android.content.DialogInterface dialog, int which) {
                    boolean newValue = (which == 1);
                    pref.edit().putBoolean("use_external_player", newValue).apply();
                    new com.files.codes.utils.ToastMsg(HomeNewActivity.this).toastIconSuccess(newValue ? "Đã chọn: External Player" : "Đã chọn: ExoPlayer");
                    dialog.dismiss();
