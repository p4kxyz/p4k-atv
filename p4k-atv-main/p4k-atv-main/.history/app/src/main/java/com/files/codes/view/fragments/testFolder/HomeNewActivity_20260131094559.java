            ...
            settingsOrb.setOnFocusChangeListener(new View.OnFocusChangeListener() {
                ...
                public void onFocusChange(View v, boolean hasFocus) {
                    if (hasFocus) {
                        ...
                        v.setBackgroundResource(R.drawable.rounded_button_gradient); // Optional: if you have a drawable for background
                        // Or just change alpha/color filter
                        settingsOrb.setColorFilter(android.graphics.Color.YELLOW); // Quick identifier
                    } else {
                        v.animate().scaleX(1.0f).scaleY(1.0f).setDuration(150).start();
                        settingsOrb.clearColorFilter();
                        v.setBackgroundResource(0); // Reset background or set to original
                        // check original background in xml: ?android:attr/selectableItemBackgroundBorderless
                         
                        // Re-apply ripple if needed, or just leave plain
