...
public class HomeNewFragment extends BrowseSupportFragment {
    private static final int LEFT_MENU_BG_COLOR = 0xCC000000;
    private ViewTreeObserver.OnGlobalFocusChangeListener leftMenuFocusListener;
    ...
    private static final long HEADER_ID_WATCH_HISTORY = 8;
    private static final String HEADER_NAME_WATCH_HISTORY = "Lịch sử xem";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // Disable built-in Search Orb to avoid duplication with custom custom_search_orb in Activity
    ...
    public void onStart() {
        super.onStart();
        attachLeftMenuFocusListener();
        applyLeftMenuBackground();
    }
    ...
    private void createRows() {
        ...
        IconHeaderItem headerItemHistory = new IconHeaderItem(HEADER_ID_WATCH_HISTORY, HEADER_NAME_WATCH_HISTORY, R.drawable.ic_history_24);
        PageRow pageRowHistory = new PageRow(headerItemHistory);
        mRowsAdapter.add(pageRowHistory);
            ...
            public Presenter getPresenter(Object item) {
                return new RowHeaderPresenter() {
                    ...
                    public ViewHolder onCreateViewHolder(ViewGroup parent) {
                        ...
                        // [FIX] Force focusable states for Old Android Boxes
                        view.setFocusable(true);
                        view.setFocusableInTouchMode(true); // Allow focus even in touch mode (some airmouses need this)
                        
                        if (view instanceof ViewGroup) {
                            ((ViewGroup) view).setDescendantFocusability(ViewGroup.FOCUS_BLOCK_DESCENDANTS); // Prevent inner text/icon from stealing focus
                        }
                    ...
                    public void onBindViewHolder(Presenter.ViewHolder viewHolder, Object item) {
                        ...
                        rootView.setOnFocusChangeListener(new View.OnFocusChangeListener() {
                            ...
                            public void onFocusChange(View v, boolean hasFocus) {
                                if (hasFocus) {
                                    ...
                                    // [FIX] Force Selection on Focus
                                    // Older boxes sometimes separate "Focus" and "Selection".
                                    // When a header gets focus, we force Leanback to actually select and expand it.
                                    // This ensures the content page loads immediately when scrolling through headers.
                                    setSelectedPosition(mRowsAdapter.indexOf(pageRow));
                        ...
                        // [FIX] Handle Mouse Click on Header Items (Prevent crash/exit on older boxes)
                        rootView.setOnClickListener(new View.OnClickListener() {
                            @Override
                            public void onClick(View v) {
                                // Force Leanback to select this row when clicked
                                setSelectedPosition(mRowsAdapter.indexOf(pageRow));
                            }
                        });
                    }
    ...
    private void setupUi() {
        ...
        // Avoid Leanback painting a dark left strip from brand color when headers animate.
        setBrandColor(Color.TRANSPARENT);
        // setTitle(getResources().getString(R.string.app_name)); // Ẩn title app
        setOnItemViewSelectedListener((itemViewHolder, item, rowViewHolder, row) -> {
             if (row != null && row.getHeaderItem() != null && getActivity() instanceof HomeNewActivity) {
                 long headerId = row.getHeaderItem().getId();
                 boolean isHome = (headerId == HEADER_ID_HOME);
                 ((HomeNewActivity) getActivity()).setOrbsVisibility(isHome);
             }
        });
    ...
    private void applyLeftMenuBackground() {
        final View fragmentRoot = getView();
        if (fragmentRoot == null) {
            return;
        }

        fragmentRoot.post(new Runnable() {
            @Override
            public void run() {
                View headersDock = findLeanbackView(fragmentRoot, "browse_headers_dock");
                if (headersDock != null) {
                    int color = isShowingHeaders() ? LEFT_MENU_BG_COLOR : Color.TRANSPARENT;
                    headersDock.setBackground(new ColorDrawable(color));
                }

                // Fallback for OEM-modified Leanback layouts.
                View containerDock = findLeanbackView(fragmentRoot, "browse_container_dock");
                if (containerDock != null) {
                    containerDock.setBackgroundColor(Color.TRANSPARENT);
                }
            }
        });
    }

    private void attachLeftMenuFocusListener() {
        final View fragmentRoot = getView();
        if (fragmentRoot == null) {
            return;
        }

        if (leftMenuFocusListener == null) {
            leftMenuFocusListener = new ViewTreeObserver.OnGlobalFocusChangeListener() {
                @Override
        ...
        ViewTreeObserver observer = fragmentRoot.getViewTreeObserver();
        if (observer.isAlive()) {
            observer.removeOnGlobalFocusChangeListener(leftMenuFocusListener);
            observer.addOnGlobalFocusChangeListener(leftMenuFocusListener);
        }
    }

    private void detachLeftMenuFocusListener() {
        View fragmentRoot = getView();
        if (fragmentRoot == null || leftMenuFocusListener == null) {
            return;
        }
    ...
    private View findLeanbackView(View root, String idName) {
        int id = root.getResources().getIdentifier(idName, "id", "androidx.leanback");
        if (id != 0) {
        ...
        int appId = root.getResources().getIdentifier(idName, "id", requireContext().getPackageName());
        if (appId != 0) {
            return root.findViewById(appId);
        }
    ...
    private static class PageRowFragmentFactory extends FragmentFactory {
        ...
        public Fragment createFragment(Object rowObj) {
            ...
            if (row.getHeaderItem().getId() == HEADER_ID_HOME) {
                ...
                return new MoviesWithFilterFragment();
            ...
            } else if (row.getHeaderItem().getId() == HEADER_ID_WATCH_HISTORY) {
                return new WatchHistoryPageFragment();
