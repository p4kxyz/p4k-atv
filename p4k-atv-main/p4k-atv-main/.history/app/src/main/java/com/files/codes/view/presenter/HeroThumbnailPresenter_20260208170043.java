...
public class HeroThumbnailPresenter extends Presenter {
    ...
    /**
     * Smart line break for thumbnail titles.
     * Splits long titles into 2 lines at ':' > ',' > ' - ' > nearest space to middle.
     */
    private String smartThumbnailLineBreak(String title) {
        if (title == null || title.trim().isEmpty()) return "";
        title = title.trim();
        // Thumbnail is small (161dp/11sp) — ~20 chars fit on one line
        ...
        for (int i = 0; i < title.length(); i++) {
            if (title.charAt(i) == ',') {
                ...
                if (dist < bestDist) { bestDist = dist; commaIdx = i; }
            }
        }
