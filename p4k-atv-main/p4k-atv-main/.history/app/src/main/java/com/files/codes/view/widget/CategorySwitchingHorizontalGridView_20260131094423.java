            ...
            if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_DOWN) {
                ...
                if (listener != null) {
                    ...
                    if (consumed) {
                        ...
                        Log.d(TAG, "🔓 At first category - attempting to escape UP");
                        
                        // [FIX] Explicitly find Settings Orb and focus it
                        // This handles the case where standard focus search fails to leave the grid
                        View root = getRootView();
                        if (root != null) {
                            // Try to find view by ID "settings_orb"
                            ...
                            if (settingsId != 0) {
                                ...
                                if (settingsOrb != null && settingsOrb.isFocusable()) {
                                    Log.d(TAG, "🎯 Found Settings Orb. Requesting focus...");
                                    settingsOrb.requestFocus();
                                    return true; // Consume event
                                }
                            }
                            
                             // Fallback: Try FocusFinder again specifically for UP
