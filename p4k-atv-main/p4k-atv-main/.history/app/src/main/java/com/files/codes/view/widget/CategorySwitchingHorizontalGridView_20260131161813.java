            ...
            if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_DOWN) {
                ...
                if (listener != null) {
                    ...
                    if (consumed) {
                        ...
                        Log.d(TAG, "🔓 At first category - attempting to escape UP to Search Orb");
                        
                        // [FIX] Explicitly find Search Orb and focus it
                        View root = getRootView();
                        if (root != null) {
                            // Try to find view by ID "custom_search_orb"
                            int searchOrbId = getResources().getIdentifier("custom_search_orb", "id", getContext().getPackageName());
                            if (searchOrbId != 0) {
                                View searchOrb = root.findViewById(searchOrbId);
                                if (searchOrb != null && searchOrb.isFocusable()) {
                                    Log.d(TAG, "🎯 Found Search Orb. Requesting focus...");
                                    searchOrb.requestFocus();
                                    return true; // Consume event
                                }
                            }
                            
                            // Check for settings orb as fallback
                            int settingsId = getResources().getIdentifier("settings_orb", "id", getContext().getPackageName());
                            if (settingsId != 0) {
                                View settingsOrb = root.findViewById(settingsId);
                                if (settingsOrb != null && settingsOrb.isFocusable()) {
                                     // settingsOrb.requestFocus(); // DISABLED: Only go to Search on UP
                                }
                            }
                            
                             // Fallback: Try FocusFinder
                            View next = FocusFinder.getInstance().findNextFocus((ViewGroup) root, this, View.FOCUS_UP);
                            if (next != null && next != this) {
                                Log.d(TAG, "🎯 FocusFinder found UP target: " + next);
                                next.requestFocus();
                                return true;
                            }
                        }
