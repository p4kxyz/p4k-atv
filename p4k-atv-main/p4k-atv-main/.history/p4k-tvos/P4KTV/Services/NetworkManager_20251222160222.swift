...
class NetworkManager {
    ...
    func fetchGenres(completion: @escaping (Result<[Genre], Error>) -> Void) {
        request(endpoint: "all_genre", completion: completion)
    }
    
    func search(query: String, genreId: String? = nil, year: String? = nil, completion: @escaping (Result<SearchResponse, Error>) -> Void) {
        var params = ["q": query, "type": "movie,series"]
        if let gId = genreId, gId != "All" {
            params["genre_id"] = gId
        }
        if let y = year, y != "All" {
            params["range_from"] = y
        ...
        request(endpoint: "search", parameters: params, completion: completion)
    }
}
