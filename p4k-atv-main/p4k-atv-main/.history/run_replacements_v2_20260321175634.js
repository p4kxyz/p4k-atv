...
let text = fs.readFileSync('app/src/main/java/com/files/codes/view/PlayerActivity.java', 'utf8');

// 0. Add Dialog Helpers at the end of the class
const helperCode = `
    // =================================================================================
...
// replace last `}` with helperCode
let lastBraceIdx = text.lastIndexOf('}');
if (lastBraceIdx !== -1) {
    text = text.substring(0, lastBraceIdx) + helperCode + text.substring(lastBraceIdx + 1);
}

// 1. showPlayerInstallDialog
text = text.replace(/    private void showPlayerInstallDialog\(final String playerName, final String packageName\) \{[\s\S]*?dialog\.show\(\);\s*\}/g,
...
text = text.replace(/    private void showUrlIssueDialog\(String videoUrl\) \{[\s\S]*?dialog\.show\(\);\s*\}/g, 
...
text = text.replace(/        AlertDialog\.Builder builder = new AlertDialog\.Builder\(this\);\s*builder\.setTitle\("Chọn tỷ lệ màn hình"\);\s*builder\.setItems\(aspectRatioOptions, \(dialog, which\) -> \{[\s\S]*?        \}\);\s*builder\.show\(\);/g, 
...
text = text.replace(/    private void showFormatErrorDialog\(\) \{[\s\S]*?\.show\(\);\s*\}/g,
...
// 5. showErrorDialog overloading
text = text.replace(/    private void showErrorDialog\(String title, String message, boolean finishOnClose\) \{[\s\S]*?\}\s*\}\);\s*\}/g,
...
text = text.replace(/    private void showErrorDialog\(String title, String message\) \{[\s\S]*?\}\s*\}\);\s*\}/g,
...
text = text.replace(/    private void showAudioDisabledNotification\(\) \{[\s\S]*?\.show\(\);\s*\}/g,
...
text = text.replace(/    private void showAudioTrackSwitchedNotification\(\) \{[\s\S]*?\.show\(\);\s*\}/g,
...
text = text.replace(/    private void showAudioCodecErrorDialog\(\) \{[\s\S]*?\.show\(\);\s*\}/g,
...
text = text.replace(/    private void showVideoCodecErrorDialog\(\) \{[\s\S]*?\.show\(\);\s*\}/g,
...
text = text.replace(/        AlertDialog\.Builder builder = new AlertDialog\.Builder\(this\);\s*builder\.setTitle\("Chọn phụ đề \(" \+ totalTracks \+ " có sẵn\)"\);\s*builder\.setItems\(labels\.toArray\(new String\[0\]\), \(dialog, which\) -> \{[\s\S]*?        \}\);\s*builder\.show\(\);/g,
...
text = text.replace(/        AlertDialog\.Builder builder = new AlertDialog\.Builder\(this\);\s*builder\.setTitle\("Chọn âm thanh \(" \+ labels\.size\(\) \+ " có sẵn\)"\);\s*builder\.setItems\(labels\.toArray\(new String\[0\]\), \(dialog, which\) -> \{[\s\S]*?        \}\);\s*builder\.show\(\);/g,
`        Runnable[] actions = new Runnable[labels.size()];
        for (int i = 0; i < labels.size(); i++) {
            final int which = i;
            actions[i] = () -> {
...
text = text.replace(/                    AlertDialog\.Builder builder = new AlertDialog\.Builder\(this\);\s*builder\.setTitle\("Subtitles"\);\s*String\[\] options = \{"On", "Off"\};\s*builder\.setItems\(options, \(dialog, which\) -> \{[\s\S]*?                    \}\);\s*builder\.show\(\);/g,
...
text = text.replace(/    private void showNextEpisodeDialog\(\) \{[\s\S]*?\n    \}\n/g,
...
fs.writeFileSync('app/src/main/java/com/files/codes/view/PlayerActivity.java', text, 'utf8');

console.log("All modifications done successfully using Node.js script.");
