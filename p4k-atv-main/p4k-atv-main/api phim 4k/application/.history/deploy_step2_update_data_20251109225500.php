...
try {
    ...
    // Lấy tất cả videos cần update
    $stmt = $pdo->query("SELECT videos_id, title FROM videos WHERE title IS NOT NULL LIMIT 1000");
    $videos = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    $updated = 0;
    $total = count($videos);
    
    echo "<p>📊 Tổng số video cần update: {$total}</p>\n";
