...
"""    // =================================================================================
    ...
    content = do_replace(r'    private void showPlayerInstallDialog\(final String playerName, final String packageName\) \{.*?dialog\.show\(\);\s*\}', r1, content)
    ...
    # We replace literal `\n` mapped inside the raw string to `\\n` so Javac sees the backslash.
    r2 = r2.replace('\n\n- Thử', '\\n\\n- Thử').replace('\n- Hoặc', '\\n- Hoặc')
    content = do_replace(r'    private void showUrlIssueDialog\(String videoUrl\) \{.*?dialog\.show\(\);\s*\}', r2, content)
    ...
    content = do_replace(r'    private void showFormatErrorDialog\(\) \{.*?\.show\(\);\s*\}', r4, content)

    r5_1 = r'''    private void showErrorDialog(String title, String message, boolean finishOnClose) {
    ...
    content = do_replace(r'    private void showErrorDialog\(String title, String message, boolean finishOnClose\) \{.*?\}\s*\}\);\s*\}', r5_1, content)
    ...
    content = do_replace(r'    private void showErrorDialog\(String title, String message\) \{.*?\}\s*\}\);\s*\}', r5_2, content)
    ...
    content = do_replace(r'    private void showAudioDisabledNotification\(\) \{.*?\.show\(\);\s*\}', r6_1, content)
    ...
    content = do_replace(r'    private void showAudioTrackSwitchedNotification\(\) \{.*?\.show\(\);\s*\}', r6_2, content)
    ...
    content = do_replace(r'    private void showAudioCodecErrorDialog\(\) \{.*?\.show\(\);\s*\}', r6_3, content)
    ...
    content = do_replace(r'    private void showVideoCodecErrorDialog\(\) \{.*?\.show\(\);\s*\}', r6_4, content)
...
'''.replace('\n\n" +', '\\n\\n" +')
    content = do_replace(r'    private void showNextEpisodeDialog\(\) \{.*?\n    \}\n', r10, content)
...
print(f"Update applied to {file_path}. Emojis removed, escape sequences clean!")
