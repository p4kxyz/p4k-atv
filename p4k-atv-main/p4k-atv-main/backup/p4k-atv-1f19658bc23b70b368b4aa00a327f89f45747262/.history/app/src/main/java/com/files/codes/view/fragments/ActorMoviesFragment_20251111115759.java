...
public class ActorMoviesFragment extends VerticalGridSupportFragment {
    ...
    private void populateMovies(List<Movie> movies) {
        ...
        // Create movie row
        ArrayObjectAdapter movieAdapter = new ArrayObjectAdapter(new VerticalCardPresenter(MoviesFragment.MOVIE));
        for (Movie movie : movies) {
            movieAdapter.add(movie);
        }
        
        HeaderItem moviesHeader = new HeaderItem(0, "Phim của " + actorName + " (" + movies.size() + " phim)");
        mRowsAdapter.add(new ListRow(moviesHeader, movieAdapter));
    }
    
    private void showEmptyState() {
        ArrayObjectAdapter emptyAdapter = new ArrayObjectAdapter(new VerticalCardPresenter(MoviesFragment.MOVIE));
        HeaderItem emptyHeader = new HeaderItem(0, "Không có phim nào của " + actorName);
        mRowsAdapter.add(new ListRow(emptyHeader, emptyAdapter));
    }
