...
public class HeroThumbnailPresenter extends Presenter {
    ...
    public void onBindViewHolder(ViewHolder viewHolder, Object item) {
        ...
        if (thumbnailUrl != null && !thumbnailUrl.isEmpty()) {
            ...
            // Convert dp to px for proper sizing
            android.content.Context context = holder.view.getContext();
            float density = context.getResources().getDisplayMetrics().density;
            int widthPx = (int) (120 * density);   // 120dp -> px
            int heightPx = (int) (180 * density);  // 180dp -> px
            
            android.util.Log.d("HeroThumbnailPresenter", "Density: " + density + ", Size: " + widthPx + "x" + heightPx + "px");
            
            Picasso.get()
                .load(optimizedUrl)
                .noFade()
                .resize(widthPx, heightPx)       // Resize theo px thực tế của màn hình
                .onlyScaleDown()
     ...
     * Size will match device density (e.g., 360x540px for xxhdpi)
     */
    private String getOptimizedThumbnailUrl(String imageUrl) {
        try {
            ...
            // Dynamic thumbnail dimensions based on device density
            // 120dp * density (e.g., 3x = 360px on xxhdpi)
            int thumbnailWidth = 360;   // For xxhdpi (3x) - adjust if needed
            int thumbnailHeight = 540;  // For xxhdpi (3x) - adjust if needed
