...
public function enhanced_movie_search($q='', $page='', $genre_id='', $country_id='', $range_to='', $range_from='') {
    ...
    if(!empty($q)):
        $this->db->order_by("CASE 
            WHEN title_vietnamese LIKE '%".$this->db->escape_like_str($q)."%' THEN 1
            WHEN title_original LIKE '%".$this->db->escape_like_str($q)."%' THEN 2
            WHEN title_vietnamese_no_accent LIKE '%".$this->db->escape_like_str($q_normalized ?? $q)."%' THEN 3
            WHEN title LIKE '%".$this->db->escape_like_str($q)."%' THEN 4
            ELSE 5 END");
    endif;
