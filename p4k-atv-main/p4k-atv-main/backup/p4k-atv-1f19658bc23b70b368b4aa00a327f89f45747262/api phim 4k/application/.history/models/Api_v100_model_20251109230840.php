    ...
    public function get_tvseries_search_result($q='',$page='',$genre_id='',$country_id='',$range_to='',$range_from=''){
        ...
        if(!empty($q) && $q !='' && $q !=NULL):
            ...
            if(!empty($keywords)):
                ...
                // First priority: Search with original query (exact phrase with accents)
                $this->db->like('title', $q, 'both');
                
                // Second priority: Search with normalized query (exact phrase without accents)
                $q_normalized = remove_vietnamese_accents($q);
                if($q !== $q_normalized):
                    $this->db->or_like('LOWER(title)', $q_normalized, 'both');
                endif;
