    ...
    public function get_movie_search_result($q='',$page='',$genre_id='',$country_id='',$range_to='',$range_from=''){
        ...
        if(!empty($q) && $q !='' && $q !=NULL):
            ...
            if(!empty($keywords)):
                ...
                // Second priority: Search in Vietnamese name
                $this->db->or_like('title_vietnamese', $q, 'both');
                
                // Third priority: Search with normalized query (exact phrase without accents)
                $q_normalized = remove_vietnamese_accents($q);
                if($q !== $q_normalized):
                    $this->db->or_like('title_search', $q_normalized, 'both');
                endif;
