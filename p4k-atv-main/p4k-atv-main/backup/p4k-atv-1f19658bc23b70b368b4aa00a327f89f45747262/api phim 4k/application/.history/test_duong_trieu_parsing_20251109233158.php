<?php
/**
 * Test script để kiểm tra parsing của title cụ thể
 */
...
echo "<h2>🧪 Test Đường Triều Quỷ Sự Lục</h2>\n";

// Test case cụ thể
$title = "Đường Triều Quỷ Sự Lục (2022) Strange Tales of Tang Dynasty";
...
echo "<h3>🔍 Test Search Queries:</h3>";
...
foreach($searchQueries as $query) {
    ...
    echo "<strong>Search query:</strong> '{$query}'<br>";
    
    $query_normalized = removeVietnameseAccents($query);
    echo "<strong>Normalized:</strong> '{$query_normalized}'<br>";
    
    // Check if it would match
    $vietnamese = $parsed['vietnamese_no_accent'];
    $original = $parsed['original'];
    
    $matchVietnamese = (stripos($vietnamese, $query) !== false || stripos($vietnamese, $query_normalized) !== false);
    $matchOriginal = (stripos($original, $query) !== false);
    
    if($matchVietnamese) {
        echo "<span style='color:green'>✅ Would match Vietnamese column</span><br>";
    }
    if($matchOriginal) {
        echo "<span style='color:green'>✅ Would match Original column</span><br>";
    }
    if(!$matchVietnamese && !$matchOriginal) {
        echo "<span style='color:red'>❌ No match</span><br>";
    }
