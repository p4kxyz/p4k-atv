                ...
                try {
                    ...
                    if (empty($movies)) {
                        ...
                        echo json_encode([
                            ...
                            'has_more' => false,
                            'next_offset' => $offset,
                            'results' => [['message' => '🎉 Hoàn thành tất cả! Không còn phim nào để xử lý.', 'type' => 'success']]
                        ]);
                        break;
                    }
...
<body>
    <div class="container">
        ...
        <div style="background: #d4edda; padding: 10px; margin: 10px 0; border-radius: 5px; font-size: 14px;">
            ...
            • Batch Size: <?php echo $config['batch_size']; ?> movies per batch<br>
            • Image Size: w300
        </div>
        ...
        <div style="text-align: center; margin: 20px 0;">
            ...
            <button class="btn" onclick="forceNext()" style="background: #ffc107;">⏭️ Force Next</button>
    ...
    <script>
        ...
        function forceNext() {
            if (!isRunning) {
                log('⚠️ Process không đang chạy!', 'warning');
                return;
            }
            
            currentOffset += 3; // Skip 3 movies
            consecutiveErrors = 0;
            log('⏭️ Force skip to offset: ' + currentOffset, 'warning');
            ...
            fetch(window.location.href + '?action=process_batch&offset=' + currentOffset, {
                ...
                .then(data => {
                    ...
                    if (data.error) {
                        ...
                        if (data.next_offset) {
                            ...
                            log('⏭️ Continuing to next batch...', 'info');
                ...
                .catch(error => {
                    ...
                    if (consecutiveErrors >= maxConsecutiveErrors) {
                        log('⚠️ Quá nhiều lỗi liên tục (' + consecutiveErrors + '). Force skip batch...', 'warning');
                        currentOffset += 3; // Skip 3 movies
                        consecutiveErrors = 0;
                        log('⏭️ Skipped to offset: ' + currentOffset, 'info');
                        setTimeout(processNextBatch, 3000);
                        return;
                    }
                    
                    if (error.name === 'AbortError') {
                        log('⏰ Request timeout (' + consecutiveErrors + '/' + maxConsecutiveErrors + ') - retrying in 5 seconds...', 'warning');
                        setTimeout(processNextBatch, 5000);
                    } else {
                        log('❌ Network error (' + consecutiveErrors + '/' + maxConsecutiveErrors + '): ' + error.message, 'error');
                        log('🔄 Auto-retry in 8 seconds...', 'info');
                        setTimeout(processNextBatch, 8000);
                    }
                });
        }
