...
class Update_Title_Search extends CI_Controller {
    ...
    private function create_indexes() {
        try {
            ...
            $indexes = $this->db->query("SHOW INDEX FROM videos WHERE Key_name = 'idx_title_vietnamese_no_accent'")->result();
            if(empty($indexes)) {
                $this->db->query("ALTER TABLE `videos` ADD KEY `idx_title_vietnamese_no_accent` (`title_vietnamese_no_accent`)");
                echo "Đã tạo index cho title_vietnamese_no_accent\n";
            }
            
            $indexes2 = $this->db->query("SHOW INDEX FROM videos WHERE Key_name = 'idx_title_original'")->result();
            if(empty($indexes2)) {
                $this->db->query("ALTER TABLE `videos` ADD KEY `idx_title_original` (`title_original`)");
                echo "Đã tạo index cho title_original\n";
            }
            
            $fulltext_indexes = $this->db->query("SHOW INDEX FROM videos WHERE Key_name = 'idx_title_search_fulltext'")->result();
            if(empty($fulltext_indexes)) {
                $this->db->query("ALTER TABLE `videos` ADD FULLTEXT KEY `idx_title_search_fulltext` (`title`, `title_vietnamese`, `title_vietnamese_no_accent`, `title_original`)");
