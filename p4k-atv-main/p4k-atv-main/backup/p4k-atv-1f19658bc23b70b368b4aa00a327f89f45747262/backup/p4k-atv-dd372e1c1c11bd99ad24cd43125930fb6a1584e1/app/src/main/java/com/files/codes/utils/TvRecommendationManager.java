...
public class TvRecommendationManager {
    ...
    public void addToContinueWatching(VideoContent content, long currentPosition, long duration) {
        try {
            ...
            // Set poster art if available
            if (content.getPosterUrl() != null && !content.getPosterUrl().isEmpty()) {
                builder.setPosterArtUri(Uri.parse(content.getPosterUrl()));
            }
    ...
    private long createRecommendationChannel() {
        try {
            ...
            if (!isTvProviderAvailable()) {
                Log.w(TAG, "TV Provider not available, skipping channel creation");
                return -1;
            }
            ...
            if (channelUri != null) {
                ...
                // IMPORTANT: Publish channel to make it visible on Android TV home screen
                ContentValues values = new ContentValues();
                values.put(TvContractCompat.Channels.COLUMN_BROWSABLE, 1);
                context.getContentResolver().update(channelUri, values, null, null);
                Log.d(TAG, "Created and published recommendation channel: " + channelId);
                return channelId;
            }
    ...
    private VideoContent convertSyncItemToVideoContent(WatchHistorySyncItem.WatchHistoryItem item) {
        try {
            ...
            content.setIsTvseries(item.isMovie() ? "0" : "1");
